...
fn main() {
    ...
    println!("WALL: nodes={} bonds={} support={} dynamic={}", wall.nodes.len(), wall.bonds.len(), support, dynamic);
    if !wall.nodes.is_empty() {
        let first = &wall.nodes[0];
        let last = &wall.nodes[wall.nodes.len()-1];
        println!("  first_centroid=({:.4}, {:.4}, {:.4})", first.centroid.x, first.centroid.y, first.centroid.z);
        println!("  last_centroid=({:.4}, {:.4}, {:.4})", last.centroid.x, last.centroid.y, last.centroid.z);
        println!("  first_bond_area={:.6}", wall.bonds[0].area);
        println!("  last_bond_area={:.6}", wall.bonds[wall.bonds.len()-1].area);
    }
    ...
    println!("TOWER: nodes={} bonds={} support={} dynamic={}", tower.nodes.len(), tower.bonds.len(), t_support, t_dynamic);
    ...
    println!("BRIDGE: nodes={} bonds={} support={} dynamic={}", bridge.nodes.len(), bridge.bonds.len(), b_support, b_dynamic);
}
