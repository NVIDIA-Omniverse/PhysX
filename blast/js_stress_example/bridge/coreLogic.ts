...
import type { BridgeCore, ProcessFracturesResult, SplitEvent } from './types.js';

export function applyForcesAndSolve(bridge: BridgeCore): void {
  const { solver } = bridge;
...
export function processSolverFractures(
  bridge: BridgeCore,
  _RAPIER: typeof import('@dimforge/rapier3d-compat').default,
  _contactForceThreshold = 0.0
): ProcessFracturesResult | null {
  const { solver } = bridge;
...
export function normalizeSplitResults(splitResults: SplitEvent[]): SplitEvent[] {
...
export function drainContactForces(bridge: BridgeCore): void {
  ...
  eventQueue.drainContactForceEvents((event) => {
    ...
    const register = (handle: number | undefined, direction: number): void => {
      ...
      if (parentHandle === undefined || parentHandle === null) return;
...
// Two-phase contact drain (split-bridge lifecycle):
// 1) Collect raw event payloads inside the Rapier drain callback (NO world access)
// 2) Accumulate into pendingContactForces outside of the drain (safe to touch the world)

export type CollectedEvent = {
  c1?: number;
  c2?: number;
  totalForce?: { x: number; y: number; z: number };
  worldPoint?: { x: number; y: number; z: number } | undefined;
  worldPoint2?: { x: number; y: number; z: number } | undefined;
  totalImpulse?: { x: number; y: number; z: number } | undefined;
};

export function collectContactForceEvents(eventQueue: any): CollectedEvent[] {
  const out: CollectedEvent[] = [];
  if (!eventQueue || typeof eventQueue.drainContactForceEvents !== 'function') return out;

  eventQueue.drainContactForceEvents((event: any) => {
    if (!event) return;
    const totalForce = event.totalForce?.();
    const magnitude = event.totalForceMagnitude?.();
    if (!totalForce || !(magnitude > 0)) return;
...
export function accumulateContactForcesFromEvents(bridge: BridgeCore, events: CollectedEvent[]): void {
  ...
  const quatApply = (v: { x: number; y: number; z: number }) => {
  ...
  const register = (handle: number | undefined, dir: 1 | -1, ev: CollectedEvent): void => {
...
export function gatherSolverForcesFromRapier(bridge: BridgeCore) {
  ...
  for (const handle of activeContactColliders) {
    ...
    if (!contact || nodeIndex === undefined || nodeIndex < 0) continue;
...
export function sweepBeforeEventfulStep(bridge: BridgeCore): void {
