...
export function spawnLoadVehicle(world, options = {}) {
  ...
  const body = world.createRigidBody(
    (bodyDesc ?? world.RAPIER.RigidBodyDesc.dynamic())
      .setTranslation(minX - margin, 2.0 + height * 0.5 + 0.1, 0)
      .setLinearDamping(0.4)
      .setAngularDamping(1.0)
  );
  const collider = world.createCollider(
    world.RAPIER.ColliderDesc.cuboid(length * 0.5, height * 0.5, width * 0.5)
      .setFriction(1.0)
      .setRestitution(0.0)
      .setMass(mass)
...
export function spawnProjectile(world, options = {}) {
  ...
  const collider = kind === 'ball'
    ...
    : world.createCollider(
        world.RAPIER.ColliderDesc.cuboid(0.6 * scale, 0.6 * scale, 0.6 * scale)
