...
import { loadStressSolver, ExtDebugMode } from './stress.js';
import { applyForcesAndSolve as coreApplyForcesAndSolve, processSolverFractures as coreProcessSolverFractures, collectContactForceEvents, accumulateContactForcesFromEvents, sweepBeforeEventfulStep } from './bridge/coreLogic.js';
import { applyPendingMigrations, pruneStaleHandles, enqueueSplitResults } from './bridge/rapierHierarchyApplier.js';
import { spawnLoadVehicle as spawnCar, spawnProjectile as spawnProj, updateProjectiles as updateProjList } from './bridge/dynamics.js';
...
async function init() {
    ...
    try {
        if (!bridge.eventQueue)
            bridge.eventQueue = new RAPIER.EventQueue(true);
    }
    catch { }
        ...
        if (isSafeFrame) {
            ...
            try {
                ...
                try {
                    ...
                    h.lastError = h.lastError ?? (err?.stack ?? String(err));
                }
                catch { }
            }
...
function initThree() {
    ...
    scene.add(dir);
    const ground = new THREE.Mesh(new THREE.BoxGeometry(40, 1, 40), new THREE.MeshStandardMaterial({ color: 0x1a1e2f, roughness: 0.8, metalness: 0.1 }));
...
function buildBridge(scene, world, runtime) {
    ...
    core.chunks.forEach((chunk) => {
        ...
        chunk.mesh = mesh;
        chunk.baseWorldPosition = new THREE.Vector3(chunk.baseLocalOffset.x, chunk.baseLocalOffset.y, chunk.baseLocalOffset.z);
    ...
    world.createCollider(RAPIER.ColliderDesc.cuboid(20, 0.5, 20).setTranslation(0, -4, 0).setFriction(1.0).setRestitution(0.0), groundBody);
...
function resetChunkColors(bridge) {
    ...
    bridge.bonds.forEach((bond) => { if (bond)
        bond.severity = 0; });
}
...
function updateProjectiles(world, bridge, delta) {
    ...
    if (bridge.projectileColliderHandles && bridge.activeContactColliders) {
        ...
        bridge.projectiles.forEach((p) => { if (p?.colliderHandle != null)
            stillAlive.add(p.colliderHandle); });
        const toDelete = [];
...
function fireProjectile(world, bridge) {
    ...
    if (bridge.activeContactColliders)
        bridge.activeContactColliders.add(p.colliderHandle);
}
// --------------------------- UI / HUD ---------------------------
