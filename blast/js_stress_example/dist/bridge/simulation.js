...
export function updateBridge(world, bridge, delta) {
    ...
    bonds.forEach((bond, index) => {
        ...
        const stress = computeBondStress(stressProcessor.bondDesc(index), impulses[index], solverNodes, bond.area);
...
export function splitChunk(world, bridge, chunk) {
    ...
    if (Number.isFinite(parentAngVel.x) &&
...
function syncMeshes(world, bridge) {
    bridge.chunks.forEach((chunk) => {
        ...
        const severity = Math.max(chunk.severity?.max ?? 0, ...bridge.bonds
