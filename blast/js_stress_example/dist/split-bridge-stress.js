...
import { loadStressSolver, ExtForceMode, ExtDebugMode // (unused here, handy if you want to add a stress overlay)
 } from './stress.js';
import { createBridgeCore } from './bridge/buildBridge.headless.js';
import { buildBridgeScenario } from './extBridgeScenario.js';
import { ConfigManager, DEFAULT_CONFIG, CONFIG_DESCRIPTORS, saveConfigToStorage, loadConfigFromStorage, clearConfigFromStorage } from './config.js';
/**
...
async function init() {
    ...
    canvas.addEventListener('pointermove', (ev) => {
        ...
        if (state.aimMarker)
            state.aimMarker.visible = wasVisible;
        if (!hitPoint) {
...
function setupConfigurationUI() {
    ...
    const updateDisplayValue = (elementId, value) => {
    ...
    const toggleInputMode = (sliderId) => {
        try {
            ...
            if (slider) {
                ...
                if (toggleBtn && toggleBtn.parentElement) {
                    inputGroup = toggleBtn.parentElement;
                }
                else {
            ...
            const valueSpan = inputGroup.querySelector('.config-value');
            const toggleBtn = inputGroup.querySelector('.config-toggle');
            if (!toggleBtn) {
    ...
    const createInputHandler = (category, key, displayId, isImmediate) => {
        return (e) => {
    ...
    for (const [category, keys] of Object.entries(CONFIG_DESCRIPTORS)) {
        for (const [key, descriptor] of Object.entries(keys)) {
            const inputId = `config-${category}-${key}`;
            const displayId = `config-${category}-${key}-value`;
            const input = document.getElementById(inputId);
            if (!input)
                continue;
            const isImmediate = descriptor.immediate ?? false;
            // Attach event listener (use 'change' for select, 'input' for others)
            const eventType = input.tagName === 'SELECT' ? 'change' : 'input';
            input.addEventListener(eventType, createInputHandler(category, key, displayId, isImmediate));
        }
    }
    ...
    for (const [category, keys] of Object.entries(CONFIG_DESCRIPTORS)) {
        for (const [key] of Object.entries(keys)) {
            ...
            if (!input)
                continue;
            const currentValue = currentConfig[category][key];
...
function updatePendingIndicator() {
    ...
    if (!indicator || !state.configManager)
        return;
...
function buildGround(scene, world) {
    ...
    const mesh = new THREE.Mesh(groundGeometry, new THREE.MeshStandardMaterial(groundMaterialOptions));
    ...
    world.createCollider(RAPIER.ColliderDesc.cuboid(30, 0.5, 30).setTranslation(groundPosition.x, groundPosition.y, groundPosition.z), body);
...
function buildBridge(scene, world) {
    ...
    core.chunks.forEach((chunk) => {
        ...
        chunk.baseLocalOffset = new THREE.Vector3(chunk.baseLocalOffset.x, chunk.baseLocalOffset.y, chunk.baseLocalOffset.z);
        chunk.localOffset = new THREE.Vector3(chunk.localOffset.x, chunk.localOffset.y, chunk.localOffset.z);
        // For support piers, also convert baseWorldPosition to THREE.Vector3
        if (chunk.isSupport && chunk.baseWorldPosition) {
            chunk.baseWorldPosition = new THREE.Vector3(chunk.baseWorldPosition.x, chunk.baseWorldPosition.y, chunk.baseWorldPosition.z);
        }
        scene.add(mesh);
    });
...
function loop() {
    ...
    if (state.safeFrames > 0) {
        ...
        updateStatus();
        if (state.debugRenderer)
            state.debugRenderer.update();
        try {
            if (state.debugRenderer?.enabled)
                renderDebugLines();
            else if (state.debugHelper)
                updateDebugLineObject(state.debugHelper, [], false);
        }
        catch { }
        controls.update();
...
function drainContactsAndApplyForces() {
    ...
    const rq = new THREE.Quaternion(rootBody.rotation().x, rootBody.rotation().y, rootBody.rotation().z, rootBody.rotation().w);
    ...
    const add = (handle, direction, totalForce, worldPoint) => {
        ...
        // console.log('Adding force to collider:', handle, direction, totalForce, worldPoint);
        const f = new THREE.Vector3((totalForce.x ?? 0) * direction, (totalForce.y ?? 0) * direction, (totalForce.z ?? 0) * direction);
    ...
    acc.forEach((val, handle) => {
        ...
        state.solver.addForce(nodeIndex, { x: localPoint.x, y: localPoint.y, z: localPoint.z }, { x: localForce.x, y: localForce.y, z: localForce.z }, ExtForceMode.Force);
    });
}
...
function queueSplitResults(splitEvents) {
    ...
    for (const evt of splitEvents) {
        ...
        for (const child of children) {
            ...
            const isActorSupport = child.nodes.some(nodeIndex => {
...
function applyPendingSpawns() {
    ...
    if (pendingBallSpawns.length === 0)
        return;
    const batch = pendingBallSpawns.splice(0, pendingBallSpawns.length);
    for (const s of batch)
        spawnBallNow(s.x, s.z);
}
function spawnBallNow(x, z, type = 'box') {
    ...
    const mesh = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({ color: 0xff9147, emissive: 0x331100 }));
...
function applyPendingMigrations() {
    ...
    if (pendingColliderMigrations.length > 0) {
        ...
        for (const mig of jobs) {
            ...
            const col = world.createCollider(R.ColliderDesc.cuboid(halfX, halfY, halfZ)
    ...
    currentBodies.forEach((bInfo) => {
        ...
        if (colliders.length > 0) {
            colliders.forEach((cInfo, idx) => {
                ...
                if (cInfo.isNew)
                    status = '✨ NEW';
                else if (cInfo.wasMoved)
                    status = `🔄 MOVED from ${cInfo.prevParentName}`;
                else
                    status = '✅';
                ...
                if (cInfo.segmentIndex != null)
                    console.log('├─ Segment Index:', cInfo.segmentIndex);
                if (cInfo.nodeIndex != null)
                    console.log('├─ Node Index:', cInfo.nodeIndex);
...
function updateStatus() {
    ...
    if (segmentCountEl)
        segmentCountEl.textContent = String(attachedCount);
    if (detachedCountEl)
        detachedCountEl.textContent = String(detachedCount);
    if (ballCountEl)
        ballCountEl.textContent = String(ballCount);
    if (rigidbodyCountEl)
        rigidbodyCountEl.textContent = String(rigidbodyCount);
    if (solverActorsEl)
        solverActorsEl.textContent = String(solverActorCount);
    if (solverNodesEl)
        solverNodesEl.textContent = String(solverNodeCount);
    if (solverOverstressedEl)
        solverOverstressedEl.textContent = String(overstressed);
    if (solverBondsEl)
        solverBondsEl.textContent = String(state.solver ? state.solver.bondCapacity() : 0);
    if (solverErrLinEl)
        solverErrLinEl.textContent = stressErr.lin.toFixed(5);
    if (solverErrAngEl)
        solverErrAngEl.textContent = stressErr.ang.toFixed(5);
    if (solverConvergedEl)
        solverConvergedEl.textContent = converged ? '(converged)' : '';
}
    ...
    for (const b of balls) {
        ...
        if (t.y < -50) {
            ...
            for (let i = 0; i < body.numColliders(); i++) {
                const cHandle = body.collider(i)?.handle;
                if (cHandle != null)
                    colliderMetadata.delete(cHandle);
            }
        }
        else {
            keep.push(b);
...
function debugPrintSolver() {
    ...
    try {
        ...
        if (top.length) {
            ...
            const fmt = (p) => (p * 100).toFixed(1).padStart(6);
            top.forEach((t, i) => {
...
function stressPctFromColor(color) {
    // Detect unbreakable color (approx 0xFF00AEFF) — treat as 0
    if ((color & 0x00FFFFFF) === 0x00AEFF)
        return 0.0;
