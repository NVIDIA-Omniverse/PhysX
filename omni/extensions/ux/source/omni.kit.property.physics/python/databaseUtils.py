...
# FIXME: this needs to refreshed every time a new schema is loaded at runtime
_derived_joints_type_names = None


def _is_a_base_joint(p):
    global _derived_joints_type_names
    if _derived_joints_type_names is None:
        _derived_joints_type_names = {
            *[Usd.SchemaRegistry().GetSchemaTypeName(prim) for prim in physxUtils.getDerivedSchemas(UsdPhysics.Joint)]
        }
...
def patch_refresh_cache_to_prim(p, lmt=100000):
    ...
    is_a_base_joint = _is_a_base_joint(p)
    canadd_filteredpairs = (
        ...
        has_api("OmniPhysicsDeformableBodyAPI", schemas) or
