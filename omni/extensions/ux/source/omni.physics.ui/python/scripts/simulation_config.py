    ...
    def _build_menu(self, args):
        def toggle_simulator(id, enable):
            ...
            if enable:
                ...
                physics.deactivate_simulation(id)

        current = "None"
        for simulator in self._simulators:
            ui.MenuItem(simulator.name, checkable=True, checked=simulator.active, hide_on_click=False, triggered_fn=partial(toggle_simulator, simulator.id, not simulator.active))
            if simulator.active:
                if current != "None":
                    current = "Multiple"
                else:
