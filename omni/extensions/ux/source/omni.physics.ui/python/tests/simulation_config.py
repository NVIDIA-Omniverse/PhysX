...
from ..scripts.simulation_config import SETTING_PHYSICS_SIMULATOR_TOGGLE_EXCLUSIVE
...
class SimulationConfigTests(OmniUiTest):
    ...
    async def test_simulator_toggle(self):
        ...
        exclusive_orig = settings.get_as_bool(SETTING_PHYSICS_SIMULATOR_TOGGLE_EXCLUSIVE)
        settings.set_bool(SETTING_PHYSICS_SIMULATOR_TOGGLE_EXCLUSIVE, False)
        ...
        await emulate_mouse_move_and_click(Vec2(40, 80))
        await run_sub_test("deactivate")

        await emulate_mouse_move_and_click(Vec2(40, 80))
        await run_sub_test("activate")

        settings.set_bool(SETTING_PHYSICS_SIMULATOR_TOGGLE_EXCLUSIVE, True)

        # Deactivate then reactivate with the setting enabled.
        await emulate_mouse_move_and_click(Vec2(40, 80))
        await emulate_mouse_move_and_click(Vec2(40, 80))
        await run_sub_test("activate_exclusive")
        ...
        settings.set_bool(SETTING_PHYSICS_SIMULATOR_TOGGLE_EXCLUSIVE, exclusive_orig)
