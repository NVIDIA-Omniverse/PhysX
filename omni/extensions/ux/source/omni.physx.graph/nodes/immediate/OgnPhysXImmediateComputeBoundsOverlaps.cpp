...
struct omni::physx::graph::ImmediateBVH
{
    ...
    std::vector<NameToken> mInputMeshPaths;
...
public:
    ...
    static bool compute(OgnPhysXImmediateComputeBoundsOverlapsDatabase& db)
    {
        ...
        for (size_t i = 0; i < childrenCount; ++i)
        {
            ...
            const auto pathAttr = extractedBundle.attributeByName(omni::fabric::Token::createImmortal("sourcePrimPath"));
            if (!pathAttr.isValid())
            {
                db.logError(
            ...
            auto path = pathAttr.getCpu<omni::fabric::Token>();
            if (!path)
            {
            ...
            if (ImmediateShared::parseBoundsFromAttributes(bounds, boxMinAttr, boxMaxAttr))
            {
                ...
                state.immediateBVH.mInputMeshPaths[i] = *path;
