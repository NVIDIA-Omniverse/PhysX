...
__all__ = [
    ...
    "register_property_display_group",
    "unregister_property_display_group",
    "register_display_group_order",
    "unregister_display_group_order",
    "register_display_group_title",
    "unregister_display_group_title",
    "refresh"
]
...
def register_property_display_group(prop_name: str, display_group: str, collapsed: bool = False):
    """Override the display group for a specific USD property name across all physics widgets."""
    database.register_property_display_group(prop_name, display_group, collapsed)
    database.register_display_group_title(display_group, display_group)
    if Manager.instance is not None:
        rebuild_property_window()


def unregister_property_display_group(prop_name: str):
    """Remove a display group override registered with register_property_display_group."""
    database.unregister_property_display_group(prop_name)
    if Manager.instance is not None:
        rebuild_property_window()


def register_display_group_title(name: str, title: str):
    """Register a verbatim display title for a group name, bypassing camel-case splitting."""
    database.register_display_group_title(name, title)
    if Manager.instance is not None:
        rebuild_property_window()


def unregister_display_group_title(name: str):
    """Remove a verbatim display title registered with register_display_group_title."""
    database.unregister_display_group_title(name)
    if Manager.instance is not None:
        rebuild_property_window()


def register_display_group_order(groups: list):
    """Set the preferred display order for named groups; unregistered groups follow after."""
    database.register_display_group_order(groups)
    if Manager.instance is not None:
        rebuild_property_window()


def unregister_display_group_order(groups: list):
    """Remove groups from the display order registry."""
    database.unregister_display_group_order(groups)
    if Manager.instance is not None:
        rebuild_property_window()
