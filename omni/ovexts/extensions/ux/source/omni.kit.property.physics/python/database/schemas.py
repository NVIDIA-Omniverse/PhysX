...
# prop_name -> (display_group, collapsed)
_property_display_groups: dict = {}

def register_property_display_group(prop_name: str, display_group: str, collapsed: bool = False):
    _property_display_groups[prop_name] = (display_group, collapsed)

def unregister_property_display_group(prop_name: str):
    _property_display_groups.pop(prop_name, None)

def get_property_display_group(prop_name: str):
    return _property_display_groups.get(prop_name)


_display_group_titles: dict = {}

def register_display_group_title(name: str, title: str):
    _display_group_titles[name] = title

def unregister_display_group_title(name: str):
    _display_group_titles.pop(name, None)

def get_display_group_title(name: str) -> str | None:
    return _display_group_titles.get(name)


_display_group_order: list = []

def register_display_group_order(groups: list):
    for g in groups:
        if g not in _display_group_order:
            _display_group_order.append(g)

def unregister_display_group_order(groups: list):
    for g in groups:
        if g in _display_group_order:
            _display_group_order.remove(g)

def get_display_group_order() -> list:
    return _display_group_order
