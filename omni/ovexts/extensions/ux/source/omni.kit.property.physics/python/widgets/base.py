...
class PhysicsWidget(UsdPropertiesWidget):
    ...
    def _filter_props_to_build(self, prim):
        ...
        for prop in filtered_props:
            override = database.get_property_display_group(prop.base_name)
            if override is not None:
                display_group, collapsed = override
                prop.metadata[Sdf.PropertySpec.DisplayGroupKey] = prop.display_group = display_group
                prop.display_group_collapsed = collapsed
    ...
    def _build_property_item(self, stage, prop, prim_paths):
        ...
        # sooner or later
        if prop.apply_schema is not None and hasattr(model, "_create_placeholder_attributes"):
    ...
    def _build_group_frame(self, name, props, stage):
        ...
        def get_title():
            if not is_instance:
                verbatim = database.get_display_group_title(name)
                return verbatim if verbatim is not None else display_name_from_name(name)
    ...
    def build_nested_group_frames(self, stage, grouped_props):
        ...
        group_order = database.get_display_group_order()
        if group_order:
            order_map = {name: i for i, name in enumerate(group_order)}
            max_idx = len(group_order)
            items = sorted(instances.items(), key=lambda kv: order_map.get(kv[0], max_idx))
        else:
