...
class VariantManager:
    ...
    def __init__(self):
        ...
        # Root-layer identifier of the stage the pending paths came from.
        # If the stage is swapped or closed between queue time and the
        # deferred process_resynced_paths call, the queue is dropped — looking
        # up paths from one stage against a different stage's instance cache
        # can crash inside Usd_InstanceCache (NvBugs 6187664).
        self._pending_resync_stage_id: Optional[str] = None
    ...
    def shutdown(self):
        ...
        self.clear_pending_resync_paths()
    ...
    def get_pending_resync_stage_id(self) -> Optional[str]:
        """Return the stage identifier the pending paths came from, or None."""
        return self._pending_resync_stage_id

    def clear_pending_resync_paths(self):
        """Drop all pending resync paths and forget their originating stage."""
        self._pending_resync_paths.clear()
        self._pending_resync_stage_id = None

    def add_pending_resync_paths(self, paths: List[Sdf.Path], stage_id: Optional[str] = None):
        """
        Add paths to the pending resync set.

        Args:
            paths: List of Sdf.Path objects to add for processing.
            stage_id: Optional identifier (root-layer identifier) of the stage
                these paths came from. If supplied and it differs from the
                stored stage id, the existing queue is dropped first — paths
                from a stale stage must not be looked up against a different
                stage (NvBugs 6187664). If None (e.g. in-place requeue/filter
                passes), the stored stage id is left untouched.
        """
        if stage_id is not None:
            if self._pending_resync_stage_id is None:
                self._pending_resync_stage_id = stage_id
            elif self._pending_resync_stage_id != stage_id:
                self._pending_resync_paths.clear()
                self._pending_resync_stage_id = stage_id
    ...
    def on_stage_closed(self):
        """Called when a stage is closed. Clears prim paths and pending resyncs but keeps registered variant names."""
        for variant_name in self._variant_prim_paths:
            self._variant_prim_paths[variant_name] = set()
        self.clear_pending_resync_paths()
    ...
    def process_resynced_paths(self, stage):
        ...
        # If the queued paths came from a different stage (swap mid-defer),
        # drop them — looking them up against this stage's instance cache can
        # crash inside Usd_InstanceCache (NvBugs 6187664).
        root_layer = stage.GetRootLayer()
        current_stage_id = root_layer.identifier if root_layer is not None else None
        if self._pending_resync_stage_id is not None \
                and self._pending_resync_stage_id != current_stage_id:
            self.clear_pending_resync_paths()
            return
