...
class VariantSwitcher:
    ...
    def __init__(self):
        ...
        # Track session layer variant opinions set by automatic switching
        # Set of (prim_path_str, variant_set_name) tuples
        self._session_layer_opinions: set = set()
        # Flag to suppress change handling during our own session layer edits
        self._suppress_change_handling: bool = False
    ...
    def _find_variant_by_substring(self, variant_name: str, available_variants: List[str], prim_path, variant_set_name: str) -> Optional[str]:
        """
        Find a variant in available_variants that contains variant_name as a substring.

        Args:
            variant_name: The variant name to search for (used as substring).
            available_variants: List of available variant names.
            prim_path: The prim path (for logging).
            variant_set_name: The variant set name (for logging).

        Returns:
            The matched variant name, or None if no match found.
        """
        matches = [v for v in available_variants if variant_name in v]
        if not matches:
            return None
        if len(matches) > 1:
            carb.log_info(
                f"VariantSwitcher: Multiple variants containing '{variant_name}' found in variant set "
                f"'{variant_set_name}' on prim '{prim_path}': {matches}, using the first one: '{matches[0]}'"
            )
        return matches[0]
    ...
    def switch_variants_for_simulation(self, stage, simulation_name: str, variant_set_name: str = "Physics") -> Tuple[int, List]:
        ...
        self._suppress_change_handling = True
        try:
            for prim_path in prim_paths:
                ...
                # Check if the variant exists in this variant set (substring match)
                available_variants = variant_set.GetVariantNames()
                matched_variant = self._find_variant_by_substring(variant_name, available_variants, prim_path, variant_set_name)
                if matched_variant is None:
                    continue

                # Skip if the target layer already has an opinion for this variant set
                target_prim_spec = target_layer.GetPrimAtPath(prim_path)
                if target_prim_spec and variant_set_name in target_prim_spec.variantSelections:
                    carb.log_info(
                        f"VariantSwitcher: Skipping '{prim_path}' - target layer already has opinion "
                        f"for variant set '{variant_set_name}'"
                    )
                    continue

                # Switch to the variant on the session layer
                current_variant = variant_set.GetVariantSelection()
                if current_variant != matched_variant:
                    prim_path_str = str(prim_path)
                    session_prim_spec = Sdf.CreatePrimInLayer(session_layer, prim_path)
                    session_prim_spec.variantSelections[variant_set_name] = matched_variant
                    self._session_layer_opinions.add((prim_path_str, variant_set_name))
                    switched_count += 1
                    switched_prim_paths.append(prim_path_str)
                    carb.log_info(
                        f"VariantSwitcher: Switched '{prim_path}' variant set '{variant_set_name}' "
                        f"from '{current_variant}' to '{matched_variant}' (on session layer)"
                    )
        finally:
            self._suppress_change_handling = False

        if switched_count > 0:
            carb.log_info(
                f"VariantSwitcher: Switched {switched_count} prim(s) to variant '{variant_name}' "
                f"for simulation '{simulation_name}' (on session layer)"
            )
    ...
    def shutdown(self):
        ...
        self._session_layer_opinions.clear()
    ...
    def _enable_variant_tracking(self):
        ...
        self._stage_event_subs = [
            ...
            for event_type, handler in (
                ...
                # Subscribe to both CLOSING and CLOSED with the same handler
                # so deferred work and queued resync paths are dropped while
                # the stage is still alive — once teardown starts, looking
                # those paths up via GetPrimAtPath can crash inside USD's
                # instance cache (NvBugs 6187664). CLOSED is kept for the
                # case where CLOSING isn't emitted; the handler is idempotent.
                (omni.usd.StageEventType.CLOSING, self._on_stage_closed),
    ...
    def _on_stage_closed(self, event):
        """Called when a stage is closing or has been closed. Idempotent so
        it can run on both CLOSING and CLOSED without harm."""
        # Cancel any pending resync update to avoid operating on stale data
        if self._update_sub is not None:
            self._update_sub = None
        # Clear session layer opinions (no stage to clean up, just clear tracking)
        self._session_layer_opinions.clear()
        if self._variant_manager:
            self._variant_manager.on_stage_closed()

    @staticmethod
    def _stage_identifier(stage) -> Optional[str]:
        """Stable identifier for a stage (root-layer identifier), or None."""
        if stage is None:
            return None
    ...
    def _switch_variants_for_paths(self, stage, resynced_paths: List) -> int:
            ...
            for variant_set_name in registered_variant_names:
                ...
                for prim_path in filtered_prim_paths:
                    ...
                    if current_variant != matched_variant:
                        ...
                        self._session_layer_opinions.add((prim_path_str, variant_set_name))
                        total_switched += 1
                        all_switched_paths.append(prim_path_str)
                        carb.log_info(
                            f"VariantSwitcher: Switched '{prim_path}' variant set '{variant_set_name}' "
                            f"from '{current_variant}' to '{matched_variant}' (on session layer)"
                        )
        ...
        if total_switched > 0:
            carb.log_info(
                ...
                f"for simulation '{sim_name}' (resynced paths only, on session layer)"
            )
    ...
    def _clear_session_opinions_for_paths(self, stage, resynced_paths):
        """
        Clear session layer variant opinions for prims at or under the given paths.

        When a change is detected (e.g., user edits the variant on the target layer),
        we remove our session layer override so the target layer opinion takes effect.

        Args:
            stage: The USD stage.
            resynced_paths: List of Sdf.Path objects that were resynced.
        """
        if not stage or not self._session_layer_opinions or not resynced_paths:
            return

        session_layer = stage.GetSessionLayer()
        opinions_to_remove = set()

        for prim_path_str, variant_set_name in self._session_layer_opinions:
            prim_path = Sdf.Path(prim_path_str)
            for resynced_path in resynced_paths:
                if prim_path == resynced_path or prim_path.HasPrefix(resynced_path):
                    session_prim_spec = session_layer.GetPrimAtPath(prim_path)
                    if session_prim_spec and variant_set_name in session_prim_spec.variantSelections:
                        self._suppress_change_handling = True
                        try:
                            del session_prim_spec.variantSelections[variant_set_name]
                            carb.log_info(
                                f"VariantSwitcher: Cleared session layer opinion for '{prim_path}' "
                                f"variant set '{variant_set_name}'"
                            )
                        finally:
                            self._suppress_change_handling = False
                    opinions_to_remove.add((prim_path_str, variant_set_name))
                    break

        self._session_layer_opinions -= opinions_to_remove

    def _clear_all_session_opinions(self, stage):
        """
        Clear all session layer variant opinions that were set by automatic switching.

        Args:
            stage: The USD stage (may be None on stage close).
        """
        if stage and self._session_layer_opinions:
            session_layer = stage.GetSessionLayer()
            self._suppress_change_handling = True
            try:
                for prim_path_str, variant_set_name in self._session_layer_opinions:
    ...
    def _on_objects_changed(self, notice, sender):
        ...
        # Ignore changes caused by our own session layer edits
        if self._suppress_change_handling:
            return
        ...
        # Normalize each entry to a prim path (drops property suffixes, matches
        # the idiom used by other resync handlers in this repo such as
        # omni.usdphysics.ui.physicsViewportOverlayManager). Tag with the
        # originating stage so a stage swap before the deferred process call
        # invalidates the queue (NvBugs 6187664).
        resynced_prim_paths = []
        for path in notice.GetResyncedPaths():
            prim_path = path.GetPrimPath()
            if not prim_path.IsAbsoluteRootOrPrimPath():
                continue
            resynced_prim_paths.append(prim_path)

        if resynced_prim_paths:
            self._variant_manager.add_pending_resync_paths(
                resynced_prim_paths, stage_id=self._stage_identifier(stage)
            )
    ...
    def _on_update(self, event):
        ...
        if self._variant_manager and self._usd_context:
            ...
            # If the stage was swapped or closed between queue time and now,
            # drop the queued paths instead of looking them up against a
            # different (or torn-down) stage — that races with USD's instance
            # cache and crashes (NvBugs 6187664).
            pending_stage_id = self._variant_manager.get_pending_resync_stage_id()
            if pending_stage_id is not None \
                    and pending_stage_id != self._stage_identifier(stage):
                self._variant_manager.clear_pending_resync_paths()
                return

            # Clear session layer opinions for resynced paths - a user change on the
            # target layer should take precedence over our automatic session layer switch
            if stage is not None and self._variant_manager.has_pending_resync_paths():
                pending_for_clearing = list(self._variant_manager._pending_resync_paths)
                self._clear_session_opinions_for_paths(stage, pending_for_clearing)
