...
class DeformableSchemaChecker(BaseRuleChecker):
    ...
    def clear():
        ...
        DeformableSchemaChecker.physxSceneGpuMaxSoftBody_paths = set()
    ...
    def convert_stage(stage: Usd.Stage, location: Usd.Prim):
        ...
        for prim in traversal:
            ...
            attr = prim.GetAttribute("physxScene:gpuMaxSoftBodyContacts")
            if attr and attr.HasAuthoredValue():
                DeformableSchemaChecker.physxSceneGpuMaxSoftBody_paths.add(prim.GetPath())
        ...
        for path in DeformableSchemaChecker.physxSceneGpuMaxSoftBody_paths:
            prim = stage.GetPrimAtPath(path)
            if prim:
                old_attr = prim.GetAttribute("physxScene:gpuMaxSoftBodyContacts")
                if old_attr and old_attr.HasAuthoredValue():
                    new_attr = prim.GetAttribute("physxScene:gpuMaxDeformableVolumeContacts")
                    if not new_attr or not new_attr.HasAuthoredValue():
                        new_attr = prim.CreateAttribute("physxScene:gpuMaxDeformableVolumeContacts", old_attr.GetTypeName())
                        new_attr.Set(old_attr.Get())
                    prim.RemoveProperty("physxScene:gpuMaxSoftBodyContacts")
    ...
    def CheckStage(self, stage: Usd.Stage):
        ...
        removed_paths = []
        removed_paths.extend(_get_prim_paths_with_specified_api(stage, "PhysxParticleClothAPI"))
        removed_paths.extend(_get_prim_paths_with_specified_api(stage, "PhysxDeformableBodyAPI"))
        removed_paths.extend(_get_prim_paths_with_specified_api(stage, "PhysxDeformableBodyMaterialAPI"))
        removed_paths.extend(_get_prim_paths_with_specified_api(stage, "PhysxDeformableSurfaceAPI"))
        removed_paths.extend(_get_prim_paths_with_specified_api(stage, "PhysxDeformableSurfaceMaterialAPI"))
        removed_paths.extend(_get_prim_paths_with_specified_type_name(stage, "PhysxPhysicsAttachment"))
        removed_paths.extend(_get_prim_paths_with_specified_type_name(stage, "TetrahedralMesh"))

        defaultPath = defaultPrim.GetPath()
        removed_paths[:] = [p for p in removed_paths if Sdf.Path(p.GetText()).HasPrefix(defaultPath)]

        has_gpu_max_soft_body = False
        for prim in Usd.PrimRange(defaultPrim, Usd.TraverseInstanceProxies(Usd.PrimAllPrimsPredicate)):
            attr = prim.GetAttribute("physxScene:gpuMaxSoftBodyContacts")
            if attr and attr.HasAuthoredValue():
                has_gpu_max_soft_body = True
                break

        need_fix = len(removed_paths) > 0 or has_gpu_max_soft_body
        if need_fix: 
            self._AddFailedCheck(
                message="There are removed and unsupported particle cloth/deformable body/deformable surface/attachment schemas or attributes, you can update to use the new schemas.",
                at=defaultPrim,
