...
extern PXR_NS::UsdStageRefPtr gStage;
...
bool CameraManager::addFollowLookCamera(const PXR_NS::SdfPath& subjectPath, const PXR_NS::SdfPath& cameraPath)
{
    ...
    PXR_NS::UsdGeomCamera usdCamera = PXR_NS::UsdGeomCamera::Define(gStage, cameraPath);
    ...
    PXR_NS::GfVec3f positionOffset = { 0.0f * lengthScale, 0.0f * lengthScale, 0.0f * lengthScale };
    PXR_NS::GfVec3f cameraPositionTC;
    PXR_NS::GfVec3f lookPositionTC;

    TfToken usdUpAxis = PXR_NS::UsdGeomGetStageUpAxis(gStage);
...
bool CameraManager::addFollowVelocityCamera(const PXR_NS::SdfPath& subjectPath, const PXR_NS::SdfPath& cameraPath)
{
    if (gStage->GetPrimAtPath(cameraPath))
    {
    ...
    PXR_NS::GfVec3f positionOffset = { 0.0f * lengthScale, 0.0f * lengthScale, 0.0f * lengthScale };
    PXR_NS::GfVec3f cameraPositionTC = { 2.0f, 2.0f, 2.0f };
    PXR_NS::GfVec3f lookPositionTC = { 0.1f, 0.1f, 0.1f };
...
bool CameraManager::addDroneCamera(const PXR_NS::SdfPath& subjectPath, const PXR_NS::SdfPath& cameraPath)
{
    if (gStage->GetPrimAtPath(cameraPath))
    {
...
CameraController* CameraManager::getCamera(const PXR_NS::UsdPrim& prim)
{
...
void CameraManager::createControllers()
{
    if (gStage && gPhysXInterface)
    {
        ...
        PXR_NS::UsdPrimRange range = gStage->Traverse();

        PXR_NS::SdfPathVector cameraPaths;
        ...
        for (const PXR_NS::SdfPath& path : cameraPaths)
        {
...
void CameraManager::addCameraController(const PXR_NS::SdfPath& cameraPath)
{
    // Skip if a controller for this camera already exists to avoid re-initialization
    // (which would reset the camera position and leak the old controller).
    if (mCameraControllerMap.find(cameraPath) != mCameraControllerMap.end())
    {
        return;
    }

    const PXR_NS::UsdPrim& prim = gStage->GetPrimAtPath(cameraPath);
    ...
    if (paths.size() == 1)
    {
        const PXR_NS::UsdPrim& subjectPrim = gStage->GetPrimAtPath(paths[0]);
...
void CameraManager::removeCameraController(const PXR_NS::SdfPath& cameraPath, bool removeReferences)
{
    ...
    if (cameraIter != mCameraControllerMap.end())
    {
        if (removeReferences)
        {
            PXR_NS::SdfPath subjectPath = cameraIter->second->getSubjectPath();
...
bool CameraManager::addPrimInternal(const PXR_NS::UsdPrim& prim)
{
    if (!prim)
        return true;
...
void CameraManager::addPrim(const PXR_NS::UsdPrim& prim)
{
    ...
    else
    {
        ...
        if (!range.empty())
        {
            PXR_NS::UsdPrimRange::const_iterator iter = range.begin();
            ++iter;  // the prim itself is part of the range
            for (; iter != range.end(); ++iter)
            {
                const PXR_NS::UsdPrim& p = *iter;                
...
void CameraManager::removePrim(const PXR_NS::SdfPath& primPath)
{
...
void CameraManager::setActiveCameraPath(PXR_NS::SdfPath activeCameraPath)
{
    mActiveCameraPath = activeCameraPath;
}

PXR_NS::SdfPath CameraManager::getActiveCameraPath()
{
    ...
    return PXR_NS::SdfPath(mCameraPath);
}
...
void CameraManager::preUpdate()
{
    ...
    PXR_NS::SdfPath activeCameraPath = getActiveCameraPath();
...
void CameraManager::onUsdObjectChange(const PXR_NS::SdfPath& path, const PXR_NS::UsdTimeCode& timeCode)
{
    ...
    if (path.IsPropertyPath())
    {
        ...
        if (!UsdGeomXformable::IsTransformationAffectedByAttrNamed(attrName) &&
            attrName != PXR_NS::UsdPhysicsTokens.Get()->physicsVelocity &&
            attrName != PXR_NS::UsdPhysicsTokens.Get()->physicsAngularVelocity)
        {
