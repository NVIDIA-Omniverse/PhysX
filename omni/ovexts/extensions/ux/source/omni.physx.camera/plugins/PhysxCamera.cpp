...
public:
    ...
    bool isInPrimAddMap(const PXR_NS::UsdPrim&) const;
...
void PrimUpdateMap::addPrim(const PXR_NS::UsdPrim& prim)
{
...
void PrimUpdateMap::removePrim(const PXR_NS::SdfPath& primPath)
{
...
{
    return gCameraManager->addFollowLookCamera(PXR_NS::SdfPath(subjectPath), PXR_NS::SdfPath(cameraPath));
}

bool addFollowVelocityCamera(const char* subjectPath, const char* cameraPath)
{
    return gCameraManager->addFollowVelocityCamera(PXR_NS::SdfPath(subjectPath), PXR_NS::SdfPath(cameraPath));
}

bool addDroneCamera(const char* subjectPath, const char* cameraPath)
{
    return gCameraManager->addDroneCamera(PXR_NS::SdfPath(subjectPath), PXR_NS::SdfPath(cameraPath));
}
...
bool attachStage(long int stageId, bool unregFromStageUpdate)
{
    ...
    if (stageId)
    {
        ...
        if (stage)
        {
            ...
            gCameraManager->setActiveCameraPath(PXR_NS::SdfPath("TestCamera"));
