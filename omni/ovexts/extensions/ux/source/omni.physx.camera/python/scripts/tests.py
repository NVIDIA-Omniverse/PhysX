...
class PhysXCameraTestKitStage(PhysicsKitStageAsyncTestCase, PhysXCameraTestBase):
    ...
    async def test_vehicle_cameras(self):
        ...
        # Regression check (bug 5867108): camera must not be at the world origin immediately
        # after simulation starts, before any timestep has been taken.
        for cameraPrim, label in [(followCameraPrim, "follow-look"), (droneCameraPrim, "drone")]:
            initPos = UsdGeom.Xformable(cameraPrim).ComputeLocalToWorldTransform(0).ExtractTranslation()
            self.assertGreater(
                (initPos - Gf.Vec3d(0, 0, 0)).GetLength(), 1.0,
                f"{label} camera initialized at origin {initPos}"
            )
    ...
    #
    # Regression test for bug 5867108: drone camera with a non-rigid-body subject must not
    # teleport to the world origin (0,0,0) when simulation starts.
    #
    async def test_drone_camera_non_rigid_body_initial_position(self):
        stage = await self.new_stage()

        # Plain cube with no PhysicsRigidBodyAPI — exercises the animated subject path.
        subjectPath = "/World/Cube"
        cubeXform = UsdGeom.Xform.Define(stage, subjectPath)
        cubeXform.AddTranslateOp().Set(Gf.Vec3d(5.0, 1.0, 3.0))

        cameraPath = "/World/DroneCamera"
        UsdGeom.Camera.Define(stage, cameraPath)
        cameraPrim = stage.GetPrimAtPath(cameraPath)

        droneCameraApi = PhysxSchema.PhysxCameraDroneAPI.Apply(cameraPrim)
        cameraApi = PhysxSchema.PhysxCameraAPI(cameraPrim)
        cameraApi.GetPhysxCameraSubjectRel().ClearTargets(True)
        cameraApi.GetPhysxCameraSubjectRel().AddTarget(subjectPath)

        followHeight = 15.0
        followDistance = 30.0
        droneCameraApi.CreateFollowHeightAttr().Set(followHeight)
        droneCameraApi.CreateFollowDistanceAttr().Set(followDistance)
        droneCameraApi.CreateMaxDistanceAttr().Set(100.0)
        droneCameraApi.CreateMaxSpeedAttr().Set(20.0)
        droneCameraApi.CreateHorizontalVelocityGainAttr().Set(1.0)
        droneCameraApi.CreateVerticalVelocityGainAttr().Set(1.0)
        ...
        droneCameraApi.CreateVelocityFilterTimeConstantAttr().Set(1.0)
        droneCameraApi.CreateRotationFilterTimeConstantAttr().Set(0.2)
        droneCameraApi.CreatePositionOffsetAttr().Set(Gf.Vec3f(0, 0, 0))

        # Start simulation — this calls addCameraController which runs stepUpdate(0) + postUpdate,
        # writing the initial camera position to USD before any real timestep.
        self._prepare_for_simulation()

        # Read back the camera position immediately — no simulation steps taken yet.
        initPos = UsdGeom.Xformable(cameraPrim).ComputeLocalToWorldTransform(0).ExtractTranslation()

        # Without the fix the camera is written to (0,0,0) by the identity transform returned
        # from updateTransform(0). With the fix it must be near the follow position above the subject.
        self.assertGreater(
            (initPos - Gf.Vec3d(0, 0, 0)).GetLength(), 1.0,
            f"Drone camera initialized at origin {initPos}; expected follow position near subject"
        )

        # Also sanity-check the height component: camera should be above the subject.
        subjectHeight = 1.0  # cubeXform Y translate
        self.assertGreater(initPos[1], subjectHeight,
            f"Drone camera Y={initPos[1]} is not above subject Y={subjectHeight}")
