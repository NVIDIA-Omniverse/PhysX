...
CharacterController* CctManager::getCct(const PXR_NS::SdfPath& path)
{
...
void CctManager::activateCct(const PXR_NS::SdfPath& path)
{
    getCct(path);
    mDirty = true;
}

void CctManager::removeCct(const PXR_NS::SdfPath& path)
{
    mCharacterControllerMap.erase(path);
    mDirty = true;
}

void CctManager::enableGravity(const PXR_NS::SdfPath& path)
{
    getCct(path)->enableGravity();
}

void CctManager::enableCustomGravity(const PXR_NS::SdfPath& path, const carb::Double3& gravity)
{
    getCct(path)->enableCustomGravity(PxVec3((float)gravity.x, (float)gravity.y, (float)gravity.z));
}

void CctManager::disableGravity(const PXR_NS::SdfPath& path)
{
    getCct(path)->disableGravity();
}

bool CctManager::hasGravityEnabled(const PXR_NS::SdfPath& path)
{
    return getCct(path)->hasGravityEnabled();
}

float CctManager::getControllerHeight(const PXR_NS::SdfPath& path)
{
    return getCct(path)->getHeight();
}

void CctManager::setControllerHeight(const PXR_NS::SdfPath& path, float height)
{
    getCct(path)->setHeight(height);
}

void CctManager::enableFirstPerson(const PXR_NS::SdfPath& path, const PXR_NS::SdfPath& cameraPath)
{
    getCct(path)->enableFirstPerson(cameraPath);
    mDirty = true;
}

void CctManager::disableFirstPerson(const PXR_NS::SdfPath& path)
{
    getCct(path)->disableFirstPerson();
    mDirty = true;
}

void CctManager::controllerDirty(const PXR_NS::SdfPath& path)
{
...
void CctManager::setPosition(const PXR_NS::SdfPath& path, const carb::Double3& pos)
{
...
void CctManager::enableWorldSpaceMove(const PXR_NS::SdfPath& path, bool enable)
{
    getCct(path)->enableWorldSpaceMove(enable);
}

void CctManager::setMove(const PXR_NS::SdfPath& path, const carb::Float3& displacement)
{
    getCct(path)->setMove(PXR_NS::GfVec3f(displacement.x, displacement.y, displacement.z));
}
