...
CharacterController::CharacterController(const PXR_NS::SdfPath& path)
    : mUsdPath(path)
...
void CharacterController::getLocalMoveFrame(PxVec3& fwd, PxVec3& right, const unsigned char stageUpAxisIndex)
{
    ...
    else
    {
        ...
        const PXR_NS::GfMatrix3d rm = tm.GetOrthonormalized().ExtractRotation();

        PXR_NS::GfVec3d gfForward(0), gfRight(0);
...
void CharacterController::setMove(const PXR_NS::GfVec3f& displ)
{
    PhysxSchemaPhysxCharacterControllerAPI cctAPI = PhysxSchemaPhysxCharacterControllerAPI(mUsdPrim);
    if (cctAPI)
    {
