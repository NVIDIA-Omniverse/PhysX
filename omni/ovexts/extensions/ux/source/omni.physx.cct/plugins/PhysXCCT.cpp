...
void setPosition(const char* path, const carb::Double3& pos)
{
    ...
    gCctManager->setPosition(PXR_NS::SdfPath(path), pos);
}
...
void enableGravity(const char* path)
{
    ...
    gCctManager->enableGravity(PXR_NS::SdfPath(path));
}
...
void enableCustomGravity(const char* path, const carb::Double3& gravity)
{
    ...
    gCctManager->enableCustomGravity(PXR_NS::SdfPath(path), gravity);
}
...
void disableGravity(const char* path)
{
    ...
    gCctManager->disableGravity(PXR_NS::SdfPath(path));
}
...
bool hasGravityEnabled(const char* path)
{
    ...
    return gCctManager->hasGravityEnabled(PXR_NS::SdfPath(path));
}
...
float getControllerHeight(const char* path)
{
    ...
    return gCctManager->getControllerHeight(PXR_NS::SdfPath(path));
}
...
void setControllerHeight(const char* path, float height)
{
    ...
    gCctManager->setControllerHeight(PXR_NS::SdfPath(path), height);
}
...
void enableFirstPerson(const char* path, const char* cameraPath)
{
    ...
    gCctManager->enableFirstPerson(PXR_NS::SdfPath(path), PXR_NS::SdfPath(cameraPath));
}
...
void disableFirstPerson(const char* path)
{
    ...
    gCctManager->disableFirstPerson(PXR_NS::SdfPath(path));
}
...
void activateCct(const char* path)
{
    ...
    gCctManager->activateCct(PXR_NS::SdfPath(path));
}
...
void removeCct(const char* path)
{
    ...
    gCctManager->removeCct(PXR_NS::SdfPath(path));
}
...
void enableWorldSpaceMove(const char* path, bool enable)
{
    ...
    gCctManager->enableWorldSpaceMove(PXR_NS::SdfPath(path), enable);
}
...
void setMove(const char* path, const carb::Float3& displacement)
{
    ...
    gCctManager->setMove(PXR_NS::SdfPath(path), displacement);
}
