...
class FilteringPairDemo(demo.Base):
    ...
    def create(self, stage):
        ...
        box0Prim = add_rigid_box(stage, boxActorPath, size, position, orientation, color, density, linVelocity, angularVelocity)

        demo.set_contact_offset(box0Prim)

        # Box1
        boxActorPath = defaultPrimPath + "/boxActor1"
        position = Gf.Vec3f(0.0, 0.0, 150.0)
        color = demo.get_primary_color(1.0)
        box1Prim = add_rigid_box(stage, boxActorPath, size, position, orientation, color, density, linVelocity, angularVelocity)

        demo.set_contact_offset(box1Prim)
