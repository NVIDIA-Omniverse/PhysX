    ...
    static bool compute(OgnPhysXImmediateComputeGeometryRaycastHitsDatabase& db)
    {
        ...
        if(db.inputs.prims.size() > 0)
        {
            ...
            for(size_t n = 0; n < state.phaseHashMeshes.inputMeshView.size(); ++n)
            {
                ...
                const omni::fabric::Path pathC(state.phaseHashMeshes.inputMeshView[n].meshView.primId);
                ...
                if(extentAttrib.IsValid() && extentAttrib.HasValue())
                {
                    ...
                    PXR_NS::GfBBox3d bbox = GfBBox3d(GfRange3d(extent[0], extent[1]), state.phaseCreateGeometries.meshGeometryData[n].worldMatrix);
                    PXR_NS::GfRange3d range = bbox.ComputeAlignedRange();
        ...
        else
        {
            ...
            for (size_t n = 0; n < childrenCount; ++n)
            {
                ...
                const auto boxMinAttr = extractedBundle.attributeByName(omni::fabric::Token::createImmortal("bboxMinCorner"));
                const auto boxMaxAttr = extractedBundle.attributeByName(omni::fabric::Token::createImmortal("bboxMaxCorner"));
                ...
                if (ImmediateShared::parseBoundsFromAttributes(bounds, boxMinAttr, boxMaxAttr))
                {
                    ...
                    const auto boxTransformAttr = extractedBundle.attributeByName(omni::fabric::Token::createImmortal("bboxTransform"));
                    const auto worldMatrixAttr = extractedBundle.attributeByName(omni::fabric::Token::createImmortal("worldMatrix"));
                    if (boxTransformAttr.isValid() && worldMatrixAttr.isValid())
                    {
                        ...
                        {
                            const auto primPathAttribute = extractedBundle.attributeByName(omni::fabric::Token::createImmortal("sourcePrimPath"));
                            omni::fabric::Token primPath = *primPathAttribute.getCpu<omni::fabric::Token>();
                            db.logError(
