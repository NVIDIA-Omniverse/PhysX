...
struct omni::physx::graph::ImmediateGenerateContacts : public ImmediateNarrowPhase
{
                               ...
                               const ImmediateGenerateContacts* self)
    {
        ...
        for (uint32_t i = 0; i < nbPairs; i++)
        {
            ...
            resultIndexMap.push_back(i);
            const PXR_NS::GfVec3d trans0 = geometries[id0].worldMatrix.ExtractTranslation();
            const PXR_NS::GfVec3d trans1 = geometries[id1].worldMatrix.ExtractTranslation();
        ...
        const uint32_t actualPairs = uint32_t(resultIndexMap.size());
        if (actualPairs == 0)
            return;

        std::vector<::physx::PxCache> contactCacheArray;
        contactCacheArray.resize(actualPairs);
        ...
        struct ContactRecorder : ::physx::immediate::PxContactRecorder
        {
            ...
            ContactRecorder(MeshGenerateContactsResult* results, const uint32_t* indexMap)
                : results(results), indexMap(indexMap)
            {
            }
            ...
            {
                const uint32_t originalIndex = indexMap[index];
                for (::physx::PxU32 i = 0; i < nbContacts; ++i)
                    results[originalIndex].contacts.push_back(contactPoints[i]);

                return true;
            }
        } contactRecorder(results, resultIndexMap.data());
