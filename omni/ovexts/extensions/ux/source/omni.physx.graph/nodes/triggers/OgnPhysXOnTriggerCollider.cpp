...
public:
    ...
    void subscribe(const GraphContextObj& graphContextObj, const NodeObj& nodeObj, const AttributeObj& attrObj)
    {
        ...
        PXR_NS::UsdStageWeakPtr stagePtr =
            PXR_NS::UsdUtilsStageCache::Get().Find(PXR_NS::UsdStageCache::Id::FromLongInt(static_cast<long int>(stageId)));
        ...
        else
        {
            ...
            {
                PXR_NS::SdfPath primPath = PXR_NS::SdfPath(db.tokenToString(it));
                if (stagePtr)
                {
                    PXR_NS::UsdPrim prim = stagePtr->GetPrimAtPath(primPath);
                    if (!prim.IsValid())
                    {
                    ...
                    else if (!prim.HasAPI<PXR_NS::PhysxSchemaPhysxTriggerAPI>() &&
                             !prim.HasAPI<PXR_NS::PhysxSchemaPhysxTriggerStateAPI>())
                    {
                        if (numErrors < MaxNumErrors)
                        {
