        ...
        float scale, uint32_t color, uint32_t yColor, uint32_t zColor, float lineWidth)
    {
        ...
        for (uint32_t i = 0; i <= nbLines; i++)
        {
            ...
            if (axis == AngularAxis::eTWIST)
            {
                ...
                a = PXR_NS::GfVec3f(GfRotation(q).TransformDir(GfVec3f(1.0f, 0.0f, 0.0f)));
            }
            else if (axis == AngularAxis::eSWING1)
            {
                ...
                a = PXR_NS::GfVec3f(GfRotation(q).TransformDir(GfVec3f(0.0f, 1.0f, 0.0f)));
            }
            else if (axis == AngularAxis::eSWING2)
            {
                ...
                a = PXR_NS::GfVec3f(GfRotation(q).TransformDir(GfVec3f(0.0f, 0.0f, 1.0f)));
            }
...
void applyGizmosDrawCall(float, float, const omni::kit::StageUpdateSettings*, void*)
{
    ...
    {
        std::string typeName;
        child.GetAttribute(PXR_NS::TfToken("omni:pvdi:class")).Get(&typeName);
        if (typeName == "PxScene")
        {
    ...
    if (isVisibleVizMode(transparencyVizMode))
    {
        ...
        PXR_NS::UsdEditContext editContext(stage, gOmniPvdAnonLayer);
