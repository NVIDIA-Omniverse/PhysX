...
namespace
{
    // RAII guard: clears OmniPvdUsd::sTokenMap/sTokenCache/sSdfPathMap on scope
    // exit. Used at the top of every ovdToUsd* entry point so allocated pxr::TfTokens
    // don't leak if the entry point returns early (error path) or normally.
    struct OmniPvdUsdResetGuard
    {
        ~OmniPvdUsdResetGuard() { OmniPvdUsd::reset(); }
    };
...
{
    OmniPvdUsdResetGuard resetGuard;
    clearMessages();

    OmniPvdDOMState domState;
    if (!buildPvdDomStateFromFile(omniPvdFile, domState))
    {
        return false;
    }
...
namespace
{
    static PXR_NS::TfToken vizToken("omni:pvdi:viz");
    static PXR_NS::TfToken handleToken("omni:pvdi:handle");
    
    void setVisibleChildrenRecursive(PXR_NS::UsdPrim prim, double timeCode)
    {
        if (!prim)
            return;
        
        PXR_NS::UsdAttribute vizAttr = prim.GetAttribute(vizToken);
        if (vizAttr)
        {
            ...
            if (imageable)
            {
                if (isVisible)
                {
                    ...
                    for (const PXR_NS::UsdPrim& child : prim.GetChildren())
                    {
...
void updateOvdVisibility(double timeCode)
{
    ...
    PXR_NS::UsdStageRefPtr stage = context->getStage();
    if (!stage)
        return;
    ...
    // Use SdfChangeBlock for batched notifications (better performance)
    {
        ...
        PXR_NS::UsdPrim scenesPrim = stage->GetPrimAtPath(PXR_NS::SdfPath("/Scenes"));
        if (scenesPrim)
        {
        ...
        PXR_NS::UsdPrim sharedPrim = stage->GetPrimAtPath(PXR_NS::SdfPath("/Shared"));
        if (sharedPrim)
        {
...
void buildHandleCache()
{
    ...
    for (const PXR_NS::UsdPrim& prim : stage->TraverseAll())
    {
        PXR_NS::UsdAttribute handleAttr = prim.GetAttribute(handleToken);
        if (handleAttr)
        {
            uint64_t handle = 0;
                             ...
                             char* outNames, bool* outIsActionable)
{
    ...
    omni::usd::UsdContext* context = omni::usd::UsdContext::getContext();
    PXR_NS::UsdStageRefPtr stage = context ? context->getStage() : nullptr;
    ...
    for (size_t i = 0; i < numHandles; ++i)
    {
        ...
        {
            PXR_NS::UsdPrim prim = stage->GetPrimAtPath(entry.path);
            if (!prim)
                continue;
