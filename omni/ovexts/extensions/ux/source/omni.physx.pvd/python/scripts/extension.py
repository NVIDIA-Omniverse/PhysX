...
class OmniPVDNodeDelegate(ui.AbstractItemDelegate):
    def __init__(self):
        ...
        self._actortype_map = dict([
            ...
            ('eDEFORMABLE_VOLUME', 'PxDeformableVolume'),
    ...
    def reset(self):
        ...
        # Visibility lock: set of prim paths that are forced hidden across frame changes
        if not hasattr(self, '_locked_visibility'):
            self._locked_visibility = set()
    ...
    def build_widget(self, model, item, column_idx, level, expanded):
        ...
        if column_idx == 0:
            ...
            is_hidden = prim_path in self._locked_visibility
            with ui.HStack(width=24, spacing=0):
                # Hide/Show toggle — sets purpose to guide/default on this prim and descendants
                eye_btn = ui.Button(
                    "__" if is_hidden else "O",
                    width=0, height=18,
                    style={
                ...
                ui.Spacer()
                def on_eye_clicked(path=prim_path, btn=eye_btn):
                    if not self._stage:
                        self._set_stage_vars()
                    if not self._stage:
                        return
                    ...
                    hiding = path not in self._locked_visibility
                    # Apply to this prim and all descendants
                    for descendant in Usd.PrimRange(p):
                        dpath = descendant.GetPath().pathString
                        imageable = UsdGeom.Imageable(descendant)
                        if hiding:
                            self._locked_visibility.add(dpath)
                            imageable.CreatePurposeAttr().Set(UsdGeom.Tokens.guide)
                        else:
                            self._locked_visibility.discard(dpath)
                            imageable.CreatePurposeAttr().Set(UsdGeom.Tokens.default_)
                    btn.text = "__" if hiding else "O"
                    # Force tree to rebuild all delegate widgets so children show updated state
                    if hasattr(self, '_tree') and self._tree:
                        self._tree.dirty_widgets()
                eye_btn.set_clicked_fn(on_eye_clicked)
        else:
            omniPvdClass = ""
                    ...
                    if prim.HasAttribute("omni:pvd:type"):
                        ...
                        if object_type:
                            if omniPvdClass == "PxActor":
                                omniPvdClass = self._actortype_map.get(object_type, 'Unknown')
                            elif omniPvdClass == "PxJoint":
                                omniPvdClass = self._jointtype_map.get(object_type, 'Unknown')
                else:
                    omniPvdClass = ""
            if item._pvd_node._found_in_search:
        ...
        with ui.VStack():
            with ui.HStack():
                if column_idx == 0:
                    ...
                    ui.Label("Vis", width=16, height=0, tooltip="Visibility lock")
                else:
...
class OmniPVdObjectTreeWindow(ui.Window):
        ...
        with self.frame:
            ...
            with ui.VStack():
                ...
                with ui.ScrollingFrame(
                    ...
                    self._tree = ui.TreeView(self._model, delegate = self._delegate, root_visible = False, header_visible = True, style={"margin": 0.5},  columns_resizable = True, style_type_name_override="TreeView", column_widths=[ui.Fraction(3.5), ui.Fraction(1.55), ui.Pixel(24)])
    ...
    def _updatedFromEvent(self):
        ...
        if stage:
            ...
            # Restore hidden prims — re-apply guide purpose after C++ visibility update
            if hasattr(self, '_delegate') and self._delegate._locked_visibility:
                for path in self._delegate._locked_visibility:
...
class PhysxPvdExtension(omni.ext.IExt):
    ...
    def _build_omnipvd_ui(self):
        ...
        self._omniPvdEnabledCheckbox.enabled = not get_physx_interface().is_running() and not carb.settings.get_settings().get_as_bool(SETTING_OMNIPVD_IS_OVD_STAGE)
    ...
    def _open_cached_stage(self, stageFile):
        if os.path.exists(stageFile):
            ...
            if not stage.GetPrimAtPath(Sdf.Path("/OvdDomeLight")) and not stage.GetPrimAtPath(Sdf.Path("/DomeLight")):
            ...
            if not stage.GetPrimAtPath(Sdf.Path("/OvdDistantLight")) and not stage.GetPrimAtPath(Sdf.Path("/DistantLight")):
