...
class OmniPVDBaseTests(PhysicsKitStageAsyncTestCase):
    ...
    async def test_ovd_bake_deformable_volume(self):
        """Test OVD baking with deformable volume actors.
        Deformable volumes require GPU support."""
        ...
        has_gpu = get_physx_foundation_interface().cuda_device_check()
        if not has_gpu:
            return  # Skip if no GPU — deformables require GPU dynamics

        outputDir = str(Path(omni_data_path).joinpath("omnipvd_output_deformable"))
        outputDir = outputDir.replace("\\", "/")
        if not outputDir.endswith("/"):
            outputDir += "/"
        os.makedirs(outputDir, exist_ok=True)
        ...
        stagePath = str(Path(outputDir).joinpath("stage_deformable." + fileType))
        stagePath = stagePath.replace("\\", "/")
        stage = get_or_create_stage(stagePath)

        simulationLayerPath = str(Path(outputDir).joinpath("layer_simulation_deformable." + fileType))
        simulationLayerPath = simulationLayerPath.replace("\\", "/")
        simulationLayer = Sdf.Layer.CreateNew(simulationLayerPath)
        stage.GetRootLayer().subLayerPaths.append(simulationLayerPath)
        ...
        # Create rigid bodies on root layer
        stage.SetEditTarget(stage.GetRootLayer())
        setup_env_2(stage, 1, Gf.Vec3d(0, 0, 0))

        # Set edit target to simulation layer for physics scene and deformables
        stage.SetEditTarget(simulationLayer)
        ...
        ############################################################################
        # Create a deformable volume from a cube mesh
        ############################################################################
        deformable_root_path = "/World/deformableVolume"
        skin_mesh_path = deformable_root_path + "/skinMesh"
        sim_mesh_path = deformable_root_path + "/simMesh"

        # Create root xform for the deformable
        UsdGeom.Xform.Define(stage, deformable_root_path)
        rootPrim = stage.GetPrimAtPath(deformable_root_path)
        UsdGeom.Xformable(rootPrim).AddTranslateOp().Set(Gf.Vec3d(0.0, 5.0, 0.0))

        # Create a 3D cube skin mesh (must be a closed 3D mesh for tetrahedralization)
        cube_points = [
        ...
        skinMesh = UsdGeom.Mesh.Define(stage, skin_mesh_path)
        skinMesh.GetPointsAttr().Set(cube_points)
        skinMesh.GetFaceVertexCountsAttr().Set([3] * 12)
        skinMesh.GetFaceVertexIndicesAttr().Set(cube_indices)

        # Create auto volume deformable hierarchy
        success = deformableUtils.create_auto_volume_deformable_hierarchy(
            stage,
            root_prim_path=deformable_root_path,
            simulation_tetmesh_path=sim_mesh_path,
            collision_tetmesh_path=sim_mesh_path,
            cooking_src_mesh_path=skin_mesh_path,
            simulation_hex_mesh_enabled=False,
            cooking_src_simplification_enabled=True,
            set_visibility_with_guide_purpose=False
        )
        self.assertTrue(success, "Failed to create deformable volume hierarchy")

        # Add deformable material
        deformableUtils.add_deformable_material(
            stage,
            deformable_root_path + "/material",
            youngs_modulus=1.0e+5,
            poissons_ratio=0.3
        )
        ...
        ############################################################################
        # Test OVD baking — verify the deformable appears in the output
        ############################################################################
        recording_dir = settings.get("/persistent/physics/omniPvdOvdRecordingDirectory")
        if not recording_dir:
            self.fail("Failed to get recording directory from settings")
        ...
        stageFilename = "stage_deformable_over.usda"

        result = physxPvdInterface.ovd_to_usd_over_with_layer_creation(
            inputOvdPath, stagePath, outputDir, stageFilename, 0.0, 0.0, True, True)

        self.assertTrue(result, "OVD baking failed for deformable scene")

        outputStagePath = Path(outputDir) / stageFilename
        self.assertTrue(outputStagePath.exists(), f"Output stage file not found: {outputStagePath}")

        # Open the output stage and verify deformable mesh has proper topology
        overStage = Usd.Stage.Open(str(outputStagePath))
        self.assertTrue(overStage, "Failed to open output stage")

        # Find the deformable mesh prim — it should be the skin mesh with over data
        skinMeshPrim = overStage.GetPrimAtPath("/World/deformableVolume/skinMesh")
        if skinMeshPrim:
            mesh = UsdGeom.Mesh(skinMeshPrim)
            if mesh:
                pointsAttr = mesh.GetPointsAttr()
                faceVertexCountsAttr = mesh.GetFaceVertexCountsAttr()
                faceVertexIndicesAttr = mesh.GetFaceVertexIndicesAttr()

                # Verify animated points exist
                if pointsAttr:
                    timeSamples = pointsAttr.GetTimeSamples()
                    print(f"Deformable mesh time samples: {len(timeSamples)}")
                    self.assertTrue(len(timeSamples) > 0, "Deformable mesh should have animated points")

                # Verify mesh topology was baked (from collisionMeshTriangleIndices)
                if faceVertexCountsAttr and faceVertexCountsAttr.Get():
                    counts = faceVertexCountsAttr.Get()
                    print(f"Deformable mesh face count: {len(counts)}")
                    self.assertTrue(len(counts) > 0, "Deformable mesh should have face vertex counts")
                    # All faces should be triangles
                    for c in counts:
                        self.assertEqual(c, 3, "All deformable mesh faces should be triangles")

                if faceVertexIndicesAttr and faceVertexIndicesAttr.Get():
                    indices = faceVertexIndicesAttr.Get()
                    print(f"Deformable mesh index count: {len(indices)}")
                    self.assertTrue(len(indices) > 0, "Deformable mesh should have face vertex indices")

        print("\n=== Deformable Bake Test Results ===")
        print(f"Output stage: {outputStagePath}")
        print(f"OVD file: {inputOvdPath}")
        print("====================================\n")
    ...
    async def test_ovd_over_with_deformable_volume(self):
        """Test OVD over layer creation with deformable volumes.
        Verifies that deformable mesh positions are baked as USD overs."""
        ...
        outputDir = str(Path(omni_data_path).joinpath("omnipvd_output_deformable_over"))
        outputDir = outputDir.replace("\\", "/")
        if not outputDir.endswith("/"):
            outputDir += "/"
        os.makedirs(outputDir, exist_ok=True)
        ...
        # Create simulation stage
        stagePath = str(Path(outputDir).joinpath("stage_deformable_over." + fileType))
        stagePath = stagePath.replace("\\", "/")
        stage = get_or_create_stage(stagePath)

        simulationLayerPath = str(Path(outputDir).joinpath("layer_simulation_deformable_over." + fileType))
        simulationLayerPath = simulationLayerPath.replace("\\", "/")
        simulationLayer = Sdf.Layer.CreateNew(simulationLayerPath)
        stage.GetRootLayer().subLayerPaths.append(simulationLayerPath)
        ...
        stageFilename = "stage_deformable_over_result.usda"

        result = physxPvdInterface.ovd_to_usd_over_with_layer_creation(
            inputOvdPath, stagePath, outputDir, stageFilename, 0.0, 0.0, True, True)

        self.assertTrue(result, "OVD over baking failed for deformable scene")

        outputStagePath = Path(outputDir) / stageFilename
        self.assertTrue(outputStagePath.exists(), f"Output stage file not found: {outputStagePath}")

        # Verify the over stage has deformable mesh data
        overStage = Usd.Stage.Open(str(outputStagePath))
        self.assertTrue(overStage, "Failed to open output over stage")

        # Check that the deformable skin mesh has over data with animated points
        skinMeshPrim = overStage.GetPrimAtPath("/World/deformableVolume/skinMesh")
        if skinMeshPrim:
            mesh = UsdGeom.Mesh(skinMeshPrim)
            if mesh:
                ...
                if pointsAttr:
                    timeSamples = pointsAttr.GetTimeSamples()
                    print(f"Over deformable mesh time samples: {len(timeSamples)}")
                    self.assertTrue(len(timeSamples) > 0, "Deformable over should have animated points")

        # Verify rigid body overs also work alongside deformables
        self.assertTrue(overStage.GetPrimAtPath("/World/env_2_1/Box"), "Box prim should exist in over stage")

        print("\n=== Deformable Over Test Results ===")
        print(f"Output stage: {outputStagePath}")
        print(f"OVD file: {inputOvdPath}")
        print("====================================\n")
    ...
    @staticmethod
    def _create_plane_mesh(stage, path, resolution=10, size=1.0):
        """Create a simple triangle plane mesh for surface deformable testing."""
        points = []
        indices = []
        counts = []
        half = size * 0.5
        for j in range(resolution + 1):
            for i in range(resolution + 1):
                x = -half + size * i / resolution
                z = -half + size * j / resolution
                points.append(Gf.Vec3f(x, 0.0, z))
    ...
    async def test_ovd_bake_deformable_surface(self):
        """Test OVD baking with deformable surface (cloth) actors.
        Deformable surfaces require GPU support."""
        ...
        outputDir = str(Path(omni_data_path).joinpath("omnipvd_output_deformable_surface"))
        outputDir = outputDir.replace("\\", "/")
        if not outputDir.endswith("/"):
            outputDir += "/"
        os.makedirs(outputDir, exist_ok=True)
        ...
        fileType = "usda"
        stagePath = str(Path(outputDir).joinpath("stage_deformable_surface." + fileType))
        stagePath = stagePath.replace("\\", "/")
        stage = get_or_create_stage(stagePath)

        simulationLayerPath = str(Path(outputDir).joinpath("layer_simulation_deformable_surface." + fileType))
        simulationLayerPath = simulationLayerPath.replace("\\", "/")
        simulationLayer = Sdf.Layer.CreateNew(simulationLayerPath)
        stage.GetRootLayer().subLayerPaths.append(simulationLayerPath)
        ...
        # Create a surface deformable from a plane mesh
        deformable_root_path = "/World/deformableSurface"
        skin_mesh_path = deformable_root_path + "/skinMesh"
        sim_mesh_path = deformable_root_path + "/simMesh"
        ...
        self._create_plane_mesh(stage, skin_mesh_path, resolution=10, size=2.0)
        ...
        self.assertTrue(success, "Failed to create deformable surface hierarchy")
        ...
        inputOvdPath = max(ovd_files, key=os.path.getctime)
        stageFilename = "stage_deformable_surface_bake.usda"

        result = physxPvdInterface.ovd_to_usd_over_with_layer_creation(
            inputOvdPath, stagePath, outputDir, stageFilename, 0.0, 0.0, True, True)
        self.assertTrue(result, "OVD baking failed for deformable surface scene")
        ...
        # Verify deformable surface mesh has animated points and topology
        skinMeshPrim = overStage.GetPrimAtPath("/World/deformableSurface/skinMesh")
        if skinMeshPrim:
            mesh = UsdGeom.Mesh(skinMeshPrim)
            if mesh:
                pointsAttr = mesh.GetPointsAttr()
                if pointsAttr:
                    timeSamples = pointsAttr.GetTimeSamples()
                    print(f"Surface deformable mesh time samples: {len(timeSamples)}")
                    self.assertTrue(len(timeSamples) > 0, "Surface deformable should have animated points")

                faceVertexCountsAttr = mesh.GetFaceVertexCountsAttr()
                if faceVertexCountsAttr and faceVertexCountsAttr.Get():
                    counts = faceVertexCountsAttr.Get()
                    print(f"Surface deformable face count: {len(counts)}")
                    self.assertTrue(len(counts) > 0, "Surface deformable should have face vertex counts")
                    for c in counts:
                        self.assertEqual(c, 3, "All faces should be triangles")

        print("\n=== Surface Deformable Bake Test Results ===")
        print(f"Output stage: {outputStagePath}")
        print(f"OVD file: {inputOvdPath}")
        print("=============================================\n")
    ...
    async def test_ovd_over_with_deformable_surface(self):
        """Test OVD over layer creation with deformable surfaces (cloth).
        Verifies that deformable surface mesh positions are baked as USD overs."""
        ...
        outputDir = str(Path(omni_data_path).joinpath("omnipvd_output_deformable_surface_over"))
        outputDir = outputDir.replace("\\", "/")
        if not outputDir.endswith("/"):
            outputDir += "/"
        os.makedirs(outputDir, exist_ok=True)
        ...
        fileType = "usda"
        stagePath = str(Path(outputDir).joinpath("stage_deformable_surface_over." + fileType))
        stagePath = stagePath.replace("\\", "/")
        stage = get_or_create_stage(stagePath)

        simulationLayerPath = str(Path(outputDir).joinpath("layer_simulation_deformable_surface_over." + fileType))
        simulationLayerPath = simulationLayerPath.replace("\\", "/")
        simulationLayer = Sdf.Layer.CreateNew(simulationLayerPath)
        stage.GetRootLayer().subLayerPaths.append(simulationLayerPath)
        ...
        # Create rigid bodies on root layer for verifying mixed scene
        stage.SetEditTarget(stage.GetRootLayer())
        setup_env_2(stage, 1, Gf.Vec3d(0, 0, 0))
        ...
        inputOvdPath = max(ovd_files, key=os.path.getctime)
        stageFilename = "stage_deformable_surface_over_result.usda"

        result = physxPvdInterface.ovd_to_usd_over_with_layer_creation(
            inputOvdPath, stagePath, outputDir, stageFilename, 0.0, 0.0, True, True)
        self.assertTrue(result, "OVD over baking failed for deformable surface scene")
        ...
        if skinMeshPrim:
            ...
            if mesh:
                ...
                if pointsAttr:
                    timeSamples = pointsAttr.GetTimeSamples()
                    print(f"Over surface deformable mesh time samples: {len(timeSamples)}")
                    self.assertTrue(len(timeSamples) > 0, "Surface deformable over should have animated points")

        # Verify rigid body overs also work alongside surface deformables
        self.assertTrue(overStage.GetPrimAtPath("/World/env_2_1/Box"), "Box prim should exist in over stage")

        print("\n=== Surface Deformable Over Test Results ===")
        print(f"Output stage: {outputStagePath}")
        print(f"OVD file: {inputOvdPath}")
        print("=============================================\n")
