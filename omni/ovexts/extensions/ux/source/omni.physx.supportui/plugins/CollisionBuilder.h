...
public:
    ...
    typedef std::unordered_set<PXR_NS::SdfPath, PXR_NS::SdfPath::Hash> TRigidBodiesForRemovalContainer;
    typedef std::unordered_set<PXR_NS::SdfPath, PXR_NS::SdfPath::Hash> TRigidBodiesForAddContainer;
...
public:
    ...
    void enumerateForColliderCreation(PXR_NS::UsdStageWeakPtr stage,
                                      const std::vector<PrimDef>& primDefs,
                                      bool avoidChangingExistingColliders = false);

    void createColliders(PXR_NS::UsdStageWeakPtr stage,
                         std::function<bool(const PXR_NS::SdfPath& primPath, bool created)> rigidBodyChangeCallback = nullptr,
...
private:
    static PXR_NS::UsdPrim isRigidBodyOnNodeOrUpInHierarchy(
        PXR_NS::UsdPrim startingPrim,
        const TRigidBodiesForAddContainer& rigidBodiesForAdd,
        const TRigidBodiesForRemovalContainer& rigidBodiesForRemoval,
        std::unordered_set<PXR_NS::SdfPath, PXR_NS::SdfPath::Hash>& primsWithRbParents);

    static bool hasGeomPrimInstancedProxyChild(PXR_NS::UsdPrim prim);

    static bool isCollApproximationPresent(PXR_NS::UsdPrim prim,
