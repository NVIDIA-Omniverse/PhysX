...
PhysXInspectorModelImpl::PhysXInspectorModelImpl(const PXR_NS::SdfPathVector& selection, PhysXInspector* inspector)
{
...
{
    int idx = 0;
    for (PXR_NS::SdfPath path : mAllArticulations)
    {
...
void PhysXInspectorModelImpl::parseArticulations(uint64_t stageId)
{
    ...
    if (PXR_NS::SdfPath::IsValidPathString(mSelectedPrimPath))
    {
        mArticulations = usdLoad->parseArticulations(stageId, PXR_NS::SdfPath(mSelectedPrimPath));
    }
}
...
void PhysXInspectorModelImpl::addArticulationHierarchyLink(PhysXInspectorItem& parent, ::physx::PxArticulationLink* link)
{
    ...
    linkNode->usdPath = PXR_NS::SdfPath(link->getName());
...
void PhysXInspectorModelImpl::addRigidShapes(PhysXInspectorItem& parent, ::physx::PxRigidActor* rigid)
{
    ...
    if (numShapes > 0)
    {
        ...
        for (::physx::PxU32 shapeIdx = 0; shapeIdx < numShapes; ++shapeIdx)
        {
            ...
            shapeNode->usdPath = PXR_NS::SdfPath(shape->getName());
...
{
    JointBodyMap::iterator bodyItr = mInspector->mJointBodyMap.find(PXR_NS::SdfPath(rigid->getName()));
...
void PhysXInspectorModelImpl::addArticulations(::physx::PxScene* scene)
{
    ...
    for (PxU32 i = 0; i < nbArticulations; i++)
    {
        ...
        articulationNode->usdPath = PXR_NS::SdfPath(articulation->getName());
...
void PhysXInspectorModelImpl::addDynamicBodies(::physx::PxScene* scene)
{
    ...
    for (PxU32 i = 0; i < nbRigidDynamic; i++)
    {
        ...
        rigidDynamic->usdPath = PXR_NS::SdfPath(rigid->getName());
        char massBuffer[100];
...
void PhysXInspectorModelImpl::addStaticBodies(::physx::PxScene* scene)
{
    ...
    for (PxU32 i = 0; i < nbRigidStatic; i++)
    {
        ...
        rigidStatic->usdPath = PXR_NS::SdfPath(rigid->getName());
        if (!staticBodiesNode)
        {
...
void PhysXInspectorModelImpl::findCollisionAPIFor(const PXR_NS::SdfPath& path, PXR_NS::SdfPathVector& collisionApis)
{
    const PXR_NS::UsdPhysicsCollisionAPI collisionApi = PXR_NS::UsdPhysicsCollisionAPI::Get(mInspector->mStage, path);
    if (collisionApi)
    {
    ...
    else
    {
        ...
        PXR_NS::UsdPrimRange primRange(mInspector->mStage->GetPrimAtPath(path));
        for (PXR_NS::UsdPrimRange::const_iterator iter = primRange.begin(); iter != primRange.end(); ++iter)
        {
            ...
            const PXR_NS::UsdPhysicsCollisionAPI subCollisionApi =
                PXR_NS::UsdPhysicsCollisionAPI::Get(mInspector->mStage, subPrim.GetPath());
            if (subCollisionApi)
            {
...
void PhysXInspectorModelImpl::selectAllConnectedLinks()
{
    ...
    PXR_NS::SdfPathVector addToSelection;
    for (PXR_NS::SdfPath& path : selectionVector)
    {
        if (!isIncludedInCurrentSelection(path))
            continue;
        PXR_NS::UsdPrim prim = mInspector->mStage->GetPrimAtPath(path);
        const bool existsInInspector = mInspector->mPhysXInterface->getObjectId(path, omni::physx::ePTJoint) !=
                                       ...
                                       mInspector->mPhysXInterface->getObjectId(path, omni::physx::ePTLinkJoint) !=
                                           omni::physx::usdparser::kInvalidObjectId;
        if (existsInInspector && prim.IsA<PXR_NS::UsdPhysicsJoint>())
        {
            PXR_NS::UsdPhysicsJoint physicsJoint(prim);
            PXR_NS::SdfPathVector target;
...
void PhysXInspectorModelImpl::selectAllConnectedJointShapes()
{
    const auto usdContext = omni::usd::UsdContext::getContext();
    auto selectionVector = usdContext->getSelection()->getSelectedPrimPathsV2(omni::usd::Selection::SourceType::eUsd);
    PXR_NS::SdfPathVector collisionApis;
    for (PXR_NS::SdfPath& path : selectionVector)
    {
        if (!isIncludedInCurrentSelection(path))
            continue;
    ...
    for (PXR_NS::SdfPath& path : selectionVector)
    {
        ...
        const bool existsInInspector = mInspector->mPhysXInterface->getObjectId(path, omni::physx::ePTActor) !=
                                       ...
                                       mInspector->mPhysXInterface->getObjectId(path, omni::physx::ePTLink) !=
                                           omni::physx::usdparser::kInvalidObjectId;
        if (existsInInspector && prim.HasAPI<PXR_NS::UsdPhysicsRigidBodyAPI>())
        {
            findCollisionAPIFor(path, collisionApis);
        }
    }
...
void PhysXInspectorModelImpl::selectAllConnectedBodyJoints()
{
    const auto usdContext = omni::usd::UsdContext::getContext();
    auto selectionVector = usdContext->getSelection()->getSelectedPrimPathsV2(omni::usd::Selection::SourceType::eUsd);
    PXR_NS::SdfPathVector allJoints;
    for (PXR_NS::SdfPath& path : selectionVector)
    {
        if (!isIncludedInCurrentSelection(path))
            continue;
...
void PhysXInspectorModelImpl::setSelectedPaths(std::vector<std::string> paths)
{
    ...
    {
        mSelectionSdf.push_back(PXR_NS::SdfPath(str));
    }
    ...
    {
        mSelectedPrimPathSdf = PXR_NS::SdfPath();
    }
    else
    {
        mSelectedPrimPathSdf = PXR_NS::SdfPath(mSelectedPrimPath);
    }
...
void PhysXInspectorModelImpl::refreshModelStructure()
{
    ...
    if (scene == nullptr)
    {
        ...
        else
        {
            ...
            const uint64_t stageId = PXR_NS::UsdUtilsStageCache::Get().GetId(mInspector->mStage).ToLongInt();
...
void PhysXInspectorModelImpl::refreshModelValues()
{
    ...
    {
        PXR_NS::UsdPrim prim = mInspector->mStage->GetPrimAtPath(it->usdPath);
        if (!prim)
        {
            continue;
        }
...
bool PhysXInspectorModelImpl::tryReadJointDrive(PXR_NS::UsdPrim& prim, float& jointValue)
{
...
bool PhysXInspectorModelImpl::tryReadJointVelocity(PXR_NS::UsdPrim& prim, float& jointValue)
{
    if (prim.IsA<PXR_NS::UsdPhysicsRevoluteJoint>())
    {
        if (prim.HasAPI<PXR_NS::UsdPhysicsDriveAPI>())
...
bool PhysXInspectorModelImpl::tryReadJointState(PXR_NS::UsdPrim& prim, float& jointValue)
{
    if (prim.IsA<PXR_NS::UsdPhysicsRevoluteJoint>())
    {
        if (prim.HasAPI<PXR_NS::PhysxSchemaJointStateAPI>())
        {
            PXR_NS::PhysxSchemaJointStateAPI jointStateAPI =
                PXR_NS::PhysxSchemaJointStateAPI::Get(prim, PXR_NS::UsdPhysicsTokens->angular);
            jointValue = 0.0f;
    ...
    else if (prim.IsA<PXR_NS::UsdPhysicsPrismaticJoint>())
    {
        ...
        {
            PXR_NS::PhysxSchemaJointStateAPI jointStateAPI =
                PXR_NS::PhysxSchemaJointStateAPI::Get(prim, PXR_NS::UsdPhysicsTokens->linear);
            jointValue = 0.0f;
            jointStateAPI.GetPositionAttr().Get(&jointValue);
            return true;
        }
...
bool PhysXInspectorModelImpl::tryWriteJointDrive(PXR_NS::UsdPrim& prim, float jointValue)
{
    if (prim.IsA<PXR_NS::UsdPhysicsRevoluteJoint>())
    {
        if (prim.HasAPI<PXR_NS::UsdPhysicsDriveAPI>())
...
bool PhysXInspectorModelImpl::tryWriteJointVelocity(PXR_NS::UsdPrim& prim, float jointValue)
{
    if (prim.IsA<PXR_NS::UsdPhysicsRevoluteJoint>())
    {
        if (prim.HasAPI<PXR_NS::UsdPhysicsDriveAPI>())
...
bool PhysXInspectorModelImpl::tryWriteJointState(PXR_NS::UsdPrim& prim, float jointValue)
{
    if (prim.IsA<PXR_NS::UsdPhysicsRevoluteJoint>())
    {
        if (prim.HasAPI<PXR_NS::PhysxSchemaJointStateAPI>())
...
bool PhysXInspectorModelImpl::isArticulationJoint(const PXR_NS::SdfPath& path) const
{
...
{
    PXR_NS::UsdPrim prim = mInspector->mStage->GetPrimAtPath(PXR_NS::SdfPath(primPath));
    if (!prim.IsValid())
    {
    ...
    case ControlType::eAutomatic: {
        ...
        if (prim.IsA<PXR_NS::UsdPhysicsRevoluteJoint>())
        {
            ...
            else if (isArticulation && prim.HasAPI<PXR_NS::PhysxSchemaJointStateAPI>())
            {
    ...
    case ControlType::eJointState: {
        ...
        if (prim.IsA<PXR_NS::UsdPhysicsRevoluteJoint>() && prim.HasAPI<PXR_NS::PhysxSchemaJointStateAPI>())
        {
            return "state:angular:physics:position";
        }
        else if (prim.IsA<PXR_NS::UsdPhysicsPrismaticJoint>() && prim.HasAPI<PXR_NS::PhysxSchemaJointStateAPI>())
        {
            return "state:linear:physics:position";
        }
    ...
    case ControlType::eJointDrive: {
        if (prim.IsA<PXR_NS::UsdPhysicsRevoluteJoint>() && prim.HasAPI<PXR_NS::UsdPhysicsDriveAPI>())
        {
            return "drive:angular:physics:targetPosition";
        }
...
PhysXInspectorModel::DataShapeType PhysXInspectorModelImpl::refreshJointsList(::physx::PxScene* scene)
{
    ...
    for (PxU32 i = 0; i < nbArticulations; i++)
    {
        ...
        articulationNode->usdPath = PXR_NS::SdfPath(path.c_str());
...
PXR_NS::SdfPath PhysXInspectorModelImpl::getPathForUserData(void* userData) const
{
...
bool PhysXInspectorModelImpl::isIncludedInCurrentSelection(const PXR_NS::SdfPath& path) const
{
