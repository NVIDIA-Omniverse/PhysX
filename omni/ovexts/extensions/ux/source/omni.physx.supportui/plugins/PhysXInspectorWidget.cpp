...
void PhysXInspectorWidgetImpl::drawSelectionJointsContextMenu(const PXR_NS::SdfPath& jointPath)
{
    auto& model = *std::static_pointer_cast<PhysXInspectorModelImpl>(getModel());
    const bool selected =
...
void PhysXInspectorWidgetImpl::drawSelectionBodiesContextMenu(const PXR_NS::SdfPath& bodyPath)
{
    auto& model = *std::static_pointer_cast<PhysXInspectorModelImpl>(getModel());
    const bool selected =
...
bool PhysXInspectorWidgetImpl::drawSelectableTreenode(const char* fullPath, const char* rigidName)
{
    ...
    if (ImGui::IsItemClicked(0)&& !ImGui::IsItemToggledOpen())
    {
        ...
        {
            usdContext->getSelection()->add(PXR_NS::SdfPath(fullPath), false, false, true);
        }
        else
        {
...
void PhysXInspectorWidgetImpl::drawJointSlider(const PXR_NS::SdfPath& jointPath)
{
    PXR_NS::UsdPrim prim = mInspector.mStage->GetPrimAtPath(jointPath);
    ...
    else if (prim.HasAPI<PXR_NS::PhysxSchemaPhysxMimicJointAPI>())
    {
        ImGui::Text("Mimic Joint");
    }
    else if (prim.IsA<PXR_NS::UsdPhysicsSphericalJoint>())
    {
        ImGui::Text("Spherical Joint");
    }
    else if (prim.IsA<PXR_NS::UsdPhysicsFixedJoint>())
    {
...
{
    PXR_NS::SdfPath jointPath = mPhysXInterface->getPhysXObjectUsdPath((omni::physx::usdparser::ObjectId)joint->userData);
...
{
    PXR_NS::SdfPath path = mPhysXInterface->getPhysXObjectUsdPath((omni::physx::usdparser::ObjectId)link->userData);
