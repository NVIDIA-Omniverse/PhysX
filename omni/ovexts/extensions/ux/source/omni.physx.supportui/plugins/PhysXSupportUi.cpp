...
class ExtensionImpl final : public omni::ext::IExt, public PXR_NS::TfWeakBase
{
public:
    ExtensionImpl()
    {
    ...
    void handleUsdObjChange(const PXR_NS::UsdNotice::ObjectsChanged& objectsChanged)
    {
        ...
        {
            static TfToken const kinematicsEnabledToken = PXR_NS::UsdPhysicsTokens.Get()->physicsKinematicEnabled;
        ...
        for (const SdfPath& path : objectsChanged.GetResyncedPaths())
        {
            if (path.IsAbsoluteRootOrPrimPath())
            {
                ...
                if (changedFields.size() > 0)
                {
                    ...
                    newAsset = hasChangedToken(objectsChanged, primPath, PXR_NS::SdfFieldKeys->Specifier);
                               //&& hasChangedToken(objectsChanged, primPath, PXR_NS::SdfFieldKeys->TypeName);
    ...
    void onStartup(const char* extId) override
    {
        ...
        mUsdNoticeListenerKey = PXR_NS::TfNotice::Register(PXR_NS::TfCreateWeakPtr(this), &ExtensionImpl::handleUsdObjChange);
    ...
    bool createColliders(const PXR_NS::SdfPath& primPath, IPhysxSupportUi::ColliderType colliderType)
    {
        if (mStage == nullptr)
        {
            return false;
    ...
    bool rigidBodyManipulationMove(const PXR_NS::SdfPath& primPath,
    ...
    bool rigidBodyManipulationRotate(const PXR_NS::SdfPath& primPath,
    ...
    void rigidBodyManipulationBegan(const PXR_NS::SdfPath& primPath)
    {
        mRigidBodyManipulatorHelper->setPxScene(SearchForPhysxScene());
        mRigidBodyManipulatorHelper->onManipulationBegan(primPath);
    }

    void rigidBodyManipulationEnded(const PXR_NS::SdfPath& primPath)
    {
