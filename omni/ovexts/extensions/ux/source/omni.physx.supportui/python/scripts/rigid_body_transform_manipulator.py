...
from pxr import Sdf, Usd, UsdGeom, UsdPhysics, PhysicsSchemaTools
...
class RigidBodyTransformManipulator(PrimTransformManipulator):
    ...
    def on_selection_changed(self, stage: Usd.Stage, selection: Union[List[Sdf.Path], None], *args, **kwargs) -> bool:  # override
        ...
        def get_rigid_body(prim):
            while prim and not prim.IsPseudoRoot():
                if prim.IsA(UsdGeom.Xformable) and prim.HasAPI(UsdPhysics.RigidBodyAPI):
                    ...
                    if rigid_body is not None:
                        if not rigid_body.GetKinematicEnabledAttr().Get():
                            ...
                            return None
                if not self.model._manip_mode_allow_rigid_body_traversal:
                    return None
                prim = prim.GetParent()
            return None

        rigid_body_paths = self.model.rigid_body_paths
        rigid_body_paths.clear()

        selection_filtered = []
        for path in selection:
            prim = stage.GetPrimAtPath(path)
            rigid_body = get_rigid_body(prim)
            if rigid_body is not None:
                selection_filtered.append(path)
                rigid_body_paths.add(PhysicsSchemaTools.sdfPathToInt(rigid_body.GetPath()))

        return super().on_selection_changed(stage, selection_filtered)
