...
import omni.usd
from omni.kit.property.physics import Manager, database, databaseUtils
from omni.kit.property.physics.widgets import MainFrameWidget, PhysicsMaterialBindingWidget
...
class PhysxPropertyTest(TestCase):
    ...
    async def _test_physics_visual_component(self, prim, component_name, skip_assert=False):
        ...
        # For joint components, expand the "Advanced" subsection so Advanced-grouped
        # attributes (e.g. physxJoint:maxJointVelocity) are visible in the snapshot
        # and pixel-diff catches schema-default drift. See NVBugs 5318061 / OMPE-50213.
        if "joint" in component_name:
            for advanced_frame in ui_test.find_all("Property//Frame/**/CollapsableFrame[*].title=='Advanced'"):
                advanced_frame.widget.collapsed = False
            await self.wait(5)
        ...
        await self.wait(20)
        return await self.do_visual_test(img_suffix=component_name, skip_assert=skip_assert)
    ...
    async def test_physics_visual_components(self):
        for component in database.components.values():
            with self.subTest(component.name) as include:
                ...
                if component.name in {"PhysxParticleSamplingAPI", "PhysxParticleSetAPI"}:
                    self.skipTest("Skipped")
                    continue
                ...
                await self.wait(20)
                print(f"subTest: {component.name}")
    ...
    async def test_physics_ui_joint_properties(self):
        ...
        expected_titles = [
            ...
            "Joint",
            "Joint (PhysX)",
            "Drive Performance Envelope (PhysX)",
        ]
    ...
    async def test_physics_ui_extended_physx_material_set_widget_state_handles_none_refs(self):
        # OMPE-95338: when a Property-window search filter hides
        # compliantContact*, ExtendedPhysxMaterialWidget's cached widget
        # refs go stale and _set_widget_state used to fire pure-virtual.
        from omni.physics.physxui.widgets.materials import ExtendedPhysxMaterialWidget

        stage = await self.new_stage()
        mat = UsdShade.Material.Define(stage, "/PhysicsMaterial")
        UsdPhysics.MaterialAPI.Apply(mat.GetPrim())
        PhysxSchema.PhysxMaterialAPI.Apply(mat.GetPrim())

        await self._select_and_focus_property_window([mat.GetPath().pathString])

        main_frame = p.get_window()._widgets_top[Manager.scheme][MainFrameWidget.name]
        material_subwidget = None
        for sw in main_frame.get_subwidgets():
            if isinstance(sw.widget, ExtendedPhysxMaterialWidget):
                material_subwidget = sw.widget
                break
        self.assertIsNotNone(material_subwidget, "ExtendedPhysxMaterialWidget not registered as Physics subwidget")

        # Simulate the post-filter rebuild state where compliantContact*
        # were filtered out and the cached refs never got reassigned.
        material_subwidget._compliant_damping_widget = None
        material_subwidget._compliant_acceleration_spring_widget = None
        material_subwidget._enabled = False  # forces the buggy branch

        # Must not raise. Pre-fix this called None.model.set_value(0.0).
        material_subwidget._set_widget_state()
    ...
    async def test_physics_ui_mesh_approximation(self):
        ...
        for name, _ in physxUtils.MESH_APPROXIMATIONS.items():
            ...
            res &= await self._test_physics_visual_component(prim, f"_mesh_approximation_{name}", skip_assert=True)
        ...
        for name, _ in physxUtils.MESH_APPROXIMATIONS.items():
            prim.GetAttribute("physics:approximation").Set(name)
            res &= await self._test_physics_visual_component(prim, f"_mesh_approximation_{name}_meshmerge", skip_assert=True)
