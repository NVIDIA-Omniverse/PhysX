...
class PhysxSimulationDataVisualizerTest(TestCase):
    ...
    async def test_simulation_data_visualizer_articulation_link(self):
        size = Gf.Vec3f(25.0)

        stage = await self.new_stage()

        # Root link of a floating-base articulation.
        root_link = physicsUtils.add_rigid_cube(stage, "/rootLink", size, Gf.Vec3f(0.0))
        UsdPhysics.ArticulationRootAPI.Apply(root_link)

        # Dynamic child link.
        child_link = physicsUtils.add_rigid_cube(stage, "/childLink", size, Gf.Vec3f(0.0, 0.0, 100.0))

        # Revolute joint that becomes an articulation joint at the PhysX layer.
        revolute_joint = UsdPhysics.RevoluteJoint.Define(self._stage, "/revoluteJoint")
        revolute_joint.CreateBody0Rel().SetTargets([root_link.GetPath()])
        revolute_joint.CreateBody1Rel().SetTargets([child_link.GetPath()])
        revolute_joint.CreateLocalPos0Attr().Set(Gf.Vec3f(0.0, 0.0, 2.0))
        revolute_joint.CreateLocalPos1Attr().Set(Gf.Vec3f(0.0, 0.0, -2.0))
        revolute_joint.CreateLocalRot0Attr().Set(Gf.Quatf(1.0))
        revolute_joint.CreateLocalRot1Attr().Set(Gf.Quatf(1.0))
        revolute_joint.CreateAxisAttr("X")

        # Visualize debug data for the child link (the one with an inbound joint).
        omni.usd.get_context().get_selection().set_selected_prim_paths([child_link.GetPath().pathString], False)

        result = await self._test_simulation_data_visualizer("articulation_link")

        await self.new_stage()

        self.assertTrue(result)

    async def test_simulation_data_visualizer_articulation_joint(self):
        size = Gf.Vec3f(25.0)
        ...
        omni.usd.get_context().get_selection().set_selected_prim_paths([revolute_joint.GetPath().pathString], False)

        result = await self._test_simulation_data_visualizer("articulation_joint")
