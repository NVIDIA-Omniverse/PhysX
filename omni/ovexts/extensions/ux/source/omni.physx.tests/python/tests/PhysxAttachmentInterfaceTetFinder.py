...
from pxr import Gf, Sdf, Vt, UsdGeom, Usd, UsdPhysics, UsdLux
...
from omni.physx.scripts.ifaces import get_physx_attachment_private_interface, get_physx_cooking_private_interface
import carb
...
class PhysxAttachmentInterfaceTetFinderTestAsync(PhysicsBaseAsyncTestCase):
    ...
    async def base_setup(self):
        ...
        self._deformable_material_path = '/World/DeformableMaterial'
        omni.kit.commands.execute("AddDeformableMaterial",
                                  stage=self._stage, path=self._deformable_material_path,
    ...
    async def _runCreateAutoVolumeDeformableHierarchy(self, skin_mesh_path: Sdf.Path = Sdf.Path()) -> bool:
        self.assertTrue(bool(skin_mesh_path))

        root_path = skin_mesh_path.GetParentPath().AppendChild(skin_mesh_path.name + "_deformable")
        sim_mesh_path = root_path.AppendChild("simMesh")
        coll_mesh_path = root_path.AppendChild("collMesh")

        UsdGeom.Xform.Define(self._stage, root_path)

        success = omni.kit.commands.execute(
            "CreateAutoVolumeDeformableHierarchy",
        ...
        physicsUtils.add_physics_material_to_prim(self._stage, self._stage.GetPrimAtPath(root_path), self._deformable_material_path)
    ...
    async def create_deformable_body(self, mesh, starting_height):
        ...
        await self._runCreateAutoVolumeDeformableHierarchy(skin_mesh_path=mesh.GetPath())

        root_path = mesh.GetPath().GetParentPath().AppendChild(mesh.GetPath().name + "_deformable")
        coll_mesh_path = root_path.AppendChild("collMesh")
        coll_prim = self._stage.GetPrimAtPath(coll_mesh_path)

        tet_mesh = UsdGeom.TetMesh(coll_prim)
        self.assertTrue(tet_mesh)

        points = list(tet_mesh.GetPointsAttr().Get())
        self._rest_coll_points = [Gf.Vec3d(p) for p in points]

        self._coll_indices = [i for tet in tet_mesh.GetTetVertexIndicesAttr().Get() for i in tet]
