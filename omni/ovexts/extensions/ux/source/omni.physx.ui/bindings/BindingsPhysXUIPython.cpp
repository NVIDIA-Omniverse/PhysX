...
PYBIND11_MODULE(_physxUI, m)
{
    ...
    physxUI.def("select_spatial_tendon_attachment_helper",
        ...
        {
            self->selectSpatialTendonAttachmentHelper(PXR_NS::SdfPath(linkBodyPath), PXR_NS::TfToken(instanceName));
        },
    ...
    physxUI.def(
        "refresh_attachment",
        [](const IPhysxUI* self, const char* attachmentPath) { self->refreshAttachment(PXR_NS::SdfPath(attachmentPath)); },
