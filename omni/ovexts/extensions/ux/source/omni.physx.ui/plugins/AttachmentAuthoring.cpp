...
static const PXR_NS::TfToken deformableBodyDataCrcToken("physxDeformableBody:deformableBodyDataCrc");
...
void AttachmentAuthoring::parseStage()
{
    ...
    {
        const std::vector<usdrt::SdfPath> paths = usdrtStage->GetPrimsWithTypeName(usdrt::TfToken(PhysxAdditionAPITokens->AutoDeformableAttachmentAPI.GetText()));
        if (!paths.empty())
            needToParse = true;
    }
...
void AttachmentAuthoring::handlePrimResync(const SdfPath path)
{
    ...
    for(PXR_NS::UsdPrimRange::const_iterator cit = range.begin(); cit != range.end(); ++cit)
    {
