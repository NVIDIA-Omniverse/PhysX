...
void ShapeUtils::getShapeScale(const usdparser::PhysxShapeDesc* desc, PXR_NS::GfVec3f& scale)
{
    if (desc == nullptr)
    {
        return;
...
RenderInstanceCache::RenderInstanceDesc* RenderInstanceCache::get(const PXR_NS::SdfPath& path)
{
...
bool RenderInstanceCache::exists(const PXR_NS::SdfPath& path, const GfMatrix4f& matrix)
{
    auto it = mRenderInstanceDescMap.find(path);
    if (it != mRenderInstanceDescMap.end())
    {
...
void RenderInstanceCache::addOrUpdate(const PXR_NS::SdfPath& path,
                                      omni::renderer::RenderInstanceBuffer renderInstanceBuffer,
                                      size_t renderInstanceNum,
                                      TriangleMeshBuffer* triangleMeshBuffer,
...
bool DebugVisualization::HasPointInstancerParent(const PXR_NS::UsdPrim& prim)
{
    UsdPrim parent = prim;
...
void DebugVisualization::draw()
{
    ...
    PXR_NS::UsdStageWeakPtr stage = omni::usd::UsdContext::getContext()->getStage();
    if (!stage)
    {
        return;
    }
    ...
    const float metersPerUnit = float(PXR_NS::UsdGeomGetStageMetersPerUnit(stage));
...
void DebugVisualization::onResyncedPrimPath(const PXR_NS::SdfPath& path)
{
...
void DebugVisualization::onAttributeChange(const PXR_NS::SdfPath& attrPath,
                                                          ...
                                                          PXR_NS::UsdStageWeakPtr stage)
{
    ...
    mXformCache.Clear();
    mXformCache.SetTime(PXR_NS::UsdTimeCode(gTimeline->getCurrentTime() * stage->GetTimeCodesPerSecond()));
    ...
    if (mSceneDirty)
    {
        ...
        if (mColliderVisualization == VisualizerMode::eAll)
        {
            ...
            {
                PXR_NS::UsdPrimRange range = stage->Traverse(UsdTraverseInstanceProxies());
...
usdparser::PhysxShapeDesc* DebugVisualization::getShapeDesc(const PXR_NS::UsdPrim& prim,
                                                            const PXR_NS::UsdPrim& colPrim,
...
void DebugVisualization::removePrimFromAABBCache(const PXR_NS::SdfPath& primPath)
{
...
void DebugVisualization::removePrimFromTrimeshCache(const PXR_NS::SdfPath& primPath)
{
...
void DebugVisualization::updateCollisionPrimSimplifiedAABB(const PXR_NS::SdfPath& primPath, const PXR_NS::GfRange3d& primAABB)
{
...
void DebugVisualization::updateCollisionPrimTriangeMeshShape(const PXR_NS::SdfPath& primPath,
                                                             const PXR_NS::GfMatrix4d& primTransformMtx,
...
void DebugVisualization::updateCollisionPrimDebugVisualization(const PXR_NS::UsdPrim& colPrim, float metersPerUnit)
{
    CARB_PROFILE_ZONE(0, "DebugVisualization::updateCollisionPrimDebugVisualization %s", colPrim.GetPath().GetText());

    const uint64_t stageId = PXR_NS::UsdUtilsStageCache::Get().GetId(colPrim.GetStage()).ToLongInt();

    PXR_NS::UsdPrimRange range(colPrim, UsdTraverseInstanceProxies());
    for (PXR_NS::UsdPrimRange::const_iterator iter = range.begin(); iter != range.end(); ++iter)
    {
        UsdPrim prim = *iter;
        ...
        PXR_NS::GfRange3d primAABB = omni::usd::UsdContext::getContext()->computePrimWorldBoundingBox(prim);
        ...
        PXR_NS::SdfPath primPath = prim.GetPrimPath();
                                        ...
                                        TargetCollisionList& collisionList)
{
    ...
    const uint64_t stageId = PXR_NS::UsdUtilsStageCache::Get().GetId(stage).ToLongInt();
...
void DebugVisualization::updatePointInstancerPrimDebugVisualization(const PXR_NS::UsdPrim& prim,
                                                                    float metersPerUnit,
                                                                    PXR_NS::UsdStageWeakPtr stage)
{
...
bool DebugVisualization::isTimeVaryingPrim(UsdStageWeakPtr stage, const UsdPrim& prim)
{
    ...
    {
        const PXR_NS::SdfPath parentPrimPath = parent.GetPath();
...
void DebugVisualization::updateUsdBasedDebugVisualizationInGroupsFabric(float metersPerUnit, PXR_NS::UsdStageWeakPtr stage)
{
    ...
    auto forEach = [&stage](const std::vector<usdrt::SdfPath>& vec, auto fn) {
        ...
        {
            const omni::fabric::Path pathC(sdfPath);
            const PXR_NS::SdfPath usdPath = omni::fabric::toSdfPath(pathC);
            const PXR_NS::UsdPrim& prim = stage->GetPrimAtPath(usdPath);
            if (!prim)
                continue;
            fn(prim);
        }
...

    {
        ...
        forEach(vecPointInst, [this, stage, metersPerUnit](const PXR_NS::UsdPrim& prim) {
    ...
    }
    {
        ...
        forEach(vecCollision, [this, stage, metersPerUnit](const PXR_NS::UsdPrim& prim) {
    ...
    }
    {
        ...
        forEach(vecVehicle, [this, stage, metersPerUnit](const PXR_NS::UsdPrim& prim) {
            if (mVehicleDebugVis.getVisualizationFlags())
            {
...
void DebugVisualization::updateUsdBasedDebugVisualizationInRange(const PXR_NS::UsdPrimRange range,
                                                                 float metersPerUnit,
                                                                 PXR_NS::UsdStageWeakPtr stage,
...
PXR_NS::GfMatrix4d DebugVisualization::GetLocalToWorldTransform(const PXR_NS::UsdPrim& prim)
{
    ...
    if (mFabricEnabled && mQueryFabricWhileSim && mSceneIsPlaying && mFabricSync)
    {
        ...
        const usdrt::GfMatrix4d matrix =
            mFabricSync->computeWorldXform(omni::fabric::convertToPathType<omni::fabric::Path>(
                iStageReaderWriter->getFabricId(stageInProgress), primPath));
        PXR_NS::GfMatrix4d convertedMat;
        memcpy(&convertedMat, &matrix, sizeof(PXR_NS::GfMatrix4d));
        return convertedMat;
    }
