...
public:
    ...
    struct Hash
    {
        ...
        {
            return PXR_NS::TfHash::Combine(obj.mObjType, static_cast<size_t>(obj.mMeshKey.getHashIndex()));
        }
    };
...
public:
    ...
    static void getShapeScale(const usdparser::PhysxShapeDesc* desc, PXR_NS::GfVec3f& scale);
    static void setMeshScale(usdparser::PhysxShapeDesc* desc, const carb::Float3& scale);
};

struct PrototypeCollDesc
{
    PrototypeCollDesc(const PXR_NS::SdfPath& primPath,
                      const PXR_NS::GfMatrix4d& matrix = PXR_NS::GfMatrix4d(1.0),
    ...
    const PXR_NS::GfMatrix4d matrix;
...
typedef PXR_NS::TfHashMap<ShapeUniqueKey, TriangleMeshBuffer, ShapeUniqueKey::Hash> TriangleMeshBufferMap;
...
public:
    ...
    typedef PXR_NS::TfHashMap<PXR_NS::SdfPath, RenderInstanceDesc, PXR_NS::SdfPath::Hash> RenderInstanceDescMap;
...
public:
    ...
    bool exists(const PXR_NS::SdfPath& path, const PXR_NS::GfMatrix4f& matrix);

    RenderInstanceDesc* get(const PXR_NS::SdfPath& path);

    void addOrUpdate(const PXR_NS::SdfPath& path,
                     omni::renderer::RenderInstanceBuffer renderInstanceBuffer,
                     size_t renderInstanceNum,
                     TriangleMeshBuffer* triangleMeshBuffer,
                     const PXR_NS::GfMatrix4f& matrix);

    bool getAndRemove(const PXR_NS::SdfPath& path,
                      omni::renderer::RenderInstanceBuffer& renderInstanceBuffer,
                      size_t& renderInstanceNum,
                      TriangleMeshBuffer*& triangleMeshBuffer,
                      PXR_NS::GfMatrix4f& matrix);
...
{
public:
    typedef PXR_NS::TfHashMap<PXR_NS::SdfPath, bool, PXR_NS::SdfPath::Hash> TimeVaryingPrimMap;
...
public:
    ...
    void onResyncedPrimPath(const PXR_NS::SdfPath&); // excluding resynced path notifications of attributes
    void onAttributeChange(const PXR_NS::SdfPath& attrPath,
                           const PXR_NS::SdfPath& primPath,
    ...
    bool HasPointInstancerParent(const PXR_NS::UsdPrim& prim);
    ...
    void setCameraPos(const PXR_NS::GfVec3f& worldPos)
    {
...
private:
    ...
    PXR_NS::GfMatrix4d GetLocalToWorldTransform(const PXR_NS::UsdPrim& prim);

    usdparser::PhysxShapeDesc* getShapeDesc(const PXR_NS::UsdPrim& prim, const PXR_NS::UsdPrim& colPrim, const uint64_t stageId);

    void updateCollisionPrimSimplifiedAABB(const PXR_NS::SdfPath& primPath, const PXR_NS::GfRange3d& primAABB);

    void updateCollisionPrimTriangeMeshShape(const PXR_NS::SdfPath& primPath,
                                             const PXR_NS::GfMatrix4d& primTransformMtx,
                                             usdparser::PhysxShapeDesc* shapeDesc);

    void removePrimFromAABBCache(const PXR_NS::SdfPath& primPath);
    void removePrimFromTrimeshCache(const PXR_NS::SdfPath& primPath);

    void updateUsdBasedDebugVisualization(float metersPerUnit, PXR_NS::UsdStageWeakPtr stage);

    void updateUsdBasedDebugVisualizationInRange(const PXR_NS::UsdPrimRange range,
                                                 float metersPerUnit,
                                                 PXR_NS::UsdStageWeakPtr stage,
                                                 bool ignorePrimsWithSelectionSupport);

    void updateUsdBasedDebugVisualizationInGroupsFabric(float metersPerUnit, PXR_NS::UsdStageWeakPtr stage);

    void updateCollisionPrimDebugVisualization(const PXR_NS::UsdPrim& prim, float metersPerUnit);

    void updatePointInstancerPrimDebugVisualization(const PXR_NS::UsdPrim& prim,
                                                    float metersPerUnit,
                                                    PXR_NS::UsdStageWeakPtr stage);

    void parsePrototype(const PXR_NS::UsdStageRefPtr stage, const PXR_NS::UsdPrim& usdPrim, TargetCollisionList& collisionList);
    ...
    bool isTimeVaryingPrim(PXR_NS::UsdStageWeakPtr stage, const PXR_NS::UsdPrim& prim);
...
private:
    ...
    PXR_NS::SdfPathSet mMergeMeshPathCache;
    ...
    PXR_NS::GfVec3f mCurrentCameraPos, mPrevCameraPos;
