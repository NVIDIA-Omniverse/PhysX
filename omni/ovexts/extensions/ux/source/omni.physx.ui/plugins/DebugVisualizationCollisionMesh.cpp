...
extern PXR_NS::UsdStageRefPtr gStage;  // Declare external reference to the global stage handle
...
typedef std::unordered_set<PXR_NS::SdfPath, PXR_NS::SdfPath::Hash> PathSet;
typedef std::unordered_map<PXR_NS::SdfPath,PXR_NS::SdfPath, PXR_NS::SdfPath::Hash> PathMap;

static PXR_NS::TfToken getCollisionMeshToken(void) // get the token we use to signify a prim is a debug collision mesh
{
    static PXR_NS::TfToken gCollisionMeshToken("CollisionMesh");
    return gCollisionMeshToken;
}

static PXR_NS::TfToken getCollisionMeshParentToken(void) // get the token we use to signify a prim is a debug collision mesh
{
...
* @param key : The key for this metadata expressed as a PXR_NS::TfToken
* @param value : A const char string to assign as the 'data' portion for this metadata
*/
static void setMetaData(PXR_NS::UsdPrim &prim, const PXR_NS::TfToken &key, const char *value)
{
    prim.SetMetadataByDictKey<const char *>(PXR_NS::SdfFieldKeys->CustomData, key, value);
}

static bool isCollisionMesh(const PXR_NS::UsdPrim &prim)
{
    bool ret = false;

    PXR_NS::TfToken ctoken = getCollisionMeshToken();
    std::string metadata;
...
static void hideSessionPrim(PXR_NS::UsdPrim &prim)
{
...
static void copyWorldTransform(const PXR_NS::UsdPrim &source,PXR_NS::UsdGeomXform &dest)
{
    PXR_NS::UsdGeomXformCache xfCache;
    PXR_NS::GfMatrix4d worldRel = xfCache.GetLocalToWorldTransform(source);
    PXR_NS::UsdGeomXformOp xform = dest.MakeMatrixXform();
    if (xform)
    {
        xform.Set(worldRel);
    }
...
static void setVisibility(PXR_NS::UsdPrim &prim,bool visible,bool isParent)
{
    ...
    if ( imageable )
    {
        ...
        {
            PXR_NS::TfToken computedVisible = imageable.ComputeVisibility();
            if ((computedVisible == PXR_NS::UsdGeomTokens->invisible) == visible)
            {
                CARB_LOG_INFO_ONCE("Solid Collision Mesh Visualization: Unable to override graphics visibility for instance proxy prims.");
            }
            return;
        }
...
public:
    void syncTransformToUSD(float diagonalDistance,float explodeDistance)
    {
        ...
        PXR_NS::UsdPrim p = gStage->GetPrimAtPath(mPrim);
        if ( p )
        {
            omni::usd::UsdUtils::ScopedLayerEdit scopedSessionLayerEdit(gStage, gStage->GetSessionLayer());
            PXR_NS::GfMatrix4d xform(1.0);
            xform.SetTranslate(PXR_NS::GfVec3d(dir.x,dir.y,dir.z));
...
{
public:
    void syncTransformToUSD(const PXR_NS::UsdPrim& prim)
    {
        PXR_NS::UsdPrim dest = gStage->GetPrimAtPath(mRootPrim);
        if (prim && dest)
        {
            PXR_NS::UsdGeomXform xform(dest);
            omni::usd::UsdUtils::ScopedLayerEdit scopedSessionLayerEdit(gStage, gStage->GetSessionLayer());
    ...
    bool syncRootTransformToFabric(omni::fabric::UsdStageId usdStageId, const PXR_NS::UsdPrim& prim)
    {
        return false;
        ...
        if (fabricStageId.id)
        {
            ...
            PXR_NS::GfMatrix4d& rootMat = *stage.getAttributeWr<PXR_NS::GfMatrix4d>(
                omni::fabric::convertToPathType<omni::fabric::Path>(stage.getFabricId(), mRootPrim), localMatrixToken);
            if (prim)
            {
                rootMat = PXR_NS::UsdGeomXformable(prim).ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
            }
            return true;
        }
        return false;
    ...
    static void updateTransformBucket(omni::fabric::StageReaderWriter& stage, omni::fabric::PrimBucketList& primBuckets)
    {
        ...
        for (size_t i = 0; i != primBuckets.bucketCount(); i++)
        {
            ...
            // loop
            gsl::span<PXR_NS::GfMatrix4d> positions =
                stage.getAttributeArrayWr<PXR_NS::GfMatrix4d>(primBuckets, i, localMatrixToken);
    ...
    void setRootPrim(const PXR_NS::SdfPath &parentPrim,    // The prim we are mirroring
                     const PXR_NS::SdfPath &rootPrim,      // the xform we are using to mirror in the session layer
    ...
    void addCollisionPrim(const PXR_NS::SdfPath &parentPrim,
                          const PXR_NS::SdfPath &cprim,
    ...
    void setDisplayMode(const PXR_NS::SdfPath &primName, // Parent prim
                        CollisionMeshDisplayMode mode)
    {
        ...
        PXR_NS::UsdPrim prim = gStage->GetPrimAtPath(primName);
        if ( prim)
        {
        ...
        for (auto &i:mCollisionPrims)
        {
            PXR_NS::UsdPrim cprim = gStage->GetPrimAtPath(i.mPrim);
            if ( cprim )
            {
    ...
    PXR_NS::SdfPath                mRootPrim;          // The xform that collision prims are attached to
...
using CollisionPrimsMap = std::unordered_map< PXR_NS::SdfPath, CollisionMesh, PXR_NS::SdfPath::Hash>;
...
public:
    ...
    void setDisplayMode(const PXR_NS::SdfPath &primName, CollisionMeshDisplayMode mode)
    {
...
using CollisionRootMap = std::unordered_map< PXR_NS::SdfPath, CollisionRoot, PXR_NS::SdfPath::Hash>;
        ...
        public:
            DebugVisualizationCollisionMeshImpl(PXR_NS::UsdGeomXformCache& xformcache) : mXformCache(xformcache)
            {
            ...
            virtual void updateDebugVisualization(void) final
            {
                ...
                if (gStage)
                {
                    const uint64_t stageId = PXR_NS::UsdUtilsStageCache::Get().GetId(gStage).ToLongInt();
                    usdStageId.id = stageId;
                }
                for (const PXR_NS::SdfPath& path : mPathCache)
                {
                    const PXR_NS::UsdPrim prim = gStage->GetPrimAtPath(path);
                    if ( prim )
                    {
            ...
            std::string getFlattenedPrimName(const PXR_NS::UsdPrim &prim)
            {
            ...
            void updateCollisionPrimDebugVisualization(omni::fabric::UsdStageId usdStageId, const PXR_NS::UsdPrim& colPrim)
            {
                const uint64_t stageId = usdStageId.id;

                PXR_NS::UsdPrimRange range(colPrim, PXR_NS::UsdTraverseInstanceProxies());
                ...
                for (PXR_NS::UsdPrimRange::const_iterator iter = range.begin(); iter != range.end(); ++iter)
                {
                    ...
                    if ( prim.HasAPI<PXR_NS::UsdPhysicsCollisionAPI>() )
                    {
                        ...
                        if (!shapeDesc)
                        {
                            ...
                            static PXR_NS::TfToken oldConvexPrim("ConvexMesh");

                            if (!(prim.IsA<PXR_NS::UsdGeomGprim>() || prim.GetTypeName() == oldConvexPrim))
                                continue;
                        ...
                        if ( gPhysXVisualization->getMeshKey(*shapeDesc,meshKey))
                        {
                            ...
                            if ( !syncTransform )
                            {
                                ...
                                if ( cr )
                                {
                                    PXR_NS::SdfPath parentPath = prim.GetPath();
                                    // create the prim here!
                                    {
...

                                        {
                                            ...
                                            PXR_NS::SdfPath scopeName("/CollisionMeshes");
                                            PXR_NS::UsdPrim scopePrim = gStage->GetPrimAtPath(scopeName);
                                            if (!scopePrim)
                                            {
                                                PXR_NS::UsdGeomScope::Define(gStage, scopeName);
                                                PXR_NS::UsdPrim prim = gStage->GetPrimAtPath(scopeName);
                                        ...
                                        cmesh.setRootPrim(prim.GetPath(),
                                                          PXR_NS::SdfPath(collisionName.c_str()),
                                                          diagonalAxisLength,
                                                          mCollisionPaths);
                                        ...
                                        {
                                            PXR_NS::UsdGeomXform xform = PXR_NS::UsdGeomXform::Define(gStage, cmesh.mRootPrim);
                                            PXR_NS::UsdPrim destPrim = gStage->GetPrimAtPath(cmesh.mRootPrim);
                                            PXR_NS::UsdPrim sourcePrim = prim;
                                            if (sourcePrim && destPrim)
                                            {
                                                hideSessionPrim(destPrim);

                                                PXR_NS::TfToken token = getCollisionMeshToken();
                                                ...
                                                for (uint32_t i=0; i<cr->meshCount; i++)
                                                {
                                                    ...
                                                    PXR_NS::SdfPath collisionPath(scratch);
                                                    ...
                                                    PXR_NS::UsdPrim meshPrim = gStage->GetPrimAtPath(PXR_NS::SdfPath(scratch));
            ...
            bool addPathCache(const PXR_NS::SdfPath &path)
            {
                bool ret = false;
            ...
            bool removePathCache(const PXR_NS::SdfPath &path)
            {
                bool ret = false;
            ...
            virtual bool releaseDescCache(const PXR_NS::SdfPath& path) final
            {
                bool ret = false;

                PXR_NS::UsdPrim colPrim = gStage->GetPrimAtPath(path);
                if ( colPrim )
                {
                    PXR_NS::UsdPrimRange range(colPrim, PXR_NS::UsdTraverseInstanceProxies());
                    // Iterate through all of the children looking for prims with a collision API
            ...
            virtual bool isXformPathTracked(const PXR_NS::SdfPath &path) const final
            {
                bool ret = false;
            ...
            bool addCollisionPrim(const PXR_NS::SdfPath &collisionPath,const PXR_NS::SdfPath &primPath)
            {
                bool ret = false;
            ...
            bool removeCollisionPrim(const PXR_NS::SdfPath &collisionPath)
            {
                bool ret = false;
            ...
            bool collisionToPrim(const PXR_NS::SdfPath &collisionPath,PXR_NS::SdfPath &primPath)
            {
                bool ret = false;
            ...
            std::vector<std::string> remapSelection(void)
            {
                ...
                {
                    const PXR_NS::SdfPath path(pathStr);
                    PXR_NS::UsdPrim prim = gStage->GetPrimAtPath(path);
                    if ( isCollisionMesh(prim))
                    {
                        needToRedirect = true;
                        PXR_NS::SdfPath remap;
...
DebugVisualizationCollisionMesh *DebugVisualizationCollisionMesh::create(PXR_NS::UsdGeomXformCache& xformcache)
{
