...
{
public:
    static DebugVisualizationCollisionMesh* create(PXR_NS::UsdGeomXformCache& xformcache);

    virtual void updateDebugVisualization(void) = 0;

    virtual bool refreshSelectionSet(void) = 0;

    virtual bool releaseDescCache(const PXR_NS::SdfPath& path) = 0;
    ...
    virtual bool isXformPathTracked(const PXR_NS::SdfPath& path) const = 0;
