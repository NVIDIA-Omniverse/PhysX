...
namespace ui
{
    ...
    void FixedTendonVisualizer::selectionChanged()
    {
        ...
        {
            mSelectedPaths.push_back(PXR_NS::SdfPath(path));
        }

        recomputeDrawList();
    }

    bool FixedTendonVisualizer::isSelected(const PXR_NS::SdfPath pathToCheck) const
    {
        for (const PXR_NS::SdfPath path : mSelectedPaths)
        {
    ...
    void FixedTendonVisualizer::draw(const PXR_NS::GfMatrix4d& viewMatrix,
        ...
        const carb::Float4& viewPortRect) const
    {
        ...
        else
        {
            radius = gCircleRadius * gSettings->getAsFloat(kViewportGizmoScalePath) /
                static_cast<float>(PXR_NS::UsdGeomGetStageMetersPerUnit(gStage));
        }
    ...
    bool FixedTendonVisualizer::hasTendon(const PXR_NS::SdfPath path)
    {
