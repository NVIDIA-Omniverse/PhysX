...
DebugVisualizationVehicle::DebugVisualizationVehicle(PXR_NS::UsdGeomXformCache& xformcache)
...
bool DebugVisualizationVehicle::isXformPathTracked(const PXR_NS::SdfPath& path)
{
...
void DebugVisualizationVehicle::updateVehicle(const PXR_NS::UsdPrim& prim,
                                              PXR_NS::UsdStageWeakPtr stage)
{
    ...
    const PXR_NS::SdfPath path = prim.GetPath();
    ...
    if (vehicleDesc)
    {
        ...
        CARB_ASSERT(prim.HasAPI<PXR_NS::UsdPhysicsMassAPI>());  // note: once referenceFrameIsCenterOfMass gets removed, this
                                                             //       will likely not be required anymore
        CARB_ASSERT(prim.IsA<PXR_NS::UsdGeomXformable>());

        const PXR_NS::GfMatrix4d vehiclePoseWorld = mXformCache.GetLocalToWorldTransform(prim);
        const PXR_NS::GfTransform tr(vehiclePoseWorld);
        const PXR_NS::GfQuatf vehicleOrientWorld(tr.GetRotation().GetQuat());
        // note: GfMatrix4d::ExtractRotationQuat() etc. gives poor results when scale is involved,
        //       thus using GfTransform

        PXR_NS::GfVec3f vehicleRefPosWorld;
        PXR_NS::GfRotation vehicleRefOrientWorld;

        if (vehicleDesc->referenceFrameIsCenterOfMass)  // deprecated
        {
            PXR_NS::UsdPhysicsMassAPI massAPI(prim);
            PXR_NS::UsdAttribute centerOfMassAttr = massAPI.GetCenterOfMassAttr();
            PXR_NS::GfVec3f centerOfMassPosLocal;
            ...
            // in)
            vehicleRefPosWorld = PXR_NS::GfVec3f(vehiclePoseWorld.TransformAffine(centerOfMassPosLocal));

            PXR_NS::UsdAttribute principalAxesAttr = massAPI.GetPrincipalAxesAttr();
            PXR_NS::GfQuatf centerOfMassOrientLocal;
            principalAxesAttr.Get(&centerOfMassOrientLocal);
            const PXR_NS::GfVec3f& imag = centerOfMassOrientLocal.GetImaginary();
            if ((centerOfMassOrientLocal.GetReal() != 0.0f) ||
                (imag[0] != 0.0f) ||
                (imag[1] != 0.0f) ||
                (imag[2] != 0.0f))
            {
                const PXR_NS::GfQuatf centerOfMassOrientWorld = (vehicleOrientWorld * centerOfMassOrientLocal).GetNormalized();
        ...
        {
            vehicleRefPosWorld = PXR_NS::GfVec3f(tr.GetTranslation());
            vehicleRefOrientWorld = tr.GetRotation();
        }

        for (const usdparser::WheelAttachmentDesc& wheelAttDesc : vehicleDesc->wheelAttachments)
        {
            const PXR_NS::GfVec3f suspDirLocal(wheelAttDesc.suspensionTravelDirection.x,
                wheelAttDesc.suspensionTravelDirection.y, wheelAttDesc.suspensionTravelDirection.z);

            PXR_NS::GfVec3f suspFramePosLocal;
            if (wheelAttDesc.state & usdparser::WheelAttachmentDesc::eHAS_SUSPENSION_FRAME)
            {
                ...
                suspFramePosLocal = PXR_NS::GfVec3f(
            ...
            else
            {
                ...
                const PXR_NS::GfVec3f wheelCenterOfMassLocal(
            ...
            const PXR_NS::GfVec3f suspFramePosWorld = vehicleRefPosWorld + PXR_NS::GfVec3f(vehicleRefOrientWorld.TransformDir(suspFramePosLocal));

            const PXR_NS::GfVec3f suspDirWorld = PXR_NS::GfVec3f(vehicleRefOrientWorld.TransformDir(suspDirLocal));
            ...
            if (wheelAttDesc.state & usdparser::WheelAttachmentDesc::eMANAGE_TRANSFORMS)
            {
                ...
                // is used.
                const PXR_NS::GfVec3f invWheelFramePosLocal(
                ...
                PXR_NS::UsdPrim wheelAttPrim = stage->GetPrimAtPath(wheelAttDesc.path);
                const PXR_NS::GfMatrix4d wheelPoseWorld = mXformCache.GetLocalToWorldTransform(wheelAttPrim);
                const PXR_NS::GfTransform trw(wheelPoseWorld);
                // note: GfMatrix4d::ExtractRotationQuat() etc. gives poor results when scale is involved,
                //       thus using GfTransform
                const PXR_NS::GfVec3f wheelPosWorld(trw.GetTranslation());
                const PXR_NS::GfRotation wheelRotWorld = trw.GetRotation();

                const PXR_NS::GfVec3f suspPosWorld = wheelPosWorld + PXR_NS::GfVec3f(wheelRotWorld.TransformDir(invWheelFramePosLocal));
            ...
            const PXR_NS::GfVec3f maxDroopPos = suspFramePosWorld + (suspDirWorld * suspTravelDistance);
            const PXR_NS::GfVec3f suspPos = suspFramePosWorld + (suspDirWorld * currentSuspDelta);
...
void DebugVisualizationVehicle::updateTrackedVehicles(PXR_NS::UsdStageWeakPtr stage)
{
    CARB_PROFILE_ZONE(0, "DebugVisualization::vehicleUpdate");

    releaseLineBuffer();

    for (const PXR_NS::SdfPath& path : mVehiclePathCache)
    {
        const PXR_NS::UsdPrim prim = stage->GetPrimAtPath(path);
