...
void DeformableBodyVisualizationManager::updateProxyProperties(UsdGeomXformCache& xformCache)
{
    ...
    {
        // Skip if topology is also pending: updateTopology sets both indices and points atomically,
        // so running updatePoints first would create a transient mismatch (stale indices, new points).
        if (mBufferDeformablePathsToUpdateTopology.find(actualDeformableBodyPath) == mBufferDeformablePathsToUpdateTopology.end())
        {
