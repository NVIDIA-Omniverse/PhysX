...
void DescCache::addDesc(const PXR_NS::SdfPath& path, ObjectCategory cat, PhysxObjectDesc* desc)
{
    mObjectDescMap.insert(std::make_pair(path, std::make_pair(cat, desc)));
}

void DescCache::releaseDesc(const PXR_NS::SdfPath& path, PhysxObjectDesc* desc)
{
...
PhysxObjectDesc* DescCache::getDesc(const PXR_NS::SdfPath& path, ObjectCategory cat) const
{
    ObjectDescMap::const_iterator it = mObjectDescMap.find(path);
    while (it != mObjectDescMap.end() && it->first == path)
    {
...
void DescCache::releasePath(const PXR_NS::SdfPath& path)
{
