...
static void handleFixedTendonsVisualizer(const class PXR_NS::UsdNotice::ObjectsChanged& objectsChanged)
{
    ...
    for (const PXR_NS::SdfPath& infoPath : objectsChanged.GetChangedInfoOnlyPaths())
    {
...
static void handleStandardManager(StandardManager* manager, const char* zoneName, const PXR_NS::UsdNotice::ObjectsChanged& objectsChanged)
{
...
void UsdNoticeListener::handle(const PXR_NS::UsdNotice::ObjectsChanged& objectsChanged)
{
    ...
    if (gDebugVisualization && gDebugVisualization->isUsdBasedVisualizationEnabled())
    {
        ...
        for (const SdfPath& path : objectsChanged.GetChangedInfoOnlyPaths())
        {
            ...
            {
                if (parent.IsA<PXR_NS::UsdGeomCamera>())
                    isCamera = true;
