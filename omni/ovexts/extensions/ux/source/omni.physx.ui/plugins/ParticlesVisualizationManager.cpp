...
#define ON_ATTRIBUTE_CHANGE_WRAPPER(name)\
    void name(ProxyVisualizationClient& client, PXR_NS::SdfPath path, PXR_NS::TfToken attributeName)\
...
bool ParticlesVisualizationManager::needsStageParse(PXR_NS::UsdStageWeakPtr stage)
{
    if (!isActive())
        return false;
...
void ParticlesVisualizationManager::handlePrimResync(SdfPath path)
{
    ...
    // check for updates
    if (mParticleSets.count(path) || mBufferParticleObjectsToRemove.count(path) > 0)
    {
...
{
    if (mParticleSets.count(path) > 0 || mParticleSystems.count(path) > 0)
    {
...
uint32_t ParticlesVisualizationManager::updateActiveProxies(ProxyInfo& proxyInfo, SdfPath actualPath)
{
    ...
    {
        ParticleObjectProxyInfo& particleObjectProxyInfo = static_cast<ParticleObjectProxyInfo&>(proxyInfo);
        bool showParticles = showParticleSetParticles();
        if (showParticles)
        {
    ...
    if (proxyInfo.type == ProxyInfoType::eParticleSystem)
    {
...
void ParticlesVisualizationManager::updateParticleObjectTransform(SdfPath actualPath, PXR_NS::UsdGeomXformCache& xformCache)
{
...
void ParticlesVisualizationManager::updateParticleObjectMesh(SdfPath actualPath)
{
    ...
    if (proxyPointInstancer)
    {
        ...
        //color
        GfVec3f color = isFluid ? kColorFluidParticle : kColorSolidParticle;
...
void ParticlesVisualizationManager::updateParticleSystemSurfaceMesh(SdfPath actualPath, PXR_NS::UsdGeomXformCache& xformCache)
{
...
void ParticlesVisualizationManager::updateParticleSystemDiffusePoints(SdfPath actualPath, PXR_NS::UsdGeomXformCache& xformCache)
{
    ParticleSystemProxyInfo* particleSystemProxyInfo = getParticleSystemProxyInfo(actualPath);
...
ParticlesVisualizationManager::ParticleObjectProxyInfo* ParticlesVisualizationManager::getParticleObjectProxyInfo(SdfPath actualParticleObjectPath)
{
    ...
    return (proxyInfo && proxyInfo->type == ProxyInfoType::eParticleSet) ? static_cast<ParticleObjectProxyInfo*>(proxyInfo) : nullptr;
}
