...
{
    PXR_NS::UsdStageWeakPtr stage = PXR_NS::UsdUtilsStageCache::Get().Find(PXR_NS::UsdStageCache::Id::FromLongInt(long(stageID)));
    PXR_NS::SdfPath jointPath = intToPath(jointUSDPath);
    PXR_NS::UsdPrim jointPrim = stage->GetPrimAtPath(jointPath);
    ...
    if (scaleAll && jointPrim.IsA<PXR_NS::PhysxSchemaPhysxPhysicsRackAndPinionJoint>())
    {
        PXR_NS::PhysxSchemaPhysxPhysicsRackAndPinionJoint rackJoint(jointPrim);
        float rackRatio;
...
bool PhysXJointAuthoring::OnJointGetCustomBillboard(uint64_t stageID, uint64_t jointUSDPath, uint32_t& assetId)
{
    ...
    PXR_NS::SdfPath jointPath = intToPath(jointUSDPath);
    PXR_NS::UsdPrim jointPrim = stage->GetPrimAtPath(jointPath);
    if (jointPrim.IsA<PXR_NS::PhysxSchemaPhysxPhysicsRackAndPinionJoint>())
    {
        assetId = mBillboardAssetIds[1];
        return true;
    }
    else if (jointPrim.IsA<PXR_NS::PhysxSchemaPhysxPhysicsGearJoint>())
    {
