...
void SpatialTendonManager::update()
{
    ...
    if (mFabricEnabled && mFabricSync)
    {
        ...
        for (const auto& bodyPath : mVisibleBodies)
        {
            ...
            PXR_NS::GfMatrix4d gfMatrix;
            memcpy(&gfMatrix, &matrix, sizeof(PXR_NS::GfMatrix4d));
...
void SpatialTendonManager::traverseTendonVisibility(const SdfPath bodyPath, const TokenToTokenMap& attachments)
{
    ...
    for (const std::pair<TfToken, TfToken>& attachment : attachments)
    {
        ...
        // 2. children
        {
            ...
            {
                // there are children:
                const PXR_NS::SdfPathSet& children = parentToChildrenIt->second;
...
void SpatialTendonManager::setVisible(const PXR_NS::SdfPath bodyPath)
{
...
void SpatialTendonManager::setInvisible(const PXR_NS::SdfPath bodyPath)
{
...
bool SpatialTendonManager::isRootAttachment(const PXR_NS::SdfPath attachmentPath)
{
...
bool SpatialTendonManager::isLeafAttachment(const PXR_NS::SdfPath attachmentPath)
{
    const TfToken apiToken = getAttachmentAPI(attachmentPath);
    if (apiToken.IsEmpty())
        return false;
...
PXR_NS::GfMatrix4d SpatialTendonManager::getBodyTransform(const PXR_NS::SdfPath& bodyPath,
