...
typedef PXR_NS::TfHashSet<PXR_NS::SdfPath, PXR_NS::SdfPath::Hash> PrimAddMap;


static void onPhysicsObjectCreation(const PXR_NS::SdfPath& sdfPath, omni::physx::usdparser::ObjectId objectId,
    omni::physx::PhysXType type, void* userData)
{
    if (type == omni::physx::ePTVehicleController)
    {
        const PXR_NS::UsdPrim& prim = gStage->GetPrimAtPath(sdfPath);
...
static void onPhysicsObjectDestruction(const PXR_NS::SdfPath& sdfPath, omni::physx::usdparser::ObjectId objectId,
    omni::physx::PhysXType type, void* userData)
{
...
void updateControllers(float elapsedSecs)
{
    if (gVehicleManager)
    {
        gVehicleManager->update(elapsedSecs, PXR_NS::UsdTimeCode::Default());
    }
}
...
CARB_EXPORT void carbOnPluginStartup()
{
    ...
    gUsdNoticeListenerKey = PXR_NS::TfNotice::Register(PXR_NS::TfCreateWeakPtr(gUsdNoticeListener), &UsdNoticeListener::handle);
...
{
    return gVehicleManager->getInputEnabled(PXR_NS::SdfPath(path));
}

void setInputEnabled(const char* path, bool inputEnabled)
{
    gVehicleManager->setInputEnabled(PXR_NS::SdfPath(path), inputEnabled);
}

bool getMouseEnabled(const char* path)
{
    return gVehicleManager->getMouseEnabled(PXR_NS::SdfPath(path));
}

void setMouseEnabled(const char* path, bool mouseEnabled)
{
    gVehicleManager->setMouseEnabled(PXR_NS::SdfPath(path), mouseEnabled);
}

bool getAutoReverseEnabled(const char* path)
{
    return gVehicleManager->getAutoReverseEnabled(PXR_NS::SdfPath(path));
}

void setAutoReverseEnabled(const char* path, bool autoReverseEnabled)
{
    gVehicleManager->setAutoReverseEnabled(PXR_NS::SdfPath(path), autoReverseEnabled);
}

float getSteeringSensitivity(const char* path)
{
    return gVehicleManager->getSteeringSensitivity(PXR_NS::SdfPath(path));
}
...
void setSteeringSensitivity(const char* path, float steeringSensitivity)
{
    ...
    gVehicleManager->setSteeringSensitivity(PXR_NS::SdfPath(path), steeringSensitivity);
}

float getSteeringFilterTime(const char* path)
{
    return gVehicleManager->getSteeringFilterTime(PXR_NS::SdfPath(path));
}
...
void setSteeringFilterTime(const char* path, float steeringFilterTime)
{
    ...
    gVehicleManager->setSteeringFilterTime(PXR_NS::SdfPath(path), steeringFilterTime);
}
...
static UsdPrim getVehiclePrim(const PXR_NS::UsdPrim& wheelAttachmentPrim)
{
...
static bool computeSuspensionFrameTransformsUSDInternal(PXR_NS::UsdPrim& wheelAttachmentPrim,
                                                        ...
                                                        UsdGeomXformCache& xfCache)
{
    ...
    if (wheelAttachmentPrim.IsA<UsdGeomXformable>())
    {
        ...
        const PXR_NS::GfTransform tr(primToWorld);
...
static bool computeSuspensionFrameTransformsUSDInternal(PXR_NS::UsdPrim& vehiclePrim, PXR_NS::UsdPrim* wheelAttachmentPrim)
{
    ...
    if (vehiclePrim.GetMetadataByDictKey(PXR_NS::SdfFieldKeys->CustomData, PhysxSchemaTokens->referenceFrameIsCenterOfMass, &referenceFrameIsCenterOfMassVal))
    {
    ...
    if (referenceFrameIsCenterOfMass)  // deprecated
    {
        ...
        GfVec3f centerOfMassWorldPos = PXR_NS::GfVec3f(primToWorld.TransformAffine(centerOfMassLocal));  // note: takes scale into account
...
bool computeSuspensionFrameTransformsUSD(const PXR_NS::SdfPath& path)
{
    ...
    if (prim)
    {
        ...
        {
            PXR_NS::UsdPrim vehiclePrim = getVehiclePrim(prim);
...
{
    return computeSuspensionFrameTransformsUSD(PXR_NS::SdfPath(path));
}
...
void detachStage(bool regToStageUpdate)
{
    ...
    {
        stage = gStage;
        stageId = PXR_NS::UsdUtilsStageCache::Get().GetId(stage).ToLongInt();
    }
    else
    {
        stageId = 0;
    ...
    if (regToStageUpdate)
    {
        if (stage)
        {
            onAttach(stageId, PXR_NS::UsdGeomGetStageMetersPerUnit(stage), nullptr);
        }
        registerToStageUpdate();
    }
}
