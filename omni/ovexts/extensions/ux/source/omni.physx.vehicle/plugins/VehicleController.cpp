...
VehicleController::VehicleController(const PXR_NS::UsdPrim& prim, const VehicleControllerSettings& vehicleControllerSettings)
...
VehicleController* VehicleController::create(const PXR_NS::UsdPrim& prim, const VehicleControllerSettings& vehicleControllerSettings,
...
void VehicleController::update(float timeStep, const PXR_NS::UsdTimeCode& timeCode)
{
    const Cache& cache = getCache();
...
void VehicleController::updateReverse(const Cache& cache, float timeStep)
{
    ...
    if (dynamicActor)
    {
        ...
        float speed = dynamicActor->getLinearVelocity().magnitude() * (float)PXR_NS::UsdGeomGetStageMetersPerUnit(gStage);
