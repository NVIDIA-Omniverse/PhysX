...
VehicleControllerSettings* VehicleManager::getVehicleSettings(const PXR_NS::SdfPath& path)
{
...
bool VehicleManager::getInputEnabled(const PXR_NS::SdfPath& path)
{
    return getVehicleSettings(path)->mInputEnabled;
}

void VehicleManager::setInputEnabled(const PXR_NS::SdfPath& path, bool inputEnabled)
{
...
bool VehicleManager::getMouseEnabled(const PXR_NS::SdfPath& path)
{
    return getVehicleSettings(path)->mMouseEnabled;
}

void VehicleManager::setMouseEnabled(const PXR_NS::SdfPath& path, bool mouseEnabled)
{
    getVehicleSettings(path)->mMouseEnabled = mouseEnabled;
}

bool VehicleManager::getAutoReverseEnabled(const PXR_NS::SdfPath& path)
{
    return getVehicleSettings(path)->mAutoReverseEnabled;
}

void VehicleManager::setAutoReverseEnabled(const PXR_NS::SdfPath& path, bool autoReverseEnabled)
{
    getVehicleSettings(path)->mAutoReverseEnabled = autoReverseEnabled;
}

float VehicleManager::getSteeringSensitivity(const PXR_NS::SdfPath& path)
{
    return getVehicleSettings(path)->mSteeringSensitivity;
}

void VehicleManager::setSteeringSensitivity(const PXR_NS::SdfPath& path, float steeringSensitivity)
{
    getVehicleSettings(path)->mSteeringSensitivity = steeringSensitivity;
}

float VehicleManager::getSteeringFilterTime(const PXR_NS::SdfPath& path)
{
    return getVehicleSettings(path)->mSteeringFilterTime;
}

void VehicleManager::setSteeringFilterTime(const PXR_NS::SdfPath& path, float steeringFilterTime)
{
...
VehicleController* VehicleManager::getVehicle(const PXR_NS::SdfPath& path)
{
    VehicleControllerMap::iterator it = mVehicleControllerMap.find(path);
...
void VehicleManager::vehicleParamDirty(const PXR_NS::SdfPath& vehiclePath, const PXR_NS::TfToken& token,
    VehicleController& vehicleController, const PXR_NS::UsdTimeCode& timeCode)
{
    if (token == PhysxSchemaTokens->physxVehicleVehicleEnabled)
    {
        const PXR_NS::UsdPrim& vehPrim = gStage->GetPrimAtPath(vehiclePath);
...
void VehicleManager::addController(const PXR_NS::UsdPrim& vehiclePrim, usdparser::ObjectId objectId)
{
    if (!vehiclePrim.HasAPI<PhysxSchemaPhysxVehicleTankControllerAPI>())
    {
        const PXR_NS::SdfPath& vehiclePath = vehiclePrim.GetPath();
...
void VehicleManager::removeController(const PXR_NS::SdfPath& vehiclePath)
{
...
void VehicleManager::update(float timeStep, const PXR_NS::UsdTimeCode& timeCode)
{
    if (!gStage)
    {
...
void VehicleManager::onUsdObjectChange(const PXR_NS::SdfPath& path, const PXR_NS::UsdTimeCode& timeCode)
{
