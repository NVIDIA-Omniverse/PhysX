...
static PXR_NS::GfMatrix4d getWorldSpaceTransform(UsdStageWeakPtr stage, const omni::physics::schema::JointDesc* desc, bool forBody0)
{
    const PXR_NS::SdfPath usePrimPath = (forBody0 ? desc->body0 : desc->body1);
    const PXR_NS::GfVec3f localPos = (forBody0 ? desc->localPose0Position : desc->localPose1Position);
    const PXR_NS::GfQuatf localRot = (forBody0 ? desc->localPose0Orientation : desc->localPose1Orientation);
...
static PXR_NS::GfMatrix4d getWorldSpacePose(UsdStageWeakPtr stage, const JointDesc* desc, bool forMesh)
{
    // default to using the transform from body0
    PXR_NS::GfMatrix4d worldPose = getWorldSpaceTransform(stage, desc, true);

    // then figure out the rotation component
    PXR_NS::GfVec3f worldAxis = {0, 0, 1.0f};
    switch (desc->type)
    {
        // default is to use the x-axis of the body
        default:
            worldAxis = PXR_NS::GfVec3f(worldPose.GetRow3(Axis::eX));
            break;

        // these joints define a specific axis in body space
        case ObjectType::eJointPrismatic:
            worldAxis = PXR_NS::GfVec3f(worldPose.GetRow3(((PrismaticJointDesc*)desc)->axis));
            break;
        case ObjectType::eJointSpherical:
            worldAxis = PXR_NS::GfVec3f(worldPose.GetRow3(((SphericalJointDesc*)desc)->axis));
            break;
        case ObjectType::eJointRevolute:
            worldAxis = PXR_NS::GfVec3f(worldPose.GetRow3(((RevoluteJointDesc*)desc)->axis));
            break;

        // distance joint defines its axis as the vector between the attach points
        case ObjectType::eJointDistance:
        {
            const PXR_NS::GfVec3d body0Pos = worldPose.ExtractTranslation();
            const PXR_NS::GfVec3d body1Pos = getWorldSpaceTransform(stage, desc, false).ExtractTranslation();

            // only have to calculate the axis vector if this is for the joint gizmo mesh
            if (forMesh)
            {
                worldAxis = PXR_NS::GfVec3f(body1Pos - body0Pos);
            ...
            const PXR_NS::GfVec3d avgPos = 0.5 * (body1Pos + body0Pos);
    ...
    if (forMesh)
    {
        const PXR_NS::GfVec3f xAxis(1.0f, 0.0f, 0.0f);
        const PXR_NS::GfRotation rotW = PXR_NS::GfRotation(xAxis, worldAxis);
...
JointAuthoring::JointAuthoring(PXR_NS::UsdStageWeakPtr stage, const PXR_NS::SdfPath& jointPath, const PXR_NS::SdfPath& selectedPrimPath)
{
    mStage = stage;
...
PXR_NS::SdfPath GetJointPrimPath(const JointDesc* desc)
{
...
static void calcLocalPoseMatrix(
    ...
    const PXR_NS::UsdAttribute& localPosAttr,
    const PXR_NS::UsdAttribute& localRotAttr
)
{
    ...
    if (relPrim)
    {
        ...
        else
        {
            ...
            const PXR_NS::GfMatrix4d newMatrix = scaleMat * matR.GetInverse() * refMatrix;
...
void JointAuthoringManager::selectionChanged()
{
    ...
    for (std::string pathStr : selectedPaths)
    {
        ...
        {
            const PXR_NS::SdfPathSet& jointPaths = bodyItr->second;
        ...
        else 
        {
            ...
            if (prim.IsA<UsdPhysicsJoint>())
            {
                ...
                mSelectedJointPrim = (selectedPaths.size() == 1 ? path : PXR_NS::SdfPath());
...
void JointAuthoringManager::setupSelectionOutline(const PXR_NS::UsdPrim& prim)
{
    const auto usdContext = omni::usd::UsdContext::getContext();
    ...
    for (int i = 0; i < 2; i++)
    {
        if (body[i] != SdfPath())
        {
            ...
            {                
                ...
                {
                    const PXR_NS::UsdPrim& meshPrim = *iter;
                    if (!meshPrim)
                        continue;
...
void JointAuthoringManager::updateJointVisibility(const PXR_NS::SdfPath& topPath)
{
    const UsdPrim topPrim = mStage->GetPrimAtPath(topPath);
    // A.B. this has to run on the traversal
    PXR_NS::UsdPrimRange range(topPrim);
...
void JointAuthoringManager::draw(const PXR_NS::GfMatrix4d& viewMatrix, const PXR_NS::GfMatrix4d& projMatrix, const carb::Float4& viewPortRect, bool clipPositiveZ)
{
...
void JointAuthoringManager::parseStage(PXR_NS::UsdStageWeakPtr stage)
{
...
void JointAuthoringManager::removeJointFromBodies(const PXR_NS::SdfPath& jointPath)
{
    // remove the given joint path from all entries in the body map
    std::vector<PXR_NS::SdfPath> bodiesToRemove;
...
bool JointAuthoringManager::removePrim(const PXR_NS::SdfPath& primPath)
{
    ...
    {
        const PXR_NS::SdfPath& path = iterator.getDescendentsPaths()[i];
...
void JointAuthoringManager::addOrUpdateBillboard(uint32_t assetId, uint32_t& billboardId, uint32_t billboardUid, const PXR_NS::GfMatrix4f& localTransform)
{
...
void JointAuthoringManager::resyncPrim(const PXR_NS::SdfPath& path)
{
    if(mHidden)
...
void JointAuthoringManager::mapBodyToJoint(const PXR_NS::SdfPath& bodyPath, const PXR_NS::SdfPath& jointPath)
{
    if (bodyPath != SdfPath())
    {
...
void JointAuthoringManager::updateJointBodyPaths(const PXR_NS::SdfPath& jointPath, bool body0)
{
...
void JointAuthoringManager::clearBufferedUpdatesForJointBodyPaths(const PXR_NS::SdfPath& jointPath)
{
        ...
        if (jointPrim && jointPrim.IsA<UsdPhysicsJoint>())
        {
            ...
            if (desc)
            {
                ...
                if (ref.second & 1)
                {
                    ...
                    const PXR_NS::UsdAttribute localPosAttr = physicsJoint.GetLocalPos0Attr();
                    const PXR_NS::UsdAttribute localRotAttr = physicsJoint.GetLocalRot0Attr();
                ...
                if (ref.second & 2)
                {
                    ...
                    const PXR_NS::UsdAttribute localPosAttr = physicsJoint.GetLocalPos1Attr();
                    const PXR_NS::UsdAttribute localRotAttr = physicsJoint.GetLocalRot1Attr();
                    SdfPathVector targets;
...
void JointAuthoringManager::resyncPath(const PXR_NS::SdfPath& resyncPath)
{
    UsdGeomXformCache xfCache;

    const uint64_t stageId = PXR_NS::UsdUtilsStageCache::Get().GetId(mStage).ToLongInt();
    const UsdPrim resyncPrim = mStage->GetPrimAtPath(resyncPath);
    PXR_NS::UsdPrimRange range(resyncPrim);
    for (PXR_NS::UsdPrimRange::iterator iter = range.begin(); iter != range.end(); ++iter)
    {
        PXR_NS::UsdPrim jointPrim = *iter;
        if (jointPrim && jointPrim.IsA<UsdPhysicsJoint>())
        {            
...
void JointAuthoringManager::createJointHelper(const PXR_NS::SdfPath& jointPath, JointDesc* desc, JointData& jointData)
{
    ...
    default: 
    {
        ...
        const auto stageID = PXR_NS::UsdUtilsStageCache::Get().GetId(mStage).ToLongInt();
        for (const auto& registration : registrations)
        {
            const auto& callbacks = registration.second.callbacks;
    ...
    if (desc && !mHidden)
    {
        ...
        const PXR_NS::GfMatrix4d localTransform = getWorldSpaceTransform(mStage, desc, true);
...
void JointAuthoringManager::updateJointGizmoTransformFromPath(const PXR_NS::SdfPath& jointPath)
{
...
{
    const uint64_t stageId = PXR_NS::UsdUtilsStageCache::Get().GetId(mStage).ToLongInt();
    for (PXR_NS::SdfPathSet::const_reference& ref : mJointGizmoTransform)
    {
...
void JointAuthoringManager::updateJointGizmoTransform(JointDesc* desc)
{
    ...
    PXR_NS::GfMatrix4f localTransform = GfMatrix4f(getWorldSpaceTransform(mStage, desc, true));
...
void JointAuthoringManager::updateJointTransformData(const PXR_NS::SdfPath& primPathIn)
{    
    PXR_NS::UsdPrimRange range(mStage->GetPrimAtPath(primPathIn));
    for (PXR_NS::UsdPrimRange::const_iterator iter = range.begin(); iter != range.end(); ++iter)
    {
        if (!(*iter))
            continue;
...
void scaleLimit(const PXR_NS::UsdAttribute& limitAttr, float scale)
{
    float limit;
    limitAttr.Get(&limit);
    if (isfinite(limit))
...
void scalePositiveLimit(const PXR_NS::UsdAttribute& limitAttr, float scale)
{
    float limit;
    limitAttr.Get(&limit);
    if (limit > 0.0f)
...
void JointAuthoringManager::updateJointScaleDataInternal()
{
    ...
    for (PXR_NS::SdfPathSet::const_reference& ref : mJointScaleData)
    {
...
{
    const uint64_t stageId = PXR_NS::UsdUtilsStageCache::Get().GetId(mStage).ToLongInt();
    for (PXR_NS::SdfPathSet::const_reference& ref : mJointTransformData)
    {
        const SdfPath& primPath = ref;
