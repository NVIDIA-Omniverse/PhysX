...
//!< A view over a volume deformable mesh (for example as read from USD)
struct PhysxCookingDeformableVolumeMeshView
{
    ...
    bool isEmpty() const
    {
        return simPoints.empty() || simIndices.empty() || collSurfaceIndices.empty();
    }
};

//!< A view over a volume/surface deformable body (for example as read from USD)
struct PhysxCookingDeformableBodyView
{
    omni::span<const carb::Float3> srcPointsInSim; // in sim space

    bool isEmpty() const
    {
        return srcPointsInSim.empty();
    }
};

struct PhysxDeformablePathInfo
{
    uint64_t bodyPrimId;
    uint64_t simMeshPrimId;
    uint64_t collMeshPrimId;
};
...
{
    CARB_PLUGIN_INTERFACE("omni::physx::IPhysxCookingService", 2, 1)
