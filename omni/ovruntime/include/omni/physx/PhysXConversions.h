...
inline ::physx::PxVec3 toPhysX(const PXR_NS::GfVec3d& usd)
{
    return ::physx::PxVec3(float(usd[0]), float(usd[1]), float(usd[2]));
}

inline ::physx::PxVec3 toPhysX(const PXR_NS::GfVec3f& usd)
{
...
inline ::physx::PxQuat toPhysX(const PXR_NS::GfQuatd& v)
{
    return ::physx::PxQuat(
...
template <typename SrcVecT>
void copyBuffer(PXR_NS::VtArray<PXR_NS::GfVec3f>& dst,
                const SrcVecT* src,
                unsigned int numElements,
                const PXR_NS::GfMatrix4f& transform)
{
...
inline void copyBuffer(PXR_NS::VtArray<PXR_NS::GfVec3f>& dst,
                       ::physx::PxVec4* src,
                       unsigned int numElements,
                       const PXR_NS::GfMatrix4f& transform)
{
...
inline void copyBuffer(PXR_NS::VtArray<carb::Float3>& dst, const PXR_NS::GfVec3f* src, unsigned int numElements)
{
    dst.resize(numElements);
    for (unsigned int i = 0; i < numElements; i++)
    {
...
inline void copyBuffer(float3* dst, const ::physx::PxVec4* src, unsigned int numElements, const PXR_NS::GfMatrix4f& transform)
{
    for (unsigned int i = 0; i < numElements; i++)
    {
        const ::physx::PxVec4& srcValue = src[i];
        PXR_NS::GfVec3f dstValue = transform.Transform(PXR_NS::GfVec3f(srcValue.x, srcValue.y, srcValue.z));
        dst[i] = { dstValue[0], dstValue[1], dstValue[2] };
    }
}

inline void copyBuffer(float3* dst, const ::physx::PxVec3* src, unsigned int numElements, const PXR_NS::GfMatrix4f& transform)
{
    for (unsigned int i = 0; i < numElements; i++)
    {
...
template <typename SrcVecT>
void scatterBuffer(PXR_NS::VtArray<PXR_NS::GfVec3f>& dst,
                   const SrcVecT* src,
                   const PXR_NS::VtArray<uint32_t>& indexMap,
                   const PXR_NS::GfMatrix4f& transform)
{
    for (size_t i = 0; i < indexMap.size(); i++)
    {
