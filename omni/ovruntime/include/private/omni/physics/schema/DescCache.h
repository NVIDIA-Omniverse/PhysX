...
using PathObjectMap = std::unordered_map<PXR_NS::SdfPath, const ShapeDesc*, PXR_NS::SdfPath::Hash>;
using PrimTypesMap = PXR_NS::TfHashMap<PXR_NS::TfType, uint64_t, PXR_NS::TfHash>;
...
public:
    ...
    void addDesc(const PXR_NS::SdfPath& path, const ShapeDesc* desc)
    {
        mPathObjectMap[path] = desc;
    }

    const ShapeDesc* getDesc(const PXR_NS::SdfPath& path) const
    {
    ...
    bool getPrimTypes(const PXR_NS::TfType& primType, uint64_t& types) const
    {
    ...
    void addPrimTypes(const PXR_NS::TfType& primType, uint64_t types)
    {
