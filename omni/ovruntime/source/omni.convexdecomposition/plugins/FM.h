...
void fm_eulerToQuat(const float euler[3], float quat[4]); // convert euler angles to quaternion. Angles must be radians
                                                          // not degrees!
...
void fm_quatToMatrix(const float quat[4], float matrix[16]); // convert quaterinion rotation to matrix, translation set
                                                             // to zero.
...
void fm_matrixToQuat(const float matrix[16], float quat[4]); // convert the 3x3 portion of a 4x4 matrix into a quaterion
                                                             // as x,y,z,w
...
bool fm_computeWindingOrder(const float p1[3], const float p2[3], const float p3[3]); // returns true if the triangle is
                                                                                      // clockwise.
...
bool fm_insideAABB(const float obmin[3], const float obmax[3], const float tbmin[3], const float tbmax[3]); // test if
                                                                                                            // bounding
                                                                                                            // box
                                                                                                            // tbmin/tmbax
                                                                                                            // is fully
...
uint32_t fm_clipTestAABB(
    const float bmin[3], const float bmax[3], const float p1[3], const float p2[3], const float p3[3], uint32_t& andCode);
...
float fm_computeBestFitAABB(
    uint32_t vcount, const float* points, uint32_t pstride, float bmin[3], float bmax[3]); // returns
                                                                                           // the
                                                                                           // diagonal
                                                                                           // distance
...
bool fm_intersectRayAABB(
    const float bmin[3], const float bmax[3], const float pos[3], const float dir[3], float intersect[3]);
...
bool fm_rayIntersectsTriangle(
    const float origin[3], const float dir[3], const float v0[3], const float v1[3], const float v2[3], float& t);
bool fm_rayIntersectsTriangle(
...
bool fm_raySphereIntersect(
    const float center[3], float radius, const float pos[3], const float dir[3], float distance, float intersect[3]);
bool fm_raySphereIntersect(
    const double center[3], double radius, const double pos[3], const double dir[3], double distance, double intersect[3]);

void fm_catmullRom(
    float out_vector[3], const float p1[3], const float p2[3], const float p3[3], const float* p4, const float s);
void fm_catmullRom(
...
void fm_combineAABB(
    const float bmin1[3], const float bmax1[3], const float bmin2[3], const float bmax2[3], float bmin[3], float bmax[3]);
...
float fm_volumeAABB(const float bmin[3], const float bmax[3]);
...
bool fm_colinear(
    const float a1[3], const float a2[3], const float b1[3], const float b2[3], float epsilon = 0.999f); // true if
                                                                                                         // these two
                                                                                                         // line
                                                                                                         // segments are
                                                                                                         // co-linear.
...
IntersectResult fm_intersectLineSegments2d(
    const float a1[3], const float a2[3], const float b1[3], const float b2[3], float intersectionPoint[3]);
IntersectResult fm_intersectLineSegments2d(
...
IntersectResult fm_intersectLineSegments2dTime(
    const float a1[3], const float a2[3], const float b1[3], const float b2[3], float& t1, float& t2);
IntersectResult fm_intersectLineSegments2dTime(
...
void fm_computeBestFitOBB(
    uint32_t vcount, const float* points, uint32_t pstride, float* sides, float matrix[16], bool bruteForce = true);
void fm_computeBestFitOBB(
...
public:
    ...
    virtual const float* triangulate3d(
        uint32_t pcount, const float* points, uint32_t vstride, uint32_t& tcount, bool consolidate, float epsilon) = 0;
};
...
bool fm_pointInsidePolygon2d(
    uint32_t pcount, const float* points, uint32_t pstride, const float* point, uint32_t xindex = 0, uint32_t yindex = 1);
...
uint32_t fm_consolidatePolygon(
    uint32_t pcount, const float* points, uint32_t pstride, float* dest, float epsilon = 0.999999f); // collapses
                                                                                                     // co-linear edges.
uint32_t fm_consolidatePolygon(
...
inline void fm_copy3(const float* source, double* dest)
{
    dest[0] = source[0];
    dest[1] = source[1];
    dest[2] = source[2];
...
inline void fm_copy3(const double* source, float* dest)
{
...
bool fm_isMeshCoplanar(uint32_t tcount, const uint32_t* indices, const float* vertices, bool doubleSided); // returns
                                                                                                           // true if
                                                                                                           // this
                                                                                                           // collection
                                                                                                           // of indexed
...
float fm_standardDeviation(uint32_t count, const float* points, float& mean);
