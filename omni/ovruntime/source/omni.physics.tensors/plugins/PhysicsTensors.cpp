...
ISimulationView* CARB_ABI CreateSimulationView(long stageId, const char* backendName)
{
    if (!gRegistryManager)
    {
        CARB_LOG_ERROR("Registry manager not initialized");
        return nullptr;
    }

    // Default to "physx" if not specified
    if (backendName == nullptr || strlen(backendName) == 0)
    {
        backendName = "physx";
    }
...
void CARB_ABI Reset(const char* backendName)
{
    if (!gRegistryManager)
    {
        CARB_LOG_ERROR("Registry manager not initialized");
