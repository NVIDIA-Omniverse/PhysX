...
def create_simulation_view(frontend_name, stage_id=-1, backend="physx"):
    """ Creates an entry point to the physics simulation through physics tensors.

        It allows creating a :py:class:`SimulationView` object given the frontend tensor framework and the physics backend.
        ...
        Args:
            ...
            backend (str): Physics backend to use: "physx" or "newton". Default is "physx".

        Returns:
            SimulationView: The simulation view object created with the specified frontend and backend.
        ...
        Example:
            .. code-block:: python
              ...
              >>> sim_view = tensors.create_simulation_view("torch", backend="newton")
    """
    tensor_api = acquire_tensor_api()
    if tensor_api is None:
        raise Exception("Failed to acquire tensor API")

    backend_view = tensor_api.create_simulation_view(stage_id, backend)
    if backend_view is None:
        raise Exception(f"Failed to create simulation view with backend '{backend}'")

    device_ordinal = backend_view.device_ordinal
    param_device_ordinal = backend_view.param_device_ordinal
    ...
    if frontend_id == "numpy" or frontend_id == "np":
        if device_ordinal == -1:
            ...
            return SimulationView(backend_view, frontend, param_device_ordinal)
        else:
...
def reset(backend="physx"):
    """ Resets the physics simulation with the specified backend.

        This will reset the underlying tensor buffers that are created based on the existing stage.

        Args:
            backend (str): Physics backend to reset: "physx" or "newton". Default is "physx".
        ...
        Example:
            .. code-block:: python
              ...
              >>> import omni.physics.tensors as tensors
              >>> sim_view = tensors.create_simulation_view("torch")
              >>> tensors.reset()  # Reset default backend
              >>> tensors.reset(backend="newton")  # Reset Newton backend
    """
    tensor_api = acquire_tensor_api()
    if tensor_api is None:
        raise Exception("Failed to acquire tensor API")
    tensor_api.reset(backend)
...
class SimulationView:
    ...
    """
    def __init__(self, backend, frontend, param_device_ordinal=-1):
        """ Constructs a SimulationView object with the given backend object and frontend framework.
            ...
            Args:
                ...
                param_device_ordinal (int): Device ordinal for parameter tensors. -1 for CPU (PhysX default),
                    or the simulation device ordinal for backends that keep parameter data on device (Newton).
        """
        self._backend = backend
        self._frontend = frontend
        self._param_device_ordinal = param_device_ordinal
    ...
    def param_device_ordinal(self):
        """ Device ordinal for parameter/property tensors (masses, stiffnesses, limits, etc.).
            Returns -1 (CPU) for PhysX. For Newton, returns the simulation device ordinal.
        """
    ...
    def create_articulation_view(self, pattern):
        """ Creates a view of articulations of the scene that match the given pattern, or a list of patterns.
            ...
            Patterns are split on ``/`` and each component is matched against a single prim name using glob
            semantics (``*``, ``?``, ``[...]``, ``|`` alternation). In addition, a ``**`` component matches
            zero or more path components, enabling recursive descent when the target prim is nested deeper
            than the rest of the pattern suggests. ``/`` is not split when it appears inside a balanced
            ``(...)`` group, so alternations that span multiple prim names are preserved as written.

            A bare ``*`` as the last component is always treated as "direct children only" — callers who
            want to collect descendants at any depth must use ``**``. When the last component is a *named*
            glob (e.g. ``base_link``, ``foo*``, ``(hip|thigh)``) it is matched against every descendant
            name rather than only direct children, which lets patterns locate a known link by name
            regardless of how deep the USD hierarchy nests it. Set
            ``/physics/tensors/recursiveLeafPatternMatch`` to ``false`` to restrict even named leaves to
            direct children; explicit ``**`` always recurses regardless of the setting.

            .. note::

                **Backwards compatibility:** prior releases treated a named leaf component
                (e.g. ``/World/envs/env_*/Robot/torso``) as a strict direct-children match. With
                recursive-leaf matching now the default, the same pattern will additionally pick up
                any prim named ``torso`` at *any* depth under each ``Robot``, which can enlarge
                views compared to earlier behaviour. Callers that relied on the old semantics should
                either set ``/physics/tensors/recursiveLeafPatternMatch`` to ``false``, or migrate to
                a list of explicit absolute paths.
            ...
            Example:
                .. code-block:: python
                  ...
                  >>> nested_view = sim_view.create_articulation_view("/World/envs/*/Robot/**/base_link") # This will find articulations whose root link is named "base_link" at any depth below each Robot prim
        """
        return ArticulationView(self._backend.create_articulation_view(pattern), self._frontend, self._param_device_ordinal)
    ...
    def create_rigid_body_view(self, pattern):
        """ Creates a view of rigid bodies of the scene that match the given pattern, or a list of patterns.
            ...
            Patterns are split on ``/`` and each component is matched against a single prim name using glob
            semantics (``*``, ``?``, ``[...]``, ``|`` alternation). In addition, a ``**`` component matches
            zero or more path components — use it to collect rigid bodies nested at varying depths (for
            example links produced by the URDF converter where body prims live inside other body prims).
            ``/`` is not split when it appears inside a balanced ``(...)`` group, so alternations that span
            multiple prim names are preserved as written.

            A bare ``*`` as the last component is always treated as "direct children only" — callers who
            want to collect rigid bodies at any depth below a prim must use ``**`` (e.g.
            ``/World/envs/*/Robot/**``). When the last component is a *named* glob (e.g. ``wheel_*``,
            ``(hip|thigh)``) it is matched against every descendant name rather than only direct children,
            which lets patterns locate a known body by name regardless of how deep the USD hierarchy nests
            it. Set ``/physics/tensors/recursiveLeafPatternMatch`` to ``false`` to restrict even named
            leaves to direct children; explicit ``**`` always recurses regardless of the setting.
            ...
            Example:
                .. code-block:: python
                  ...
                  >>> all_links_view = sim_view.create_rigid_body_view("/World/envs/*/Robot/**") # This will match every rigid body under each Robot subtree, regardless of nesting depth
        """
        return RigidBodyView(self._backend.create_rigid_body_view(pattern), self._frontend, self._param_device_ordinal)
    ...
    def create_deformable_material_view(self, pattern):
        ...
        """
        return DeformableMaterialView(self._backend.create_deformable_material_view(pattern), self._frontend, self._param_device_ordinal)
...
class ArticulationView:
    ...
    """
    def __init__(self, backend, frontend, param_device=-1):
        """ Constructs an ArticulationView object with the given backend object and frontend framework.
            ...
            Args:
                ...
                param_device (int): Device ordinal for parameter/property tensors. -1 for CPU (PhysX default),
                    or the simulation device ordinal for backends that keep parameter data on device (Newton).
        """
        self._backend = backend
        self._frontend = frontend
    ...
    def get_dof_types(self):
        ...
        if not hasattr(self, "_dof_types"):
            self._dof_types, self._dof_types_desc = self._frontend.create_tensor((self.count, self.max_dofs), uint8, self._param_device)
    ...
    def get_dof_motions(self):
        ...
        if not hasattr(self, "_dof_motions"):
            self._dof_motions, self._dof_motions_desc = self._frontend.create_tensor((self.count, self.max_dofs), uint8, self._param_device)
    ...
    def get_dof_limits(self):
        ...
        if not hasattr(self, "_dof_limits"):
            self._dof_limits, self._dof_limits_desc = self._frontend.create_tensor(
                (self.count, self.max_dofs, 2), float32, self._param_device
            )
    ...
    def get_drive_types(self):
        ...
        if not hasattr(self, "_drive_types"):
            self._drive_types, self._drive_types_desc = self._frontend.create_tensor((self.count, self.max_dofs), uint8, self._param_device)
    ...
    def get_dof_stiffnesses(self):
        ...
        if not hasattr(self, "_dof_stiffnesses"):
            self._dof_stiffnesses, self._dof_stiffnesses_desc = self._frontend.create_tensor((self.count, self.max_dofs), float32, self._param_device)
    ...
    def get_dof_dampings(self):
        ...
        if not hasattr(self, "_dof_dampings"):
            self._dof_dampings, self._dof_dampings_desc = self._frontend.create_tensor((self.count, self.max_dofs), float32, self._param_device)
    ...
    def get_dof_max_forces(self):
        ...
        if not hasattr(self, "_dof_max_forces"):
            self._dof_max_forces, self._dof_max_forces_desc = self._frontend.create_tensor((self.count, self.max_dofs), float32, self._param_device)
    ...
    def get_dof_drive_model_properties(self):
        ...
        if not hasattr(self, "_dof_drive_model_properties"):
            self._dof_drive_model_properties, self._dof_drive_model_properties_desc = self._frontend.create_tensor((self.count, self.max_dofs, 3), float32, self._param_device)
    ...
    def get_dof_friction_coefficients(self):
        ...
        if not hasattr(self, "_dof_friction_coefficients"):
            self._dof_friction_coefficients, self._dof_friction_coefficients_desc = self._frontend.create_tensor((self.count, self.max_dofs), float32, self._param_device)
    ...
    def get_dof_friction_properties(self):
        ...
        if not hasattr(self, "_friction_properties"):
            self._friction_properties, self._friction_properties_desc = self._frontend.create_tensor((self.count, self.max_dofs, 3), float32, self._param_device)
    ...
    def get_dof_max_velocities(self):
        ...
        if not hasattr(self, "_dof_max_velocities"):
            self._dof_max_velocities, self._dof_max_velocities_desc = self._frontend.create_tensor((self.count, self.max_dofs), float32, self._param_device)
    ...
    def get_dof_armatures(self):
        ...
        if not hasattr(self, "_dof_armatures"):
            self._dof_armatures, self._dof_armatures_desc = self._frontend.create_tensor((self.count, self.max_dofs), float32, self._param_device)
    ...
    def get_masses(self):
        ...
        if not hasattr(self, "_masses"):
            self._masses, self._masses_desc = self._frontend.create_tensor((self.count, self.max_links), float32, self._param_device)
    ...
    def get_inv_masses(self):
        ...
        if not hasattr(self, "_inv_masses"):
            self._inv_masses, self._inv_masses_desc = self._frontend.create_tensor((self.count, self.max_links), float32, self._param_device)
    ...
    def get_coms(self):
        ...
        if not hasattr(self, "_coms"):
            self._coms, self._com_desc = self._frontend.create_tensor((self.count, self.max_links, 7), float32, self._param_device)
    ...
    def get_inertias(self):
        ...
        if not hasattr(self, "_inertias"):
            self._inertias, self._inertias_desc = self._frontend.create_tensor((self.count, self.max_links, 9), float32, self._param_device)
    ...
    def get_inv_inertias(self):
        ...
        if not hasattr(self, "_inv_inertias"):
            self._inv_inertias, self._inv_inertias_desc = self._frontend.create_tensor((self.count, self.max_links, 9), float32, self._param_device)
    ...
    def get_disable_gravities(self):
        ...
        if not hasattr(self, "_disable_gravities"):
            self._disable_gravities, self._disable_gravities_desc = self._frontend.create_tensor((self.count, self.max_links), uint8, self._param_device)
    ...
    def get_material_properties(self):
        ...
        if not hasattr(self, "_material_properties"):
            self._material_properties, self._material_properties_desc = self._frontend.create_tensor((self.count, self.max_shapes, 3), float32, self._param_device)
    ...
    def get_compliant_material_properties(self):
        ...
        if not hasattr(self, "_compliant_material_properties"):
            self._compliant_material_properties, self._compliant_material_properties_desc = self._frontend.create_tensor((self.count, self.max_shapes, 2), float32, self._param_device)

        if not hasattr(self, "_compliant_material_combination"):
            self._compliant_material_combination, self._compliant_material_combination_desc = self._frontend.create_tensor((self.count, self.max_shapes, 2), uint8, self._param_device)
    ...
    def get_contact_offsets(self):
        ...
        if not hasattr(self, "_contact_offsets"):
            self._contact_offsets, self._contact_offsets_desc = self._frontend.create_tensor((self.count, self.max_shapes), float32, self._param_device)
    ...
    def get_rest_offsets(self):
        ...
        if not hasattr(self, "_rest_offsets"):
            self._rest_offsets, self._rest_offsets_desc = self._frontend.create_tensor((self.count, self.max_shapes), float32, self._param_device)
...
class RigidBodyView:
    ...
    def __init__(self, backend, frontend, param_device=-1):
        """ Constructs a RigidBodyView object with the given backend object and frontend framework.
            ...
            Args:
                ...
                frontend (FrontendBase): The tensor framework that is used to handle data.
                param_device (int): Device ordinal for parameter/property tensors. -1 for CPU (default).
        """
        self._backend = backend
        self._frontend = frontend
        self._param_device = param_device
    ...
    def get_masses(self):
        ...
        """
        if not hasattr(self, "_masses"):
            self._masses, self._masses_desc = self._frontend.create_tensor((self.count, 1), float32, self._param_device)
    ...
    def get_inv_masses(self):
        ...
        """
        if not hasattr(self, "_inv_masses"):
            self._inv_masses, self._inv_masses_desc = self._frontend.create_tensor((self.count, 1), float32, self._param_device)
    ...
    def get_coms(self):
        ...
        """
        if not hasattr(self, "_coms"):
            self._coms, self._com_desc = self._frontend.create_tensor((self.count, 7), float32, self._param_device)
    ...
    def get_inertias(self):
        ...
        """
        if not hasattr(self, "_inertias"):
            self._inertias, self._inertias_desc = self._frontend.create_tensor((self.count, 9), float32, self._param_device)
    ...
    def get_inv_inertias(self):
        ...
        """
        if not hasattr(self, "_inv_inertias"):
            self._inv_inertias, self._inv_inertias_desc = self._frontend.create_tensor((self.count, 9), float32, self._param_device)
    ...
    def get_disable_gravities(self):
        ...
        """
        if not hasattr(self, "_disable_gravities"):
            self._disable_gravities, self._disable_gravities_desc = self._frontend.create_tensor((self.count, 1), uint8, self._param_device)
    ...
    def get_disable_simulations(self):
        ...
        if not hasattr(self, "_disable_simulations"):
            self._disable_simulations, self._disable_simulations_desc = self._frontend.create_tensor((self.count, 1), uint8, self._param_device)
    ...
    def get_compliant_material_properties(self):
        ...
        if not hasattr(self, "_compliant_material_combine_modes"):
            self._compliant_material_combine_modes, self._compliant_material_combine_modes_desc = self._frontend.create_tensor((self.count, self.max_shapes, 2), uint8, self._param_device)
    ...
    def get_rest_offsets(self):
        ...
        if not hasattr(self, "rest_offsets"):
            self.rest_offsets, self._rest_offsets_desc = self._frontend.create_tensor((self.count, self.max_shapes), float32, self._param_device)
...
class DeformableMaterialView:
    ...
    def get_dynamic_friction(self):
        ...
        if not hasattr(self, "_dynamic_friction"):
            self._dynamic_friction, self._dynamic_friction_desc = self._frontend.create_tensor((self.count, 1), float32, self._param_device)
    ...
    def get_youngs_modulus(self):
        ...
        if not hasattr(self, "_youngs_modulus"):
            self._youngs_modulus, self._youngs_modulus_desc = self._frontend.create_tensor((self.count, 1), float32, self._param_device)
    ...
    def get_poissons_ratio(self):
        ...
        if not hasattr(self, "_poissons_ratio"):
            self._poissons_ratio, self._poissons_desc = self._frontend.create_tensor((self.count, 1), float32, self._param_device)
...
class SdfShapeView:
    ...
    def get_sdf_and_gradients(self, data):
        """ Gets the signed distance functions and their gradients for the shapes in the view.
            ...
            Sample points are in each shape's local space; the returned distances and
            gradients are in SDF local space.
            ...
            Args:
                data (Union[np.ndarray, torch.Tensor, wp.array]): Sample point locations for every shape, in each shape's local space. Shape (count, max_num_points, 3) where count is the number of shapes in the view and max_num_points is the maximum number of points per shape. The last dimension holds the x, y, z components of the points.

            Returns:
                Union[np.ndarray, torch.Tensor, wp.array]: Evaluated distances and gradients in SDF local space. Shape (count, max_num_points, 4): the first component of the last dimension is the signed distance, the next three are the x, y, z components of the gradient.
