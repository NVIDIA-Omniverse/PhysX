...
struct CookingComputeService : public ICookingComputeService
{
        ...
        CookingStageAndPrim& stageAndPrim)
    {
        ...
        if (collMeshPrim != simMeshPrim)
        {
            ...
            {
                TfType poseType = UsdSchemaRegistry::GetAPITypeFromSchemaTypeName(OmniPhysicsDeformableAPITokens->DeformablePoseAPI);
                TfToken simMeshBindPoseToken = getPoseNameFromPurpose(simMeshPrim, OmniPhysicsDeformableAttrTokens->bindPose);
                TfToken collisionMeshBindPoseToken = getPoseNameFromPurpose(collMeshPrim, OmniPhysicsDeformableAttrTokens->bindPose);
                                          ...
                                          CookingStageAndPrim& stageAndPrim)
{
    ...
    if (request.primId != 0)
    {
        ret1 = result.requestSource == PhysxCookingComputeResult::eREQUEST_SOURCE_USD ?
    ...
    else
    {
        if (request.dataType == PhysxCookingDataType::eVOLUME_DEFORMABLE_BODY)
        {
            // TODO: Implement Fabric support for volume deformable body view-filling (OMPE-74355)
            //ret2 = CookingComputeService::fillFabricVolumeDeformableBodyView(request, request.volumeDeformableBodyView, stageAndPrim);
            CARB_LOG_WARN("Fabric source not yet supported for volume deformable body");
            ret2 = false;

        }
        else if (request.dataType == PhysxCookingDataType::eDEFORMABLE_VOLUME_MESH)
        {
            // TODO: Implement Fabric support for deformable volume mesh view-filling (OMPE-74355)
            //ret2 = CookingComputeService::fillFabricDeformableVolumeMeshView(request, request.volumeMeshView, stageAndPrim);           
            CARB_LOG_WARN("Fabric source not yet supported for deformable volume mesh");
            ret2 = false;

        }
        else if (request.dataType == PhysxCookingDataType::eSURFACE_DEFORMABLE_BODY)
        {
            // TODO: Implement Fabric support for surface deformable body view-filling (OMPE-74355)
            //ret2 = CookingComputeService::fillFabricSurfaceDeformableBodyView(request, request.surfaceDeformableBodyView, stageAndPrim);         
            CARB_LOG_WARN("Fabric source not yet supported for surface deformable body");
            ret2 = false;
        }
    }
