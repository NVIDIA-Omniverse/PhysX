...
template <typename T, size_t N, typename DynamicRequestT>
inline bool getRequestValueFixedArray(T(&value)[N], DynamicRequestT& request, const char* key)
{
    using ArrayType = T[N];
    const ArrayType* valuePtr;

    if (!request.template getAs<ArrayType>(key, valuePtr) || !valuePtr) {
        return false;
    }
...
#define GET_REQUEST_PARAM_FIXED_ARRAY_RETURN_FALSE_ON_FAIL(_name) CHECK_RETURN_FALSE_ON_FAIL(getRequestValueFixedArray(params._name, request, #_name))
...
// VolumeDeformableBodyCookingParams load/unload

template<typename RequestBuilderT>
void addParamsToRequest(RequestBuilderT& builder, const VolumeDeformableBodyCookingParams& params)
{
    ADD_REQUEST_PARAM(simToCookingTransform);
    ADD_REQUEST_PARAM(simToCollTransform);

    ADD_REQUEST_PARAM(isAutoMeshSimplificationEnabled);
    ADD_REQUEST_PARAM(isAutoRemeshingEnabled);
    ADD_REQUEST_PARAM(hasAutoForceConforming);
    ADD_REQUEST_PARAM(isAutoHexahedralMeshEnabled);
    ADD_REQUEST_PARAM(autoRemeshingResolution);
    ADD_REQUEST_PARAM(autoTriangleTargetCount);
    ADD_REQUEST_PARAM(autoHexahedralResolution);
}

template<typename DynamicRequestT>
bool getParamsFromRequest(VolumeDeformableBodyCookingParams& params, DynamicRequestT& request)
{
    GET_REQUEST_PARAM_FIXED_ARRAY_RETURN_FALSE_ON_FAIL(simToCookingTransform);
    GET_REQUEST_PARAM_FIXED_ARRAY_RETURN_FALSE_ON_FAIL(simToCollTransform);

    GET_REQUEST_PARAM_RETURN_FALSE_ON_FAIL(isAutoMeshSimplificationEnabled);
    GET_REQUEST_PARAM_RETURN_FALSE_ON_FAIL(isAutoRemeshingEnabled);
    GET_REQUEST_PARAM_RETURN_FALSE_ON_FAIL(hasAutoForceConforming);
    GET_REQUEST_PARAM_RETURN_FALSE_ON_FAIL(isAutoHexahedralMeshEnabled);
    GET_REQUEST_PARAM_RETURN_FALSE_ON_FAIL(autoRemeshingResolution);
    GET_REQUEST_PARAM_RETURN_FALSE_ON_FAIL(autoTriangleTargetCount);
    GET_REQUEST_PARAM_RETURN_FALSE_ON_FAIL(autoHexahedralResolution);

    return true;
}

// DeformableVolumeMeshCookingParams load/unload

template<typename RequestBuilderT>
void addParamsToRequest(RequestBuilderT& builder, const DeformableVolumeMeshCookingParams& params)
{
    ADD_REQUEST_PARAM(simToCookingTransform);
    ADD_REQUEST_PARAM(numTetsPerElement);
}

template<typename DynamicRequestT>
bool getParamsFromRequest(DeformableVolumeMeshCookingParams& params, DynamicRequestT& request)
{
    GET_REQUEST_PARAM_FIXED_ARRAY_RETURN_FALSE_ON_FAIL(simToCookingTransform);
    GET_REQUEST_PARAM_RETURN_FALSE_ON_FAIL(numTetsPerElement);

    return true;
}

// SurfaceDeformableBodyCookingParams load/unload

template<typename RequestBuilderT>
void addParamsToRequest(RequestBuilderT& builder, const SurfaceDeformableBodyCookingParams& params)
{
    ADD_REQUEST_PARAM(simToCookingTransform);
...
template<typename DynamicRequestT>
bool getParamsFromRequest(SurfaceDeformableBodyCookingParams& params, DynamicRequestT& request)
{
    GET_REQUEST_PARAM_FIXED_ARRAY_RETURN_FALSE_ON_FAIL(simToCookingTransform);
...
REGISTER_REQUEST_TOKEN_TYPE(carb::Double4[4]);
