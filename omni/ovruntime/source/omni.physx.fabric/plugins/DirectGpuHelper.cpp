...
extern PXR_NS::GfMatrix4d computeMatrix(const PXR_NS::GfVec3d& translate,
                                     const PXR_NS::GfMatrix3d& rotate,
                                     const PXR_NS::GfVec3d& scale);

extern PXR_NS::GfMatrix4d computeLocalMatrix(omni::fabric::USDHierarchy& usdHierarchy,
...
bool DirectGpuHelper::prepareMappings(omni::fabric::StageReaderWriter& srw)
{
    ...
    if (mNumRds > 0 || mLinkBufSize > 0)
    {
        ...
        if (mNumRds > 0)
        {
            ...
            mFabricMapperRb.build<RigidBodyData, PXR_NS::GfMatrix4d>(mRigidBodies, mNumRds, getRigidBodyIndex, mWorldMatrixToken, primBuckets);
        ...
        if (mLinkBufSize > 0)
        {
            ...
            mFabricMapperArticulation.build<RigidBodyData, PXR_NS::GfMatrix4d>(mRigidBodies, mLinkBufSize, getArticulationIndex, mWorldMatrixToken, primBuckets);
...
void DirectGpuHelper::applyRigidBodiesToFabric_CPU(omni::fabric::StageReaderWriter& srw, bool updateTransforms, bool updateVelocities)
{
    ...
    {
        gsl::span<PXR_NS::GfMatrix4d> worldMatrices = srw.getAttributeArray<PXR_NS::GfMatrix4d>(primBuckets, i, mWorldMatrixToken);
                ...
                if (rb.isArticulationLink)
                {
                    if (updateTransforms && mLinkTransforms.size() > rb.link.linkOffset)
                    {
                        ...
                        worldMatrices[j] = computeMatrix(omni::physx::toVec3d(omni::physx::toDouble3(transform.p)),
                                                            PXR_NS::GfMatrix3d(omni::physx::toQuatd(omni::physx::toDouble4(transform.q))),
                    ...
                    if (rb.rd.idx < mRdTransforms.size())
                    {
                        if (updateTransforms)
                        {
                            ...
                            worldMatrices[j] = computeMatrix(omni::physx::toVec3d(omni::physx::toDouble3(rdTransform.p)),
                                                                PXR_NS::GfMatrix3d(omni::physx::toQuatd(omni::physx::toDouble4(rdTransform.q))),
                                                                omni::physx::toVec3d(scale));
                            if (j < rbWorldPositions.size())
                            {
...
void DirectGpuHelper::applyRigidBodiesToFabric_GPU(omni::fabric::StageReaderWriter& srw, bool updateTransforms, bool updateVelocities)
{
    ...
    size_t bucketCount = primBuckets.bucketCount();
    {
        ...
        for (size_t i = 0; i != bucketCount; i++)
        {
            gsl::span<const PXR_NS::GfMatrix4d> worldMatrices = srw.getAttributeArrayGpu<PXR_NS::GfMatrix4d>(primBuckets, i, mWorldMatrixToken, deviceId);
