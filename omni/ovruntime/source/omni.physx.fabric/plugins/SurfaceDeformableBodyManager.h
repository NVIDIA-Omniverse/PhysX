...
class SurfaceDeformableBodySet
{
public:
    SurfaceDeformableBodySet();
    ~SurfaceDeformableBodySet();

    void releaseBuffers();
    void prepareBuffers(omni::fabric::StageReaderWriter& srw);
    void updateInternalSurfaceDeformableBodyData();
    void updateSurfaceDeformableBodies(omni::fabric::StageReaderWriter& srw);
    ...
    std::vector<SurfaceDeformableBody*> mSurfaceDeformableBodies;
...
public:
    ...
    // Marks the manager dirty so that the next update() call rebuilds mSurfaceDeformableBodySets.
    // Should be invoked when a PhysX deformable surface is destroyed externally so that stale
