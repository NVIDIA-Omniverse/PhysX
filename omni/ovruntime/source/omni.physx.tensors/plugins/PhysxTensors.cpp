...
{
    // Create a CUDA primary context only when GPU mode is active.
    // In CPU mode, calling cudaMalloc() (via cudart_static) triggers
    // LoadLibrary("nvcuda.dll") which can crash on Windows machines that have
    // the driver DLL in System32 but no physical GPU.
    {
        auto* framework = carb::getFramework();
        auto* foundation = framework ? framework->tryAcquireInterface<omni::physx::IPhysxFoundation>() : nullptr;
        if (foundation && foundation->isCpuMode && !foundation->isCpuMode())
        {
            omni::physx::tensors::createPrimaryCudaContext();
        }
        else
        {
            CARB_LOG_INFO("omni.physx.tensors: skipping CUDA context creation (%s)",
                foundation ? "CPU mode active" : "IPhysxFoundation not loaded");
        }
    }
