...
// Collects a prim and all of its descendants in pre-order (self before
// descendants, children in declaration order). Iterative to avoid a
// stack-overflow footgun on deep user-authored USD hierarchies.
void collectSelfAndDescendants(UsdPrim prim, std::vector<UsdPrim>& out)
{
    ...
    while (!stack.empty())
    {
        ...
        // preserving the pre-order traversal the recursive version produced.
...
void collectSelfAndDescendantsUsdRt(const usdrt::UsdPrim& prim, std::vector<usdrt::UsdPrim>& out)
{
    if (!prim)
    {
        return;
    }
    std::vector<usdrt::UsdPrim> stack;
    stack.push_back(prim);
    while (!stack.empty())
    {
        usdrt::UsdPrim current = stack.back();
        stack.pop_back();
        out.push_back(current);
        auto children = current.GetAllChildren();
        std::vector<usdrt::UsdPrim> childVec(children.begin(), children.end());
        for (auto it = childVec.rbegin(); it != childVec.rend(); ++it)
        {
            stack.push_back(*it);
        }
...
//
// When enabled (default), a *named* last token in a pattern (e.g. `link_1`,
// `wheel_*`, `(hip|thigh)`) is matched against the name of every descendant
// of the current candidate roots rather than only their direct children.
// This lets callers locate a known link by name regardless of how deeply the
// USD converter nested it. A bare `*` leaf is deliberately NOT promoted here
// — silently turning `A/*` into "all descendants of A" changes the meaning
// of every pre-existing pattern that relied on strict per-level matching
// and would mask legitimate regressions. Callers who want to collect an
// entire subtree must spell it with `**`.
//
// Users who need strict semantics even for named leaves can disable it via
// kSettingRecursiveLeafPatternMatch; the explicit `**` token always works
// regardless.
//
// Intermediate tokens are unaffected — they still disambiguate the subtree
// you're targeting. Only the leaf is traversed.
//
...
// Collects every descendant of `prim` whose name matches the regex `matcher`.
// Does NOT include `prim` itself — the pivot is the penultimate match level,
// we only want deeper hits.
void collectMatchingDescendants(UsdPrim prim, TfPatternMatcher& matcher, std::vector<UsdPrim>& out)
{
    if (!prim)
    {
        return;
    ...
    for (auto child : prim.GetAllChildren())
    {
        ...
        collectMatchingDescendants(child, matcher, out);
    }
}

void collectMatchingDescendantsUsdRt(const usdrt::UsdPrim& prim,
                                     TfPatternMatcher& matcher,
                                     std::vector<usdrt::UsdPrim>& out)
{
    if (!prim)
    {
        return;
    ...
    for (auto child : prim.GetAllChildren())
    {
        ...
        collectMatchingDescendantsUsdRt(child, matcher, out);
    }
}
...
void BaseSimulationView::InitializeKinematicBodies()
{
    ...
    long stageID = mStage ? PXR_NS::UsdUtilsStageCache::Get().GetId(mStage).ToLongInt() : 0;

    if (stageID == 0)
    {
        CARB_LOG_ERROR("InitializeKinematicBodies: Failed to retrieve USD stage id.");
        return;
    }
    ...
    PXR_NS::UsdPrimRange primRange = mStage->Traverse();
    ...
    for (PXR_NS::UsdPrimRange::const_iterator iter = primRange.begin(); iter != primRange.end(); ++iter)
    {
        ...
        const PXR_NS::SdfPath path = prim.GetPrimPath();
        if (prim.HasAPI<UsdPhysicsRigidBodyAPI>())
        {
            ...
            if (rboAPI)
            {
                ...
                if (kinematic)
                {
                    ...
                    PXR_NS::GfMatrix4d& wMatData =
                        *(PXR_NS::GfMatrix4d*)(iSip->getAttributeWr(stageRW.getId(), primPath, mWorldMatrixToken)).ptr;
                    PXR_NS::GfMatrix4d& lMatData =
                        *(PXR_NS::GfMatrix4d*)(iSip->getAttributeWr(stageRW.getId(), primPath, mLocalMatrixToken)).ptr;
                    ...
                    PXR_NS::GfMatrix4d localPose(1.0);
                    omni::fabric::Path parentPath = usdHierarchy.getParent(primPath);
                    while (parentPath != omni::fabric::Path())
                    {
                        PXR_NS::GfMatrix4d* parentWorldMatrix = (PXR_NS::GfMatrix4d*)(iSip->getAttributeWr(stageRW.getId(), parentPath, mWorldMatrixToken)).ptr;
...
void BaseSimulationView::findMatchingPaths(const std::string& pattern_, std::vector<SdfPath>& pathsRet)
{
    ...
    if (useUsdrt)
    {
        ...
        // Split on '/' while preserving '(...)' groups; supports '**' as a
        // recursive-descent marker spanning zero or more USD path components.
        ...
        const bool recursiveLeaf = isRecursiveLeafPatternMatchEnabled();
        ...
        for (int i = 0; i < numTokens; i++)
        {
            ...
            // Bare '*' is an explicit "direct children of this level" — do
            // not promote it to a descendant walk even when recursiveLeaf
            // is on. Callers who want nested matching must use '**'.
            const bool isBareWildcard = (tokens[i] == "*");

            if (isRecursiveDescent)
            {
                // '**' matches zero or more path components: expand the
                // current candidate set to each root and every descendant.
                for (auto& prim : roots)
                {
                    collectSelfAndDescendantsUsdRt(prim, matches);
                }
            }
            else if (isLeaf && recursiveLeaf && !isBareWildcard)
            {
                // Leaf-recursive legacy-compat mode: match the leaf token
                // against every descendant name, not just direct children.
                // wrap in '^' and '$' so the glob anchors to the whole name.
                const std::string tokPattern = '^' + tokens[i] + '$';
                TfPatternMatcher matcher(tokPattern, true, true);
                for (auto& prim : roots)
                {
                    collectMatchingDescendantsUsdRt(prim, matcher, matches);
                }
            }
            else
            {
                // wrap the token in '^' and '$' so the glob anchors to the
                // whole prim name rather than matching a substring.
                const std::string tokPattern = '^' + tokens[i] + '$';
                for (auto& prim : roots)
                {
                    findMatchingChildrenUsdRt(prim, tokPattern, matches);
                }
            }
        ...
        for (auto& prim : matches)
        {
            ...
            const PXR_NS::SdfPath& sdfPath = omni::fabric::toSdfPath(fabricPath);
    ...
    else
    {
        ...
        for (int i = 0; i < numTokens; i++)
        {
            ...
            else if (isLeaf && recursiveLeaf && !isBareWildcard)
            {
                ...
                for (auto& prim : roots)
                {
                    collectMatchingDescendants(prim, matcher, matches);
                }
            }
            else
            {
                                                         ...
                                                         std::unordered_set<PXR_NS::SdfPath, PXR_NS::SdfPath::Hash>& seenSensorPaths)
{
    ...
    // Validate all filter patterns up-front and treat any malformed
    // filter pattern as fatal for this sensor pattern. Previously this
    // code logged and continued with an empty filter column, which
    // quietly produced sensor entries whose filter mapping was blank —
    // a pairing bug that's now even easier to hit with the shared
    // dedup set above. Bail before we touch any shared state.
    ...
    for (uint32_t i = 0; i < numFilterPatterns; i++)
    {
        ...
        {
            // Size mismatch is fatal too — the previous behaviour of
            // clearing filterPaths[i] and resizing to empty SdfPaths left
            // the resulting sensors with a silently blank filter column,
            // which is the same partial-disable footgun as a malformed
            // pattern. Fail the call so the user notices.
