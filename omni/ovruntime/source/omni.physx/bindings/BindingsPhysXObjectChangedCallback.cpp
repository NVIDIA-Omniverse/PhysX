    ...
    dev.def(
        ...
        [](const IPhysx* self, PyObjectCreationNotificationFn object_creation_fn,
           ...
           bool stopCallbackWhenSimStopped) -> SubscriptionId {
            ...
            cb.objectCreationNotifyFn = [wrappedCreateFn=std::move(wrappedCreateFn)](const PXR_NS::SdfPath& sdfPath, usdparser::ObjectId objectId, PhysXType type,
                                            void*) {
                if (wrappedCreateFn)
                    wrappedCreateFn(asInt(sdfPath), static_cast<int>(objectId), static_cast<int>(type));
            };
            cb.objectDestructionNotifyFn = [wrappedDestructionFn=std::move(wrappedDestructionFn)](const PXR_NS::SdfPath& sdfPath, usdparser::ObjectId objectId,
                                               PhysXType type, void*) {
