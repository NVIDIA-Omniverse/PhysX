...
namespace physx
{
    ...
    PXR_NS::SdfPath getSdfPath(const std::vector<InternalDatabase::Record>& records, const void* userData, uint32_t& protoIndex)
    {
        SdfPath path;
    ...
    PXR_NS::SdfPath getSdfPath(const std::vector<InternalDatabase::Record>& records, const void* userData)
    {
        SdfPath path;
        const size_t index = (size_t)userData;
        CARB_ASSERT(index < records.size());
    ...
    PXR_NS::SdfPath createEventFromSdfPath(
        ...
        const std::vector<PxContactPairFrictionAnchor>& frictionAnchors)
    {
        ...
        const int64_t stageId = omniPhysX.getStageId();
