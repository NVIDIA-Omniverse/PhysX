...
void OmniPhysX::physXAttach(long int stageId, bool loadPhysics)
{
    ...
    if(mISettings->getStringBuffer(kSettingForceParseOnlySingleScene) != nullptr)
        getPhysXUsdPhysicsInterface().setForceParseOnlySingleScene(PXR_NS::SdfPath(mISettings->getStringBuffer(kSettingForceParseOnlySingleScene)));
    else
...
void OmniPhysX::onStartup()
{
    ...
    // correctly returns false when the foundation's device probe forced CPU mode.
    {
        ...
        const bool alreadyCpuMode = physxFoundation && physxFoundation->isCpuMode && physxFoundation->isCpuMode();
        if (!alreadyCpuMode)
        {
...
void OmniPhysX::subscribeToSettingsChangeEvents()
{
    ...
    subID = mISettings->subscribeToNodeChangeEvents(
        "/physics/suppressFabricUpdate",
        [](const carb::dictionary::Item* changedItem, carb::dictionary::ChangeEventType eventType, void* userData) {
            OmniPhysX& omniPhysX = OmniPhysX::getInstance();
            carb::dictionary::IDictionary* dict = carb::getCachedInterface<carb::dictionary::IDictionary>();
            omniPhysX.mCachedSettings.suppressFabricUpdate = dict->getAsBool(changedItem);
        },
        nullptr);
    mCachedSettings.suppressFabricUpdate = mISettings->getAsBool("/physics/suppressFabricUpdate");
    mSubscribedSettings.push_back(subID);
