...
struct OmniCachedSettings
{
    ...
    /// When true, skip the per-frame USD-to-Fabric-to-PhysX sync that picks up
    /// USD prim edits (transforms, material properties, etc.) and feeds them into
    /// the running simulation.  In DirectGPU / TensorAPI mode nothing modifies USD
    /// during simulation, so this sync is pure overhead.
    bool suppressFabricUpdate = false;
};
...
private:
    ...
    // Usd stage
    PXR_NS::UsdStageRefPtr mStage{ nullptr }; // Current stage
    long mStageId{ 0u };
