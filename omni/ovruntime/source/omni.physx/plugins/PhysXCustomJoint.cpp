...
CustomPhysXJoint* PhysXCustomJointManager::createCustomJoint(const PXR_NS::SdfPath& primPath, const usdparser::CustomPhysxJointDesc& jointDesc,::physx::PxRigidActor* actor0, const ::physx::PxTransform& localFrame0,
...
void PhysXCustomJointManager::removeCustomJoint(const PXR_NS::SdfPath& primPath)
{
...
void PhysXCustomJointManager::markJointDirty(const PXR_NS::SdfPath& primPath)
{
    CustomJointMap::iterator fit = mCustomJointMap.find(primPath);
    if (fit != mCustomJointMap.end())
    {
...
void PhysXCustomJointManager::setJointFlags(const PXR_NS::SdfPath& primPath, CustomJointFlag::Enum inFlags)
{
    CustomJointMap::iterator fit = mCustomJointMap.find(primPath);
    if (fit != mCustomJointMap.end())
    {        
...
CustomJointFlag::Enum PhysXCustomJointManager::getJointFlags(const PXR_NS::SdfPath& primPath)
{
    CustomJointMap::iterator fit = mCustomJointMap.find(primPath);
    if (fit != mCustomJointMap.end())
    {
...
void markJointDirty(const PXR_NS::SdfPath& primPath)
{
    OmniPhysX::getInstance().getCustomJointManager().markJointDirty(primPath);
}

void setJointFlags(const PXR_NS::SdfPath& primPath, CustomJointFlag::Enum flags)
{
    OmniPhysX::getInstance().getCustomJointManager().setJointFlags(primPath, flags);
}

CustomJointFlag::Enum getJointFlags(const PXR_NS::SdfPath& primPath)
{
