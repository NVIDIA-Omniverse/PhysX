...
uint32_t omni::physx::getDebugDrawCollShapeColor(const PXR_NS::SdfPath& primPath)
{
...
void drawTriMesh(debugrender::RenderOutput& out, const PxTriangleMesh* triMesh, const PxVec3& scale)
{
    ...
    const float maxSize = 0.05f / (float)PXR_NS::UsdGeomGetStageMetersPerUnit(stage);
...
const DebugLine* omni::physx::getShapeDebugDraw(const PXR_NS::SdfPath& primPath,
                                                ...
                                                uint32_t& numLines)
{
    ...
    case ePlaneShape:
    {
        ...
        const float normalLength = 0.5f / (float)PXR_NS::UsdGeomGetStageMetersPerUnit(stage);
