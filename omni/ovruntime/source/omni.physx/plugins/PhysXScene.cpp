    ...
    OmniFilterCallback* filterCallback, OmniContactReportCallback* reportCallback)
{
    ...
    if (!sceneDesc.cudaContextManager)
    {
        if (sceneDesc.flags.isSet(PxSceneFlag::eENABLE_GPU_DYNAMICS) || sceneDesc.flags.isSet(PxSceneFlag::eENABLE_DIRECT_GPU_API))
        {
            ...
            auto* physxFoundation = carb::getCachedInterface<omni::physx::IPhysxFoundation>();
            const bool explicitCpuMode = physxFoundation && physxFoundation->isCpuMode && physxFoundation->isCpuMode();
            if (!explicitCpuMode)
            {
...
PhysXScene::PhysXScene(const usdparser::AttachedStage& attachedStage)
    : mAttachedStage(attachedStage), mScene(nullptr), mControllerManager(nullptr), mInternalScene(nullptr), mTimeStepsPerSeconds(60),
      mSceneUpdateType(Synchronous), mMaterial(nullptr),
