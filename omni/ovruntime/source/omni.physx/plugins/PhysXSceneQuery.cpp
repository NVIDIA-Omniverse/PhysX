...
static PxBoxGeometry usdToPxBoxGeometry(const UsdPrim& prim, const PXR_NS::GfVec3d& scale)
{
...
static PxSphereGeometry usdToPxSphereGeometry(const UsdPrim& prim, const PXR_NS::GfVec3d& scale)
{
...
static PxCapsuleGeometry usdToPxCapsuleGeometry(const UsdPrim& prim, const PXR_NS::GfVec3d& scale)
{
...
static PxConvexMeshGeometry usdCylinderOrConeToPxConvexMeshGeometry(const UsdPrim& prim, const PXR_NS::GfVec3d& scale)
{
...
static PxConvexMeshGeometry usdToPxConvexMeshGeometry(UsdPrim& prim, const PXR_NS::GfVec3d& scale)
{
...
static PXR_NS::GfTransform computeWorldTransform(const UsdPrim& prim)
{
    ...
    return PXR_NS::GfTransform(UsdGeomImageable(prim).ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default()));
}
...
static bool doShapeSceneQuery(uint64_t rPrimPath, geometrySceneQueryFn queryFn)
{
    ...
    if (!prim.IsA<UsdGeomGprim>() && !prim.HasAPI<PhysxSchemaPhysxMeshMergeCollisionAPI>())
    {
        CARB_LOG_ERROR("omni::physx::doShapeSceneQuery: Provided prim is not a UsdGeomGPrim: %s", prim.GetPath().GetText());
        return false;
    }

    const PXR_NS::GfTransform tr = computeWorldTransform(prim);
    ...
    else if (prim.HasAPI<PhysxSchemaPhysxMeshMergeCollisionAPI>())
    {
        // for meshmerge the convex must already exist, lets find it
        PxShape* shapePrim = (PxShape*)(getPhysXPtr(primPath, ePTShape));
...
static bool doMeshSceneQuery(uint64_t meshPath, geometrySceneQueryFn queryFn)
{
    ...
    {
        if (!prim.HasAPI<PhysxSchemaPhysxMeshMergeCollisionAPI>())
        {
            return false;
        }
        else
    ...
    else
    {
        ...
        convexMesh = cookingDataAsync->getConvexMesh(convexMeshDesc, prim, false);
    }
...
uint32_t reportCollisionShapes(uint64_t path, ICollisionShapeQueryCallback& reportCallback)
{
    ...
    PXR_NS::UsdPrimRange primRange = PXR_NS::UsdPrimRange::AllPrims(topPrim);
