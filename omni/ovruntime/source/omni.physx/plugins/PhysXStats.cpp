...
std::vector<PhysXStats::PhysicsSceneStats> PhysXStats::getStatsPerPhysicsScene()
{
    ...
    auto usdrtStage = usdrt::UsdStage::Attach(omni::fabric::UsdStageId(mStageId));
    if (!usdrtStage)
    {
        return std::vector<PhysXStats::PhysicsSceneStats>();
    }

    const usdrt::SdfPathVector vecUsdrtPhysicsScenes = usdrtStage->GetPrimsWithTypeName(tokenPhysicsScene);

    // skip inactive scenes and instance proxies and convert paths from usdrt::SdfPath to PXR_NS::SdfPath
    PXR_NS::SdfPathVector vecPhysicsScenes;
