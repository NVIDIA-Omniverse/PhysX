              ...
              T& retVal, bool useBatching = true)
{
    ...
    if (changeSource != usdparser::ChangeSource::eUsd)
    {
        ...
        if (iStageReaderWriter && timeCode == PXR_NS::UsdTimeCode::Default())
        {
    ...
    // fallback to USD
    PXR_NS::UsdPrim prim = attachedStage.getStage()->GetPrimAtPath(path);
    if (!prim)
        return false;

    const PXR_NS::UsdProperty prop = prim.GetProperty(attributeName);

    if (!prop.IsDefined())
        return false;

    CARB_ASSERT(prop.Is<PXR_NS::UsdAttribute>());

    const PXR_NS::UsdAttribute& attr = (const PXR_NS::UsdAttribute&)(prop);
                                 ...
                                 PXR_NS::SdfPathVector& retVal)
{
    ...
    const PXR_NS::UsdProperty prop = prim.GetProperty(relName);

    if (!prop.IsDefined())
        return false;

    CARB_ASSERT(prop.Is<PXR_NS::UsdRelationship>());

    const PXR_NS::UsdRelationship& rel = (const PXR_NS::UsdRelationship&)(prop);
...
inline bool getJointAndLocalPose(const omni::physx::usdparser::AttachedStage& attachedStage,
                                 const PXR_NS::SdfPath& jointPath,
