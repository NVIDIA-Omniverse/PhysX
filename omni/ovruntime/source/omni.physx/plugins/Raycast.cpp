                                                         ...
                                                         ::physx::PxHitFlags& flag)
{
    ...
    if (objectRecord &&
        ...
        PXR_NS::UsdGeomImageable(mStage->GetPrimAtPath(objectRecord->mPath)).ComputeVisibility() == PXR_NS::UsdGeomTokens->invisible)
    {
                          ...
                          PhysXScene*& outPhysXScene)
{
    ...
    for (PhysXScenesMap::const_reference ref : physXScenes)
    {
        ...
        for (size_t i = 0; i < intScene->mVolumeDeformableBodies.size(); ++i)
        {
            ...
            if ((PXR_NS::UsdGeomImageable(body->mBodyPrim).ComputeVisibility() == PXR_NS::UsdGeomTokens->invisible))
                continue;
