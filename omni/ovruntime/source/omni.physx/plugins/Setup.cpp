...
namespace omni
{
    namespace physx
    {
        ...
        void PhysXSetup::setupGPU()
        {
            ...
            if (auto* optCuda = carb::getCachedInterface<IOptionalCuda>())
                optCuda->deviceGetCount(&availableDeviceCount, nullptr);
        ...
        void PhysXSetup::createPhysics(const PxTolerancesScale& tolerances)
        {
            ...
            if (!cpuMode && omniPhysX.isCudaLibPresent())
            {
                ...
                if (!mPhysxFoundation->cudaDeviceCheck())
                {
                    ...
                    // Force CPU mode and ensure getCudaContextManager() returns nullptr.
                    mPhysxFoundation->setCpuMode(true);
                    clearCudaContextManagersAndRefillWithNull(1);
                }
                else
                {
        ...
        ::physx::PxTolerancesScale   PhysXSetup::getDefaultTolerances(PXR_NS::UsdStageRefPtr stage)
        {
