...
void InternalActor::initializeDynamicActor(bool runtimeInitialization)
{
    ...
    {        
        ...
        if (initFit == db.mInitialActorDataMap.end())
        {
            ...
            initialData.xformOpStorage.store(PXR_NS::UsdGeomXformable(mPrim));
            ...
            PXR_NS::UsdAttribute velAttr = mPrim.GetAttribute(UsdPhysicsTokens->physicsVelocity);
            if (velAttr)
            {
            ...
            PXR_NS::UsdAttribute angVelAttr = mPrim.GetAttribute(UsdPhysicsTokens->physicsAngularVelocity);
            if (angVelAttr)
            {
        ...
        if (useXformCommonAPI)
        {
            ...
            PXR_NS::UsdGeomXformCommonAPI xformAPI(mPrim);
            if (xformAPI)
            {
    ...
    {
        mVelocityPath = mPrim.GetPrimPath().AppendProperty(PXR_NS::UsdPhysicsTokens.Get()->physicsVelocity);
        mAngularVelocityPath = mPrim.GetPrimPath().AppendProperty(PXR_NS::UsdPhysicsTokens.Get()->physicsAngularVelocity);
    ...
    if (updateUSD && mPrim && !(mFlags & InternalActorFlag::eUSE_XFORM_COMMON_API))
    {
        ...
        {
            PXR_NS::UsdEditContext editContext(mPrim.GetStage(), UsdEditTarget(OmniPhysX::getInstance().getSimulationLayer()));
        ...
        if (!(mFlags & InternalActorFlag::eSKIP_UPDATE_TRANSFORM))
        {
            ...
            {
                static TfToken gTranslate("xformOp:translate");
                PXR_NS::SdfPath attributePath = mPrim.GetPrimPath().AppendProperty(gTranslate);
...
void InternalActor::enableSplineSurfaceVelocity(bool enable,
                                                ...
                                                const PXR_NS::UsdGeomBasisCurves& splinesCurvePrim)
{
