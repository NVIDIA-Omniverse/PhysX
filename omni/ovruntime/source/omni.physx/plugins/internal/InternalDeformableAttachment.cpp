...
PxActor* getPhysxActorFromPath(PXR_NS::SdfPath path, PhysXType& type)
{
    ...
    {
        const OmniPhysX& omniPhysX = OmniPhysX::getInstance();
        PXR_NS::UsdGeomXformable xformable = PXR_NS::UsdGeomXformable::Get(omniPhysX.getStage(), path);
...
{
    const OmniPhysX& omniPhysX = OmniPhysX::getInstance();
    PXR_NS::UsdGeomImageable imageable = PXR_NS::UsdGeomImageable::Get(omniPhysX.getStage(), mData[1].path);
    PXR_NS::GfMatrix4d xformToWorld;
    xformToWorld.SetIdentity();
    if (imageable)
    {
        xformToWorld = imageable.ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
    }
    ...
    PXR_NS::UsdStageWeakPtr stage = OmniPhysX::getInstance().getStage();
    PXR_NS::UsdPrim usdPrim = stage->GetPrimAtPath(mData[1].path);
    ...
    if (mData[1].actor)
    {
        ...
        if (matchingRigidBody(usdPrim.GetPath(), mData[0].actor, rigidActor))
        {
            ...
            PXR_NS::UsdGeomImageable imageable = PXR_NS::UsdGeomImageable::Get(omniPhysX.getStage(), usdPrim.GetPath());
            PXR_NS::GfMatrix4d rigidToWorld;
            rigidToWorld.SetIdentity();
            if (imageable)
            {
                rigidToWorld = imageable.ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
            }
...
InternalDeformableAttachment::InternalDeformableAttachment(PXR_NS::SdfPath path, const PhysxDeformableAttachmentDesc& desc)
{
    ...
    if (mData[1].actor == nullptr)
    {
        const std::string errorStr = "Physics Deformable Attachment " + mPath.GetString() +
            ": no rigid actor at or above " + mData[1].path.GetString() + ".";
    ...
    // [OMPE-92466] Resolve mInternalScene without relying on
    // PxActor::getScene() for the rigid side. For an articulation link,
    // getScene() returns null until finalizeArticulations runs - which
    // happens AFTER createDeformableAttachments during stage load - so any
    // attachment whose rigid side is an articulation link would otherwise
    // be silently dropped (isValid() short-circuits when mInternalScene is
    // null). Fall back to PxArticulationLink::getArticulation().getScene(),
    // and finally to the omni.physx-side InternalLink/InternalActor whose
    // mPhysXScene is wired up at construction time and is valid throughout
    // load.
    ...
    mInternalScene = resolveInternalScene(mData[0].actor, mData[0].path);
    if (!mInternalScene)
        mInternalScene = resolveInternalScene(mData[1].actor, mData[1].path);
}
    ...
    else if (mDirtyEvent & eUpdateXform)
    {
        if (mDeformableAttachment)
        {
            if (mData[1].actor)
            {
                ...
                PXR_NS::UsdGeomImageable imageableXform = PXR_NS::UsdGeomImageable::Get(omniPhysX.getStage(), mData[1].path);
                PXR_NS::GfMatrix4d xformToWorld;
                xformToWorld.SetIdentity();
                if (imageableXform)
                {
                    xformToWorld = imageableXform.ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
                }

                PXR_NS::UsdGeomImageable imageableRigid = PXR_NS::UsdGeomImageable::Get(omniPhysX.getStage(), mData[1].rootPath);
                PXR_NS::GfMatrix4d rigidToWorld;
                rigidToWorld.SetIdentity();
                if (imageableRigid)
                {
                    rigidToWorld = imageableRigid.ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
                }
            ...
            else
            {
                ...
                if (imageable)
                {
                    localToWorld = imageable.ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
                }
