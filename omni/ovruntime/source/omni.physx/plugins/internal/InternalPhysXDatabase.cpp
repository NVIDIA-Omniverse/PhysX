...
InternalPhysXDatabase::InternalPhysXDatabase() : mInitialTransformsStored(false), mDebugDrawFlags(0ull), mNestedBodiesUsed(false)
{
    ...
    addRecord(ePTRemoved, nullptr, nullptr, PXR_NS::SdfPath());
...
SplinesCurve* InternalPhysXDatabase::addSplinesCurve(const PXR_NS::UsdGeomBasisCurves& curve, bool& added)
{
...
void InternalPhysXDatabase::storePxJoint(const ::physx::PxRigidActor* actor, const PXR_NS::SdfPath& path)
{
    ...
    for (size_t i = 0; i < constaints.size(); i++)
    {
        ...
        if (joint && (typeId == PxConstraintExtIDs::eJOINT))
        {
            ...
            if (actor0 == actor)
                mPxJointMap.insert(std::pair<PXR_NS::SdfPath, std::pair<PxJoint*, bool>>(path, std::make_pair(joint, true)));
            if (actor1 == actor)
                mPxJointMap.insert(std::pair<PXR_NS::SdfPath, std::pair<PxJoint*, bool>>(path, std::make_pair(joint, false)));
        }
    }
}
...

    {
        ...
        for (size_t idx = 0; idx < getRecords().size(); idx++)
        {
            ...
            if (record.mType == ePTLinkJoint)
            {
                ...
                PXR_NS::UsdPrim jointPrim = stage->GetPrimAtPath(record.mPath);
