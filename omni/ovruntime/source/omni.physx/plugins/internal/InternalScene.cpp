...
bool writeTransformUsingXformCommonAPI(const InternalActor& actor, const Transform& transform)
{
    ...
    PXR_NS::UsdGeomXformCommonAPI xformAPI(actor.mPrim);
    if (!xformAPI)
    {
        CARB_LOG_WARN_ONCE(
    ...
    const PXR_NS::GfVec3d translation(transform.position[0], transform.position[1], transform.position[2]);
    const PXR_NS::GfVec3f scale(transform.scale[0], transform.scale[1], transform.scale[2]);

    // Convert quaternion to rotation matrix
    const PXR_NS::GfQuatd inputQuat(transform.orientation);
    PXR_NS::GfMatrix3d rotMatrix = PXR_NS::GfMatrix3d(inputQuat);

    // Get current rotation order from prim
    PXR_NS::UsdGeomXformCommonAPI::RotationOrder rotOrder;
    PXR_NS::GfVec3f currentRotation, currentScale, currentPivot;
    PXR_NS::GfVec3d currentTranslation;
    xformAPI.GetXformVectors(
        &currentTranslation, &currentRotation, &currentScale, &currentPivot, &rotOrder, PXR_NS::UsdTimeCode::Default());
    ...
    const bool success = xformAPI.SetXformVectors(
        translation, eulerAngles, scale, currentPivot, rotOrder, PXR_NS::UsdTimeCode::Default());
