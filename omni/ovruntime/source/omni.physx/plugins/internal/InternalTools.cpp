...
static inline void setVecWithPrecision(UsdGeomXformOp& op, const GfVec3d& vec)
{
    ...
    if (precision == PXR_NS::UsdGeomXformOp::PrecisionFloat)
        op.Set(PXR_NS::GfVec3f(vec));
    else if (precision == PXR_NS::UsdGeomXformOp::PrecisionDouble)
        op.Set(vec);
    else if (precision == PXR_NS::UsdGeomXformOp::PrecisionHalf)
        op.Set(PXR_NS::GfVec3h(vec));
}
...
static inline void setQuatWithPrecision(UsdGeomXformOp& op, const GfQuatd& orient)
{
    ...
    const UsdGeomXformOp::Precision precision = op.GetPrecision();
    if (precision == PXR_NS::UsdGeomXformOp::PrecisionFloat)
        op.Set(PXR_NS::GfQuatf(orient));
    else if (precision == PXR_NS::UsdGeomXformOp::PrecisionDouble)
        op.Set(orient);
    else if (precision == PXR_NS::UsdGeomXformOp::PrecisionHalf)
        op.Set(PXR_NS::GfQuath(orient));
}
...
bool setupTransformOpsAsScaleOrientTranslate(const PXR_NS::UsdPrim& prim, PXR_NS::GfMatrix4d* preMatrix, bool* preMatrixValid, PXR_NS::GfMatrix4d* postMatrix, bool* postMatrixValid)
{
    ...
    if (!isStackDefault)
    {
        ...
        PXR_NS::SdfPrimSpecHandle primSpec =
            PXR_NS::SdfCreatePrimInLayer(prim.GetStage()->GetEditTarget().GetLayer(), primPath);

        if (!primSpec)
            return false;

        static TfToken gTranslate("xformOp:translate");
        PXR_NS::SdfPath attributePath = primPath.AppendProperty(gTranslate);
