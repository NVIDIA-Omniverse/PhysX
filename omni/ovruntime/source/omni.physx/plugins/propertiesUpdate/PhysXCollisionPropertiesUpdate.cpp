    ...
    const PXR_NS::TfToken& property, const PXR_NS::UsdTimeCode& timeCode)
{
    ...
    // PhysX wins: skip when physxCollision:restOffset is authored.
    ...
    const bool physxContactAuthored = physxContactAttr && physxContactAttr.HasAuthoredValue();
    float gap = 0.0f;
    const bool hasGapFallback = !physxContactAuthored
        && newton::getFloatFallback(gap, prim, NewtonSchemaTokens->newtonContactGap)
    ...
    float newRest;
    if (!getValue<float>(attachedStage, objectRecord->mPath, property, timeCode, newRest))
        return true;
    ...
    auto applyShape = [&](PxShape* shape) {
        ...
        if (!contactGrowsFirst)
        {
            if (newContact > shape->getRestOffset())
                ...
                CARB_LOG_ERROR("Collision contact offset must be greater than restOffset, prim: %s", objectRecord->mPath.GetText());
        }
    };
