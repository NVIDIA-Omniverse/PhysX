...
static const PXR_NS::UsdAttribute& getAttribute(const PXR_NS::UsdProperty& property)
{
    CARB_ASSERT(property.Is<PXR_NS::UsdAttribute>());
    return static_cast<const PXR_NS::UsdAttribute&>(property);
}

static const PXR_NS::UsdRelationship& getRelationship(const PXR_NS::UsdProperty& property)
{
    CARB_ASSERT(property.Is<PXR_NS::UsdRelationship>());
    return static_cast<const PXR_NS::UsdRelationship&>(property);
}

static bool getSingleRelationshipPath(const PXR_NS::UsdRelationship& relationship, PXR_NS::SdfPath& path)
{
    SdfPathVector paths;
    ...
    const PXR_NS::TfToken& property, const PXR_NS::UsdTimeCode& timeCode)
{
    ...
    const PXR_NS::UsdAttribute& attribute = getAttribute(attachedStage.getStage()->GetPrimAtPath(objectRecord->mPath).GetProperty(property));
    ...
    const PXR_NS::TfToken& property, const PXR_NS::UsdTimeCode& timeCode)
{
    ...
    const PXR_NS::UsdRelationship& relationship = getRelationship(attachedStage.getStage()->GetPrimAtPath(objectRecord->mPath).GetProperty(property));
    SdfPath path;
...
static bool updateVehicleWheelAttachmentData(InternalVehicleWheelAttachment* wheelAttachment,
    const PxVec3* valueFloat3, const bool* valueBool, const PXR_NS::UsdPrim* valuePrim, const PxQuat* valueQuat,
    ...
    const PXR_NS::TfToken& property, const PXR_NS::UsdTimeCode& timeCode)
{
    ...
    if (wheelAttachment)
    {
        ...
        const PXR_NS::UsdRelationship& relationship = getRelationship(wAttPrim.GetProperty(property));
    ...
    const PXR_NS::TfToken& property, const PXR_NS::UsdTimeCode& timeCode)
{
    ...
    if (wheelAttachment)
    {
        const PXR_NS::UsdAttribute& attribute = getAttribute(attachedStage.getStage()->GetPrimAtPath(path).GetProperty(property));
