...
void updateDeformableAttachmentShapeEvents(InternalActor* intActor, ObjectId shapeId, PXR_NS::SdfPath shapePath, DirtyEventType eventType)
{
    PhysXSetup& physxSetup = OmniPhysX::getInstance().getPhysXSetup();
...
void updateDeformableCollisionFilterShapeEvents(InternalActor* intActor, ObjectId shapeId, PXR_NS::SdfPath shapePath, DirtyEventType eventType)
{
    PhysXSetup& physxSetup = OmniPhysX::getInstance().getPhysXSetup();
...
bool hasPhysxJointAxisAPI(usdparser::AttachedStage& attachedStage, const PXR_NS::SdfPath& jointPrimPath, const TfToken& instance)
{
                                                ...
                                                const ObjectInstance* instance)
{
    ...
    case eScene:
    {
        ...
        PhysXScene* physxScene = physxSetup.createPhysXScene(attachedStage, outId, PXR_NS::UsdGeomGetStageMetersPerUnit(stage), PXR_NS::UsdPhysicsGetStageKilogramsPerUnit(stage), desc);
        if (physxScene)
        {
    ...
    case eDynamicBody:
    {
        ...
        {
            // setup envId for the body check for the prim var            
            UsdGeomPrimvar scenePartitionPrimVar = UsdGeomPrimvar(usdPrim.GetAttribute(TfToken(kScenePartitionPrimvar)));            
    ...
    case eStaticBody:
    {
        ...
        if (attachedStage.isUsingReplicatorEnvIds())
        {
            ...
            rigidStatic->setEnvironmentID(partition_id);
        }
...
GfTransform getSourceGPrimTransform(const AttachedStage& attachedStage, const PXR_NS::SdfPath& path, bool fabricChange)
{
    ...
    if (fabricChange && iStageReaderWriter)
    {
        ...
        if (fabricStageRW.getId() != omni::fabric::kInvalidStageReaderWriterId)
        {
            ...
            {
                return PXR_NS::GfTransform(*fabricStageRW.getAttributeRd<PXR_NS::GfMatrix4d>(pathHandle, *fcTokens.worldMatrix));
            }
        }
    }
    
    return PXR_NS::GfTransform(UsdGeomImageable(attachedStage.getStage()->GetPrimAtPath(path))
                                .ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default()));
}
                                               ...
                                               bool resetVelocity, bool scaleProvided)
{
    ...
    else if (internalType == ePTParticleSet)
    {
        ...
        PXR_NS::UsdGeomXform xform(internalParticleSet->mPrim);
        PXR_NS::GfMatrix4d localToWorld = xform.ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
        ...
        internalParticleSet->mWorldToLocal = xform.ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default()).GetInverse();
    }
...
static void addArticulationToScene(const usdparser::AttachedStage& attachedStage, InternalDatabase::Record& articulationRecord)
{
    ...
    if (!intArt->mAggregate)
    {
        ...
        if (attachedStage.isUsingReplicatorEnvIds())
        {
            ...
            const UsdPrim artPrim = attachedStage.getStage()->GetPrimAtPath(articulationRecord.mPath);
            UsdGeomPrimvar scenePartitionPrimVar = UsdGeomPrimvar(artPrim.GetAttribute(TfToken(kScenePartitionPrimvar)));            
            uint32_t partition_id = 0;
            if (scenePartitionPrimVar)
            {
                PXR_NS::TfToken scenePartitionToken;
...
inline ::physx::PxTransform computeWheelShapeLocalPose(const PXR_NS::UsdGeomXformable& wheelXform, const PXR_NS::UsdGeomXformable& shapeXform)
{
    ::physx::PxTransform pxWheelPose;
    {
        const PXR_NS::GfMatrix4d wheelPose = wheelXform.ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
    ...
    {
        const PXR_NS::GfMatrix4d shapePose = shapeXform.ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
...
static void prepareWheelTransforms(InternalVehicle::WheelTransformManagementEntry& wheelTMEntry, ::physx::PxTransform& wheelShapeLocalPose)
{
    ...
    PXR_NS::UsdGeomXformable wheelXform(wheelTMEntry.wheelRootPrim);
    ...
    if (wheelTMEntry.shape)
    {
        ...
        {
            PXR_NS::UsdGeomXformable shapeXform(wheelTMEntry.shapePrim);
...
PXR_NS::SdfPath PhysXUsdPhysicsInterface::getParentJointPathInArticulation(const usdparser::AttachedStage& attachedStage, const PXR_NS::SdfPath& jointPath)
{
