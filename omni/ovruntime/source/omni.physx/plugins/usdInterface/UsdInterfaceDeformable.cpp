                         ...
                         const SdfPathVector& skinGeomPaths)
{
    ...
    for (SdfPath skinGeomPath : skinGeomPaths)
    {
        ...
        if (skinGeom)
        {
            GfMatrix4d skinGeomToWorld = skinGeom.ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
...
bool PhysXUsdPhysicsInterface::updateDeformableRestOffset(const usdparser::AttachedStage& attachedStage, const ObjectId objectId, float value)
{
    ...
    float metersPerUnit = float(PXR_NS::UsdGeomGetStageMetersPerUnit(attachedStage.getStage()));
    ...
    const SdfPath& path, PhysxVolumeDeformableBodyDesc const& desc)
{
    ...
    GfMatrix4d simMeshToWorld = PXR_NS::UsdGeomMesh(simMeshPrim).ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
    GfMatrix4d worldToSimMesh = simMeshToWorld.GetInverse();

    GfMatrix4d worldToCollMesh = worldToSimMesh;
    if (collMeshPrim != simMeshPrim)
    {
        GfMatrix4d collMeshToWorld = PXR_NS::UsdGeomMesh(collMeshPrim).ComputeLocalToWorldTransform(PXR_NS::UsdTimeCode::Default());
    ...
    if (mExposePrimNames)
        deformableVolume->setName(bodyPrim.GetPrimPath().GetText());
