...
bool checkArticulatonBodySimulationOwners(AttachedStage& attachedStage, PXR_NS::UsdStageWeakPtr stage, const omni::physics::schema::ArticulationDesc& inDesc)
{
    bool firstBody = true;
...
void parseArticulation(const PXR_NS::UsdStageWeakPtr stage, const omni::physics::schema::ArticulationDesc& inDesc,
    CollisionPairVector& filteredPairs, ArticulationMap& articulationMap)
{
    for (size_t i = 0; i < inDesc.rootPrims.size(); i++)
    {
        ...
        // Newton fallback: newton:selfCollisionEnabled -> physxArticulation:enabledSelfCollisions
        {
            bool physxSelfCollAuthored = physxArticulationAPI &&
                physxArticulationAPI.GetEnabledSelfCollisionsAttr() &&
                physxArticulationAPI.GetEnabledSelfCollisionsAttr().HasAuthoredValue();
            if (!physxSelfCollAuthored)
            {
                newton::getBoolFallback(desc->selfCollision, inDesc.usdPrim,
                    NewtonSchemaTokens->newtonSelfCollisionEnabled);
            }
        }
...
void convertBodyToArticulationLink(UsdStageWeakPtr stage, PXR_NS::UsdGeomXformCache& xformCache, const SdfPath& bodyPath,
...
void createArticulationLinks(AttachedStage& attachedStage, PXR_NS::UsdGeomXformCache& xformCache,
                             BodyMap& bodyMap,
