...
template <typename T>
static bool getAttribute(T& val, const PXR_NS::UsdAttribute& attribute, T minVal, T maxVal, usdparser::OnUpdateObjectFn onUpdate)
{
    bool retVal = false;
...
static void getBoolAttribute(bool& val, const PXR_NS::UsdAttribute& attribute, usdparser::OnUpdateObjectFn onUpdate)
{
    if (attribute && attribute.HasAuthoredValue())
    {
        attribute.Get(&val);
...
template <typename T>
static void getAttribute(T& val, const PXR_NS::UsdAttribute& attribute, usdparser::OnUpdateObjectFn onUpdate)
{
    if (attribute)
    {
        attribute.Get(&val);
...
static bool getAttribute(
    T& val, const PXR_NS::UsdPrim prim, const PXR_NS::TfToken attrName, T minVal, T maxVal, usdparser::OnUpdateObjectFn onUpdate)
{
...
template <typename T>
static void getAttribute(T& val, const PXR_NS::UsdPrim prim, const PXR_NS::TfToken attrName, usdparser::OnUpdateObjectFn onUpdate)
{
