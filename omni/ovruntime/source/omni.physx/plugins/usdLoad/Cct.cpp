...
CctDesc* getCctShape(const PXR_NS::UsdStageWeakPtr stage, const PXR_NS::UsdPrim& usdPrim)
{
    if (usdPrim.IsA<UsdGeomCapsule>())
    {
        ...
        // Get xform transform
        PXR_NS::UsdGeomXformable xform(usdPrim);

        {
            const PXR_NS::GfTransform tr(xform.ComputeLocalToWorldTransform(UsdTimeCode()));
...
CctDesc* parseCct(AttachedStage& attachedStage, const PXR_NS::UsdPrim& usdPrim)
{
