...
bool fillPhysxShapeDesc(AttachedStage& attachedStage, const ShapeDesc& inDesc, PhysxShapeDesc& outDesc, CollisionPairVector& filteredPairs)
{
    ...
    // physx params
    bool physxContactOffsetAuthored = false;
    bool physxRestOffsetAuthored = false;
    ...
    if (physxCollisionAPI)
    {
        ...
        float restOffset = outDesc.restOffset;
        {
            ...
            {
                physxContactOffsetAuthored = true;
                float attrVal;
    ...
    // Newton fallbacks: NewtonCollisionAPI -> PhysxCollisionAPI
    // newton:contactMargin -> physxCollision:restOffset  (direct: surface inflation)
    // newton:contactGap    -> physxCollision:contactOffset = margin + gap
    //   (Newton gap is additive on top of margin, whereas PhysX contactOffset
    //    is measured from the original shape surface.)
    {
        const UsdPrim& collPrim = inDesc.usdPrim;

        float newtonMargin = 0.0f;
        bool hasNewtonMargin = false;
        if (!physxRestOffsetAuthored)
        {
            if (newton::getFloatFallback(newtonMargin, collPrim, NewtonSchemaTokens->newtonContactMargin))
            {
                if (newtonMargin >= 0.0f)
                {
                    outDesc.restOffset = newtonMargin;
                    hasNewtonMargin = true;
                }
            }
        }

        if (!physxContactOffsetAuthored)
        {
            float newtonGap;
            if (newton::getFloatFallback(newtonGap, collPrim, NewtonSchemaTokens->newtonContactGap))
            {
                // Newton uses -inf as "use default" sentinel; skip in that case.
                if (!isinf(newtonGap) && newtonGap >= 0.0f)
                {
                    // Newton gap is on top of margin; PhysX contactOffset is from the surface.
                    float margin = hasNewtonMargin ? newtonMargin : outDesc.restOffset;
                    outDesc.contactOffset = margin + newtonGap;
                }
            }
        }

        // Re-validate constraint: contactOffset > restOffset
        if (outDesc.contactOffset >= 0.0f && outDesc.contactOffset <= outDesc.restOffset)
        {
            // Equal values are not accepted by PhysX, lets add an epsilon
            const float epsilon = 1e-4f;
            outDesc.contactOffset = outDesc.restOffset + epsilon;
        }
    }
...
void processMeshesToMerge(UsdGeomXformCache& xfCache, const UsdPrim& prim, const SdfPathSet& paths, MergeMeshDesc& meshDesc, UsdTimeCode time)
{
    ...
    auto&& computeFunc = [numDataToProcess, batchSize, numBatches, meshDataVector, prim, &meshDesc](size_t batchIndex)
    {
        ...
        for (size_t dataIndex = startIndex; dataIndex < endIndex; dataIndex++)
        {
            ...
            {
                const GfVec3f newPoint = PXR_NS::GfVec3f(relMatrix.Transform(meshData.pointsValue[i]));
    ...
    omni::physx::PhysxCookingComputeRequest& request, UsdTimeCode time)
{
    ...
    if (useConvexHull)
    {
        ...
        // Newton fallback: newton:maxHullVertices -> hullVertexLimit
        {
            bool physxHullVertexAuthored = physxColMeshAPI &&
                physxColMeshAPI.GetHullVertexLimitAttr() &&
                physxColMeshAPI.GetHullVertexLimitAttr().HasAuthoredValue();
            if (!physxHullVertexAuthored)
            {
                newton::getIntAsUint32Fallback(convexMeshDesc->convexCookingParams.maxHullVertices,
                    shapeDesc.usdPrim, NewtonSchemaTokens->newtonMaxHullVertices);
            }
        }
        desc = convexMeshDesc;
    ...
    else
    {
        ...
        if (useConvexDecomposition)
        {
            ...
            // Newton fallback: newton:maxHullVertices -> hullVertexLimit (decomposition)
            {
                bool physxHullVertexAuthored = physxColMeshAPI &&
                    physxColMeshAPI.GetHullVertexLimitAttr() &&
                    physxColMeshAPI.GetHullVertexLimitAttr().HasAuthoredValue();
                if (!physxHullVertexAuthored)
                {
                    newton::getIntAsUint32Fallback(convexDecompositionDesc->convexDecompositionCookingParams.maxHullVertices,
                        shapeDesc.usdPrim, NewtonSchemaTokens->newtonMaxHullVertices);
                }
            }
            request.onFinished = [&](const omni::physx::PhysxCookingComputeResult& result)
