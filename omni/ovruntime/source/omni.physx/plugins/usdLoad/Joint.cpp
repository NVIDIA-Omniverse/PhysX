...
bool checkJointBodySimulationOwners(AttachedStage& attachedStage, PXR_NS::UsdStageWeakPtr stage, const omni::physics::schema::JointDesc& inDesc)
{
...
void setToDefault(PhysxJointAxisProperties& properties)
{
    ...
    properties.maxJointVelocity = 1000000.0f;
...
void fillPhysxLimitDesc(const omni::physics::schema::JointDesc& inDesc, PhysxJointLimit& limit, const PXR_NS::TfToken& axis)
{
    setToDefault(limit);
...
void fillPhysxJointAxisPropertiesDesc(const omni::physics::schema::JointDesc& inDesc, PhysxJointAxisProperties& properties, const PXR_NS::TfToken& axis)
{
    ...
    // Seed maxJointVelocity / armature from PhysxJointAPI (joint-level) using the USD schema to use it as a fallback
    // The per-axis block below then overrides them when the user explicitly opined per-axis.
    const PhysxSchemaPhysxJointAPI physxJointAPI = PhysxSchemaPhysxJointAPI::Get(inDesc.usdPrim.GetStage(), inDesc.usdPrim.GetPrimPath());
    if (physxJointAPI)
    {
    ...
    {
        if (axis == PXR_NS::UsdPhysicsTokens->linear)
        {
        ...
        else if (axis == PXR_NS::UsdPhysicsTokens->angular)
        {
        ...
        else if (axis == PXR_NS::UsdPhysicsTokens->rotX)
        {
        ...
        else if (axis == PXR_NS::UsdPhysicsTokens->rotY)
        {
        ...
        else if (axis == PXR_NS::UsdPhysicsTokens->rotZ)
        {
...
void fillPhysxPerformanceEnvelopeDesc(const omni::physics::schema::JointDesc& inDesc, PhysxJointDrive& drive, const PXR_NS::TfToken& axis)
{
...
void fillPhysicsJointStateDesc(const omni::physics::schema::JointDesc& inDesc, PhysicsJointState& state, const PXR_NS::TfToken& axis)
{
    // physx bits
...
PhysxJointDesc* parseJoint(PXR_NS::UsdStageWeakPtr stage, const omni::physics::schema::JointDesc& inDesc, PXR_NS::UsdGeomXformCache& xfCache)
{
    switch (inDesc.type)
    {
    ...
    case omni::physics::schema::ObjectType::eJointCustom:
    {
        ...
        {            
            ...
            if (PXR_NS::UsdRelationship rel = gearJoint.GetHinge0Rel())
            {
            ...
            if (PXR_NS::UsdRelationship rel = gearJoint.GetHinge1Rel())
            {
                PXR_NS::SdfPathVector paths;
                rel.GetTargets(&paths);
                if (!paths.empty())
        ...
        else if (inDesc.usdPrim.IsA<PhysxSchemaPhysxPhysicsRackAndPinionJoint>())
        {
            ...
            if (PXR_NS::UsdRelationship rel = rackJoint.GetHingeRel())
            {
                PXR_NS::SdfPathVector paths;
                rel.GetTargets(&paths);
                if (!paths.empty())
            ...
            if (PXR_NS::UsdRelationship rel = rackJoint.GetPrismaticRel())
            {
                PXR_NS::SdfPathVector paths;
                rel.GetTargets(&paths);
                if (!paths.empty())
