...
void refreshAutoDeformableAttachments(UsdStageWeakPtr stage,
    DeformableAttachmentVector& attachments, DeformableCollisionFilterVector& collisionFilters)
{
    const TfType autoAttachmentType = UsdSchemaRegistry::GetAPITypeFromSchemaTypeName(
        PhysxAdditionAPITokens->AutoDeformableAttachmentAPI);
    ...
    {
        SdfPath parentPath = attachment.path.GetParentPath();
        UsdPrim parentPrim = stage->GetPrimAtPath(parentPath);
        if (parentPrim && parentPrim.HasAPI(autoAttachmentType))
        {
                                 ...
                                 CollisionPairVector& filteredPairs)
{
    ...
    if (prim.HasAPI<PhysxSchemaPhysxCharacterControllerAPI>())
    {
    ...
    void parsePrim(const UsdPrim& prim, omni::physics::schema::ObjectDesc* objectDesc, uint64_t primTypes, const TfTokenVector& appliedApis) override
    {
        ...
        if (objectDesc)
        {
            ...
            case omni::physics::schema::ObjectType::eJointDistance:
            {
                ...
                parseNewtonMimicJoints(mAttachedStage.getStage(), prim, mMimicJoints);
            }
            break;
...
*/
        ...
        refreshAutoDeformableAttachments(mStage, mDeformableAttachmentVector, mDeformableCollisionFilterVector);
