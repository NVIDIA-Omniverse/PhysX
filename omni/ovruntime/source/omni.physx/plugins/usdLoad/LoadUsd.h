...
PhysxJointDesc* parseJoint(uint64_t stageId, const PXR_NS::SdfPath& path);
...
std::vector<PhysxTendonAttachmentHierarchyDesc*> parseSpatialTendons(uint64_t stageId, const PXR_NS::SdfPath& parentPath);
std::vector<PhysxArticulationDesc*> parseArticulations(uint64_t stageId, const PXR_NS::SdfPath& articulationRootPath);
std::vector<PhysxTendonAxisHierarchyDesc*> parseFixedTendons(uint64_t stageId, const PXR_NS::SdfPath& parentPath);
PhysxShapeDesc* parseCollision(uint64_t stageId, const PXR_NS::SdfPath& path, const PXR_NS::SdfPath& gPrimPath);
ParticleSetDesc* parseParticleSet(uint64_t stageId, const PXR_NS::SdfPath& path);
ParticleSamplingDesc* parseParticleSampling(uint64_t stageId, const PXR_NS::SdfPath& path);
...
VehicleDesc* parseVehicle(uint64_t stageId, const PXR_NS::SdfPath&, usdparser::VehicleComponentTrackerHandle);
ParticleSystemDesc* parseParticleSystem(uint64_t stageId, const PXR_NS::SdfPath& particleSystemPath);
...
public:
    ...
    void registerTimeSampledAttribute(const PXR_NS::UsdAttribute& attribute, OnUpdateObjectFn onUpdate)
    {
        uint64_t stageId = PXR_NS::UsdUtilsStageCache::Get().GetId(attribute.GetStage()).ToLongInt();
        AttachedStageMap::iterator fit = mAttachedStages.find(stageId);
        if (fit != mAttachedStages.end())
        {
...
private:
    ...
    PXR_NS::TfNotice::Key mAttributeValuesChangedListenerKey;
