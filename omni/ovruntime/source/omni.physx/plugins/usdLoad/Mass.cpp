...
float convertSiValueToStageUnits(const PXR_NS::UsdStageWeakPtr stage, float siValue, int distanceExponent, int massExponent)
{
...
void RequestRigidBodyMassUpdate(AttachedStage& attachedStage, const PXR_NS::UsdPrim& usdPrim)
{
...
RigidBodyMass computeRigidBodyMass(AbstractComputeRigidBodyMass* crbmInterface, const PXR_NS::UsdStageWeakPtr stage, const PXR_NS::UsdPrim& usdPrim, usdparser::ObjectId rbId)
{
        ...
        {
            {
                ...
                if (massDesc.accumulateMass || !massInfo.hasInertia || !hasCoM)
                {
                    ...
                    for (const std::pair<usdparser::ObjectId, PXR_NS::UsdPrim>& shapePair : shapeIds)
                    {
...
void RequestParticleMassUpdate(AttachedStage& attachedStage, const PXR_NS::UsdPrim& usdPrim)
{
    const SdfPath primPath = usdPrim.GetPrimPath();
    const ObjectIdMap* entries = attachedStage.getObjectIds(primPath);
