...
void RequestRigidBodyMassUpdate(AttachedStage& stage, const PXR_NS::UsdPrim& usdPrim);
void RequestParticleMassUpdate(AttachedStage& stage, const PXR_NS::UsdPrim& usdPrim);
 ...
 */
float convertSiValueToStageUnits(const PXR_NS::UsdStageWeakPtr stage,
                                 float siValue,
                                 int distanceExponent = 0,
                                 int massExponent = 0);

inline float getScaledDensity(const PXR_NS::UsdStageWeakPtr stage, float baseValue)
{
    return baseValue <= 0.0f ? baseValue : convertSiValueToStageUnits(stage, baseValue, -3, 1);
}

inline float getScaledMass(const PXR_NS::UsdStageWeakPtr stage, float baseValue)
{
