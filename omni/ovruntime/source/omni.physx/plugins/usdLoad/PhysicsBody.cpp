    ...
    CollisionPairVector& filteredPairs, bool ignoreOwners)
{
    ...
    if (inDesc.rigidBodyEnabled)
    {
        ...
        if (outputVelocitiesLocalSpace)
        {
            ...
            transformedVelocity = PXR_NS::GfVec3f(rot.TransformDir(transformedVelocity));
            transformedVelocity = GfCompMult(scale, transformedVelocity);
            transformedAngularVelocity = PXR_NS::GfVec3f(rot.TransformDir(transformedAngularVelocity));
        }
    ...
    if ((schemaFlags & SchemaAPIFlag::eAutoDeformableBodyAPI) > 0)
    {
        ...
        if (cookingSourceMeshRel)
        {
            ...
            if (pathVector.size() > 0)
            {
                ...
                if (cookingSrcMesh)
                {
                    PXR_NS::TfToken bindPoseToken = getPoseNameFromPurpose(cookingSrcMesh.GetPrim(), OmniPhysicsDeformableAttrTokens->bindPose);
