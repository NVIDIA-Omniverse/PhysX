...
bool isMovableBody(const AttachedStage& attachedStage, const PXR_NS::SdfPath& primPath)
{
...
bool isNonMovable(const AttachedStage& attachedStage, const PXR_NS::UsdPrim& prim, PXR_NS::UsdPrim& resyncPrim)
{
    ...
    while (parent)
    {
        ...
        if (parentAPIs & SchemaAPIFlag::eRigidBodyAPI)
        {
            ...
            {
                const PXR_NS::UsdPrim& primC = *iter;
                if (!primC)
                    continue;
...
SdfPath findDeformableBodyAncestor(AttachedStage& attachedStage, const SdfPath primPath)
{
    ...
    PXR_NS::UsdPrim prim = attachedStage.getStage()->GetPrimAtPath(primPath);
    PXR_NS::UsdPrim deformableBodyPrim = PXR_NS::UsdPrim();
    while (prim && !deformableBodyPrim)
    {
        ...
        else if (prim.HasAPI<PXR_NS::UsdPhysicsRigidBodyAPI>())
        {
            prim = PXR_NS::UsdPrim();
        }
        else
        {
            ...
            prim = resetXformStack ? PXR_NS::UsdPrim() : prim.GetParent();
        }
    }
...
void moveBody(AttachedStage& attachedStage, const PXR_NS::SdfPath& primPath, const PXR_NS::UsdPrim* primIn, UsdGeomXformCache* xfCache, bool fabricCache)
{
    UsdLoad* usdLoad = UsdLoad::getUsdLoad();
        ...
        if (iStageReaderWriter)
        {
            ...
            if (stageInProgress.id)
            {
                ...
                // case a pointer to a T. Will be NULL if attribute doesn't exist in fabric
                {
                    ...
                    const PXR_NS::GfMatrix4d* valuePtr = (const PXR_NS::GfMatrix4d*)valueSpan.ptr;
...
void movePointInstancer(AttachedStage& attachedStage, const PXR_NS::UsdPrim& prim)
{
...
void PrimChangeMap::handleTransformChange(AttachedStage& attachedStage, const PXR_NS::SdfPath& primPath, const PXR_NS::UsdPrim* primIn, PXR_NS::UsdGeomXformCache* xfCache, bool fastCache)
{
    TRACE_FUNCTION();
...
void PrimChangeMap::checkPrimChange(AttachedStage& attachedStage, const PXR_NS::SdfPath& primPath, const PXR_NS::TfToken& propertyName, const PXR_NS::UsdPrim* primIn)
{
            ...
            (change.onPrimCheckExt == nullptr || change.onPrimCheckExt(attachedStage, primPath, propertyName, primIn, resyncPrimPath)))
        {
            ...
            if (!usdLoad->getAsyncUSDUpdate())
            {
                ...
                if (entries && !entries->empty())
                {
                    ...
                    while (it != entries->end())
                    {
                        attachedStage.getPhysXPhysicsInterface()->updateObject(attachedStage, primPath, it->second, change.onUpdate, propertyName, PXR_NS::UsdTimeCode::Default());
                        it++;
                    }
                }
                return;
            ...
            else
            {
                ...
                if (!attributeSet)
                {
                    ...
                    ChangeMap::iterator it =
                        m_changeMap.insert(std::pair<const PXR_NS::SdfPath, ChangeData>(primPath, { change.onUpdate, propertyName }));
                }
                return;
            }
        }
...
void PrimChangeMap::removePrim(const PXR_NS::SdfPath& primPath)
{
...
bool PrimUpdateMap::needsSceneReset(const PXR_NS::UsdPrim& usdPrim)
{
...
void PrimUpdateMap::addPrim(const AttachedStage& attachedStage, const PXR_NS::UsdPrim& prim)
{
    if (m_isNewScene)
        return;
    ...
    if (!isInPrimAddMap(prim))
    {
        ...
        for (PXR_NS::UsdPrimRange::const_iterator iter = range.begin(); iter != range.end(); ++iter)
        {
            const PXR_NS::UsdPrim& childPrim = *iter;
            if (!childPrim)
                continue;
...
void PrimUpdateMap::removePrim(AttachedStage& attachedStage, const PXR_NS::SdfPath& primPath)
{
...
bool PrimUpdateMap::isInPrimAddMap(const PXR_NS::UsdPrim& prim) const
{
    if (m_isNewScene)
        return true;
...
void PrimUpdateMap::checkMap(const PXR_NS::UsdStageWeakPtr stage)
{
...
void processChangeMap(AttachedStage& attachedStage)
{
    ...
    while (itCh != itChEnd)
    {
        ...
        if (entries && !entries->empty())
        {
            ...
            while (it != entries->end())
            {
                attachedStage.getPhysXPhysicsInterface()->updateObject(attachedStage, primPath, it->second, changeData.first, changeData.second, PXR_NS::UsdTimeCode::Default());
                it++;
            }
        }
        itCh++;
    ...
    if (change.onPrimCheck(attachedStage, primPath, attributeName, nullptr))
    {
        ...
        if (change.onUpdate)
        {
            ...
            if (entries && !entries->empty())
            {
                ...
                while (it != entries->end())
                {
                    attachedStage.getPhysXPhysicsInterface()->updateObject(attachedStage, primPath, it->second, change.onUpdate, attributeName, PXR_NS::UsdTimeCode::Default());
                    it++;
                }
            }
        }
...
// Fabric codepath
void fabricTransformationChange(AttachedStage& attachedStage, const gsl::span<const PXR_NS::GfMatrix4d>& matrices, size_t index,  const PXR_NS::SdfPath& primPath)
{
    bool bodyMoved = false;
    if (isMovableBody(attachedStage, primPath))
    {
            ...
            for (size_t j = 0; j < changes.attrChangedIndices.size(); j++)
            {
                ...
                while (changeFound && itChange != itChangeEnd && itChange->first == attributeName)
                {
                    ...
                    else
                    {
                        for (const size_t index : changeType.changedIndices)
                        {
                            ...
                            const PXR_NS::SdfPath primPath = omni::fabric::toSdfPath(paths[index]);
                ...
                if (!changeFound)
                {
                    if ((attributeName == fabricLocalMatrix ) && !transformationUpdated)
                    {
                        ...
                        gsl::span<const PXR_NS::GfMatrix4d> matrices = stageInProgress.getAttributeArrayRd<PXR_NS::GfMatrix4d>(changesByType, i, *usdLoad->getFabricTokens().worldMatrix);
...
void flushBufferedChanges(AttachedStage& attachedStage, float currentTime)
{
...

    {
        ...
        if (!OmniPhysX::getInstance().getCachedSettings().suppressFabricUpdate && OmniPhysX::getInstance().getIStageReaderWriter())
        {
    ...
    if (!attachedStage.getAnimatedKinematicBodies().empty() || !attachedStage.getTimeSampleMap().empty())
    {
...

        {
            ...
            for (PathPrimMap::const_reference& prims : attachedStage.getAnimatedKinematicBodies())
            {
                ...
                PXR_NS::GfMatrix4d localToWorld = xfCache.GetLocalToWorldTransform(prim);
...
void UsdNoticeListener::Handle(const class PXR_NS::UsdNotice::ObjectsChanged& objectsChanged)
{
    TRACE_FUNCTION();
    ...
    const uint64_t stageId = PXR_NS::UsdUtilsStageCache::Get().GetId(objectsChanged.GetStage()).ToLongInt();
...
void UsdNoticeListener::HandleAttributeValuesChanged(const class omni::fabric::AttributeValuesChangedNotice& valuesChanged)
{
    ...
    {
        const PXR_NS::SdfPath& path = valuesChanged.GetPrimPaths()[i];
        const PXR_NS::TfToken& name = valuesChanged.GetAttributeNames()[i];
