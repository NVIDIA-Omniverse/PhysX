...
using ChangeMap = std::unordered_multimap<const PXR_NS::SdfPath, ChangeData, PXR_NS::SdfPath::Hash>; // A.B. this could become
...
public:
    ...
    void addPrim(const AttachedStage& attachedStage, const PXR_NS::UsdPrim&);
    ...
    bool needsSceneReset(const PXR_NS::UsdPrim&);
...
public:
    ...
    void addTransformChange(const PXR_NS::SdfPath& path, const PXR_NS::UsdPrim* prim, bool fabric)
    {
        if (fabric)
