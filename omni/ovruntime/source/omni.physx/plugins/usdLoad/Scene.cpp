...
void setToDefault(UsdStageWeakPtr stage, PhysxSceneDesc& desc)
{
    ...
    const TfToken upAxis = PXR_NS::UsdGeomGetStageUpAxis(stage);
...
PhysxSceneDesc* parseSceneDesc(const UsdStageWeakPtr stage, const omni::physics::schema::SceneDesc& inDesc)
{
    ...
    if (physxSceneAPI)
    {
        ...
        physxTimeStepsAuthored = getAttribute(sceneDesc->timeStepsPerSecond, physxSceneAPI.GetTimeStepsPerSecondAttr(), 1u, UINT_MAX, nullptr);
        ...
        if (physxSceneAPI.GetGpuMaxDeformableVolumeContactsAttr())
            physxSceneAPI.GetGpuMaxDeformableVolumeContactsAttr().Get(&sceneDesc->gpuMaxDeformableVolumeContacts);
    ...
    // Newton schema fallbacks (NewtonSceneAPI / NewtonXpbdSceneAPI / NewtonKaminoSceneAPI).
    // Only apply when the corresponding PhysX attribute was not authored.
    {
        const UsdPrim& scenePrim = inDesc.usdPrim;

        // newton:timeStepsPerSecond -> physxScene:timeStepsPerSecond.
        // Newton schema declares this as int, so read via the int->uint32 helper to
        // avoid attr.Get<uint32_t>() silently failing on the int-typed attribute.
        if (!physxTimeStepsAuthored)
        {
            newton::getIntAsUint32Fallback(sceneDesc->timeStepsPerSecond, scenePrim,
                NewtonSchemaTokens->newtonTimeStepsPerSecond);
        }

        // newton:gravityEnabled — when false, disable gravity by zeroing magnitude.
        // When true (default), per-body PhysX disableGravity flags still work.
        // PhysX wins: skip the Newton toggle when physics:gravityMagnitude is authored.
        const UsdAttribute physxMagnitudeAttr = scenePrim.GetAttribute(UsdPhysicsTokens.Get()->physicsGravityMagnitude);
        const bool physxMagnitudeAuthored = physxMagnitudeAttr && physxMagnitudeAttr.HasAuthoredValue();
        if (!physxMagnitudeAuthored)
        {
            bool gravityEnabled = true;
            if (newton::getBoolFallback(gravityEnabled, scenePrim, NewtonSchemaTokens->newtonGravityEnabled))
            {
                if (!gravityEnabled)
                {
                    sceneDesc->gravityMagnitude = 0.0f;
                }
            }
        }
    }
