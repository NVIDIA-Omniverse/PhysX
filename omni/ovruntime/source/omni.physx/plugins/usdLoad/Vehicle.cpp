...
struct ParseContext
{
    ParseContext(const PXR_NS::UsdStageWeakPtr stage_, VehicleComponentTracker& vehicleComponentTracker_)
        : stage(stage_),
...
TireFrictionTableDesc* parseTireFrictionTable(const PXR_NS::UsdStageWeakPtr stage, const PXR_NS::UsdPrim& usdPrim)
{
    ...
    if (tireFrictionTableDesc)
    {
        ...
        if (frictionValuesAttr.HasAuthoredValue())
        {
            ...
            const PXR_NS::VtArray<float>& arrayData = arrayDataValue.Get<PXR_NS::VtArray<float>>();
...
bool parseComponent(const PXR_NS::UsdPrim& prim, const PXR_NS::SdfPath& path,
                    VehicleComponentTracker& vehicleComponentTracker,
                    std::map<PXR_NS::SdfPath, TDesc*>& componentMap,
                    bool (*parseFunction)(TDesc&, const PXR_NS::UsdPrim&, const void*),
                    const void* userData,
...
template <typename TDesc, typename TDescOut>
bool parseComponentWithMapCheck(const PXR_NS::UsdPrim& prim, const PXR_NS::SdfPath& path,
                    VehicleComponentTracker& vehicleComponentTracker,
                    std::map<PXR_NS::SdfPath, TDesc*>& componentMap,
                    bool (*parseFunction)(TDesc&, const PXR_NS::UsdPrim&, const void*),
                    const void* userData,
...
bool parseRelationship(const PXR_NS::UsdStageWeakPtr stage,
                       const PXR_NS::UsdPrim& primWithRelationship,
                       const PXR_NS::SdfPath& relationshipPath,
                       VehicleComponentTracker& vehicleComponentTracker,
                       std::map<PXR_NS::SdfPath, TDesc*>& componentMap,
                       bool (*parseFunction)(TDesc&, const PXR_NS::UsdPrim&, const void*),
                       const void* userData,
...
bool parseRelationshipOrAPI(const PXR_NS::UsdStageWeakPtr stage,
                       const PXR_NS::UsdPrim& primWithRelationship,
                       ...
                       TDescOut*& out)
{
    ...
    else if (errCode == 1)
    {
        if (primWithRelationship.HasAPI<TSchema>())
        {
            ...
            const PXR_NS::SdfPath& path = primWithRelationship.GetPath();
                          ...
                          VehicleComponentTracker& vehicleComponentTracker)
{
    ...
    PXR_NS::GfQuatf suspFrameOrient;
    ...
    PXR_NS::GfQuatf wheelFrameOrient;
...
static bool parseDrive(const PXR_NS::UsdPrim& usdPrim, const PhysxSchemaPhysxVehicleAPI& usdVehicleAPI,
    const ParseContext& parseContext,
    DriveDesc*& out)
{
    SdfPath drivePath;
...
bool parseVehicleContext(const PXR_NS::UsdPrim& usdPrim, VehicleContextDesc& vehicleContextDesc)
{
    ...
    {
        if (axis != PXR_NS::PhysxSchemaTokens.Get()->undefined)
        {
...
static void testDriveAndWheelIndexList(const DriveDesc* drive,
    ...
    const PXR_NS::UsdPrim& usdPrim, const char* apiName,
    bool& isValid)
{
    if (!drive)
    {
        ...
        if (usdPrim.HasAPI<UsdPhysicsRigidBodyAPI>())
        {
            if (usdPrim.IsA<UsdGeomXformable>())
            {
                ...
                if ((vehicleDesc.scale.x != 0.0f) && (vehicleDesc.scale.y != 0.0f) && (vehicleDesc.scale.z != 0.0f))
                {
                    ...
                    if (usdPrim.GetMetadataByDictKey(PXR_NS::SdfFieldKeys->CustomData, PhysxSchemaTokens->referenceFrameIsCenterOfMass, &referenceFrameIsCenterOfMassVal))
                    {
