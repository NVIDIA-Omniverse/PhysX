...
def AddPBDMaterialWater(p):
    ...
    add_pbd_particle_material(stage=p.GetStage(), path=p.GetPath(), cohesion=0.01, damping=0, friction=0.1, surface_tension=0.0074*mpu*mpu*mpu, viscosity=0.0000017/(mpu*mpu*mpu), vorticity_confinement=0)

def AddPBDMaterialViscous(p):
    mpu = UsdGeom.GetStageMetersPerUnit(p.GetStage())
    add_pbd_particle_material(stage=p.GetStage(), path=p.GetPath(), cohesion=0.2, damping=0, friction=0.2, surface_tension=0.1*mpu*mpu*mpu, viscosity=0.0017/(mpu*mpu*mpu), vorticity_confinement=0)
