...
# --- Threading defaults (match Kit CI configuration, OMPE-59303) ---
os.environ.setdefault("OPENBLAS_NUM_THREADS", "16")

# --- Path setup ---

# _carb_setup.py is in source/omni.physx/tests/python/ -> ovruntime root is four levels up
...
# USD ext physics (physxSchema, physicsSchemaTools, etc.)
# In devschema mode (OVRUNTIME_DEV_SCHEMA=ON), the locally-built schema in
# ../schema/_build/ is used instead of the packman usd_ext_physics package.
_SCHEMA_LOCAL = OVRUNTIME_DIR.parent / "schema" / "_build" / _PLATFORM / "release" / "schema"
if _SCHEMA_LOCAL.exists():
    USD_EXT_PHYSICS_DIR = _SCHEMA_LOCAL
else:
    USD_EXT_PHYSICS_DIR = TARGET_DEPS / "usd_ext_physics" / "release"

# ovruntime build output (physics plugins: omni.physx.plugin.so, _physx.so bindings, etc.)
OVRUNTIME_BIN_DIR = BUILD_DIR / _PLATFORM / "release"

# USD libraries (needed at runtime by _physx.so and other physics bindings)
...
# Add physics Python bindings (_physx.so etc.) to sys.path
...
import carb.settings  # noqa: E402 - registers ISettings pybind11 type

framework = carb.get_framework()

# Threading limits are passed via the startup config JSON so they are in the
# settings database BEFORE any plugins are loaded.  carb.tasking.plugin and
# omni.tbb.globalcontrol read their thread counts during carbOnPluginStartup,
# so setting them after load_plugins() is too late.  (Match Kit CI defaults,
# OMPE-59303.)
_startup_config = """{
    "log": { "level": "warning" },
    "plugins": {
        "carb.tasking.plugin": { "threadCount": 16 },
        "omni.tbb.globalcontrol": { "maxThreadCount": 16 }
    }
}"""
framework.startup([], _startup_config)
