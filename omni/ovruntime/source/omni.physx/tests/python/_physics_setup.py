...
_rtx_search_paths = []
for _subdir in [
    "carb.stats", "omni.fabric",
    "carb.datastore", "omni.blobkey", "carb.ujitsoagent", "carb.ujitso.default",
]:
    _p = _RTX / _subdir
    if _p.exists():
...
# Load plugins from the omni.usd dir (omni.tbb.globalcontrol, omni.usd-abi, etc.)
_usd_dir = _RTX / "omni.usd"
if _usd_dir.exists():
    _carb_setup.framework.load_plugins(["*"], search_paths=[str(_usd_dir)])
...
# (already added in _carb_setup via USD_EXT_PHYSICS_DIR / "lib", but ensure it's present)
if sys.platform == "win32" and hasattr(os, "add_dll_directory"):
    _ext_lib = _carb_setup.USD_EXT_PHYSICS_DIR / "lib"
    if _ext_lib.exists():
...
_USD_EXT_PHYSICS = _carb_setup.USD_EXT_PHYSICS_DIR
...
# --- Register Newton USD schemas ---
# The newton-usd-schemas pip package is installed into newton_prebundle/.
# Its __init__.py calls Plug.Registry().RegisterPlugins() on import.
_NEWTON_PREBUNDLE = _carb_setup.TARGET_DEPS / "newton_prebundle"
if _NEWTON_PREBUNDLE.exists():
    if str(_NEWTON_PREBUNDLE) not in sys.path:
        sys.path.insert(0, str(_NEWTON_PREBUNDLE))
    try:
        import newton_usd_schemas  # noqa: F401, E402 - self-registers with Plug.Registry
    except ImportError:
        pass
