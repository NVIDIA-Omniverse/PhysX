...
#
# Ported from omni.physx.tests articulationTestBase.py for standalone ovruntime testing.
...
class ArticulationTestBase:
    ...
    def articulation_stage_and_scene_setup(self):
    ...
    def _setup_fixed_joint(self):
        ...
        joint_parent_pos = Gf.Vec3f(0, 0.5, 0)
        ...
        joint_child_pos = Gf.Vec3f(0, -0.5, 0)
    ...
    def _axis_angle_to_quat(self, axis: Union[str, Gf.Vec3d, Gf.Vec3f], angle: float) -> Gf.Quatf:
        if axis == "X":
            axis = Gf.Vec3d.XAxis()
        elif axis == "Y":
            axis = Gf.Vec3d.YAxis()
    ...
    def _setup_stage_and_bodies(self, setupAsArticulation=False):
