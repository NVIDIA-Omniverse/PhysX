...
#
# Ported from omni.physx.tests filterTestBase.py for standalone ovruntime testing.

import carb
import physicsUtils
import _physx
from omni.physx.scripts import deformableMeshUtils, deformableUtils, particleUtils
...
class FilterTestBase:
    ...
    def setup_units_and_scene(self):
        ...
        # Create scene with 0 gravity (using direct USD API instead of Kit command)
    ...
    def setup_cube_trimesh(self, stage, path, transform, dim):
    ...
    def setup_plane_trimesh(self, stage, path, transform, dim):
    ...
    def setup_colliders_volume_deformable(self, path, ext, density, pos, vel):
        defbody_path = Sdf.Path(physicsUtils.get_stage_next_free_path(self._stage, path, True))
    ...
    def setup_colliders_volume_deformable_hierarchy(self, path, ext, density, pos, vel):
        ...
        success = _physx.acquire_physx_cooking_interface().cook_auto_deformable_body(str(defbody_path))
        self.assertTrue(success)
    ...
    def setup_colliders(self, physics_type, is_dyn, ext, pos, vel):
        ...
        if physics_type == 'RigidBody':
            ...
            articulation_path = Sdf.Path(physicsUtils.get_stage_next_free_path(self._stage, path, True))
        ...
        # DEPRECATED
            ...
            _physx.acquire_physx_cooking_interface().cook_deformable_body_mesh(str(defbody_path))
            ...
            defsurface_path = Sdf.Path(physicsUtils.get_stage_next_free_path(self._stage, path, True))
            scale = Gf.Vec3d(ext*0.5)
            translate = Gf.Vec3d(pos)
            transform = create_transform(translate=translate, scale=scale)
        ...
        #~DEPRECATED
            ...
            points_path = Sdf.Path(physicsUtils.get_stage_next_free_path(self._stage, path, True))
            ...
            raise NotImplementedError(f"Physics type '{physics_type}' is not supported in ovruntime standalone tests")
    ...
    @staticmethod
    def _compute_prim_aabb(prim):
        """Compute world-space AABB for a single prim from its extent or points."""
        aabb = Gf.Range3d()
    ...
    def get_min_max(prim):
        aabb = FilterTestBase._compute_prim_aabb(prim)
        if aabb.IsEmpty():
            # Xform or other non-geometry prim: union bounds of children
            for child in prim.GetChildren():
                aabb.UnionWith(FilterTestBase._compute_prim_aabb(child))
