...
#
# Ported from omni.physx.tests BaseTestCases.py for standalone ovruntime testing.
...
from massTestBase import MassTestBase
import physicsUtils
from pxr import Gf, UsdGeom, UsdUtils


class MemoryStageBaseTest(PhysicsMemoryStageBaseTestCase):
    ...
    def test_new_stage(self):
        ...
        detached_stage = self.new_stage(attach_stage=False)
    ...
    def test_attach_stage(self):
        ...
        self.new_stage(attach_stage=False)
    ...
    def test_detach_stage(self):
        self.new_stage(attach_stage=True)
        physicsUtils.add_rigid_box(self._stage, "/box")

        self.detach_stage()
        self._check_physx_object_counts({"numBoxShapes": 0, "numDynamicRigids": 0, "numStaticRigids": 0})

    @unittest.skip("Requires omni.usd.get_context() and Kit stage (Kit-only)")
    def test_kit_stage_not_present_in_test_when_stepping_after_detaching_stage(self):
        pass
...
class ArticulationTestBaseTests(PhysicsMemoryStageBaseTestCase, ArticulationTestBase):
    category = TestCategory.Core
    ...
    def test_get_rotation_quaternion(self):
        self.new_stage()
...
class MassTestBaseTests(PhysicsMemoryStageBaseTestCase, MassTestBase):
    category = TestCategory.Core

    def test_calculate_shape_inertia(self):
        self.mass_test_stage_setup()
        # shape: expected inertia
        expected_results = {
            "cube": Gf.Vec3f(5 / 3),
            "cuboid": Gf.Vec3f(65 / 6, 25 / 3, 25 / 6),
            "sphere": Gf.Vec3f(4),
    ...
    def test_calculate_shape_mass(self):
    ...
    def test_check_shape_mass_properties(self):
        self.mass_test_stage_setup()
        physicsUtils.add_rigid_box(self._stage, "/cube", density=self.defaultDensity)
        # check when com is none and scale_com is true.
        self.check_shape_mass_properties("cube", "/cube", Gf.Vec3f(1), Gf.Vec3f(1), scale_com=True)
