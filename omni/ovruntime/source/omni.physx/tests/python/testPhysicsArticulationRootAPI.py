...
#
# Ported from omni.physx.tests PhysicsArticulationRootAPI.py for standalone ovruntime testing.
...
class PhysicsArticulationRootAPITestMemoryStage(PhysicsMemoryStageBaseTestCase, ArticulationTestBase):
    ...
    def test_articulation_joint_excluded_error_closed_circuit(self):
    ...
    def test_articulation_change_listener_enable_disable(self):
        UsdPhysics.ArticulationRootAPI.Apply(self._boxes_parent_xform.GetPrim())
    ...
    def test_articulation_parse_not_created_when_disabled(self):
        UsdPhysics.ArticulationRootAPI.Apply(self._boxes_parent_xform.GetPrim())
        physx_articulation_api = PhysxSchema.PhysxArticulationAPI.Apply(self._boxes_parent_xform.GetPrim())
        physxUtils.createJoint(self._stage, "Fixed", None, self._box1_rigid.GetPrim())
        physxUtils.createJoint(self._stage, "Revolute", self._box1_rigid.GetPrim(), self._box2_rigid.GetPrim())
    ...
    def test_articulation_parse_created_when_enabled(self):
        UsdPhysics.ArticulationRootAPI.Apply(self._boxes_parent_xform.GetPrim())
        physx_articulation_api = PhysxSchema.PhysxArticulationAPI.Apply(self._boxes_parent_xform.GetPrim())
        physxUtils.createJoint(self._stage, "Fixed", None, self._box1_rigid.GetPrim())
        physxUtils.createJoint(self._stage, "Revolute", self._box1_rigid.GetPrim(), self._box2_rigid.GetPrim())
    ...
    def test_articulation_created_when_applied_to_xform(self):
        UsdPhysics.ArticulationRootAPI.Apply(self._boxes_parent_xform.GetPrim())
    ...
    def test_articulation_not_created_when_applied_to_static_body(self):
        UsdPhysics.CollisionAPI.Apply(self._box0.GetPrim())
    ...
    def test_articulation_not_created_when_applied_to_disabled_rigidbody(self):
        UsdPhysics.CollisionAPI.Apply(self._box0.GetPrim())
    ...
    def test_articulation_parse_created_then_deleted(self):
        UsdPhysics.ArticulationRootAPI.Apply(self._boxes_parent_xform.GetPrim())
        physx_articulation_api = PhysxSchema.PhysxArticulationAPI.Apply(self._boxes_parent_xform.GetPrim())
        physxUtils.createJoint(self._stage, "Fixed", None, self._box1_rigid.GetPrim())
        physxUtils.createJoint(self._stage, "Revolute", self._box1_rigid.GetPrim(), self._box2_rigid.GetPrim())

        _physx.acquire_physx_simulation_interface().flush_changes()
    ...
    def test_articulation_parse_kinematics_error(self):
        UsdPhysics.ArticulationRootAPI.Apply(self._boxes_parent_xform.GetPrim())
    ...
    def _run_articulation_teleport_test(self):
    ...
    def test_floating_root_teleport(self):
        self._disable_gravity()
        self._setup_two_link_articulation(make_fixed_base=False)
        self._run_articulation_teleport_test()

    def test_fixed_root_teleport(self):
        self._disable_gravity()
        self._setup_two_link_articulation(make_fixed_base=True)
        self._run_articulation_teleport_test()

    def test_fixed_articulation_delete(self):
        self._setup_two_link_articulation(make_fixed_base=False)
    ...
    def test_floating_root_linear_velocity(self):
        self._disable_gravity()
        self._setup_two_link_articulation(make_fixed_base=False)
        self._setup_fixed_joint()
    ...
    def test_floating_root_angular_velocity(self):
        self._disable_gravity()
        self._setup_two_link_articulation(make_fixed_base=False)
        self._setup_fixed_joint()
        self._body_api = UsdPhysics.RigidBodyAPI(self._base_link.GetPrim())
    ...
    def test_articulation_root_delete(self):
        self._disable_gravity()
        self._setup_two_link_articulation(make_fixed_base=False)
    ...
    def test_articulation_sleeping(self):
        self._disable_gravity()
        self._setup_two_link_articulation(make_fixed_base=True)
        self._setup_revolute_joint()
        ...
        is_sleeping = _physx.acquire_physx_simulation_interface().is_sleeping(stage_id, art_encoded)
        self.assertTrue(not is_sleeping)

        _physx.acquire_physx_simulation_interface().put_to_sleep(stage_id, art_encoded)
        self.step()
        is_sleeping = _physx.acquire_physx_simulation_interface().is_sleeping(stage_id, art_encoded)
        self.assertTrue(is_sleeping)

        _physx.acquire_physx_simulation_interface().wake_up(stage_id, art_encoded)
        self.step()
        is_sleeping = _physx.acquire_physx_simulation_interface().is_sleeping(stage_id, art_encoded)
        self.assertTrue(not is_sleeping)

    def test_articulation_link_sleeping(self):
        self._disable_gravity()
        self._setup_two_link_articulation(make_fixed_base=False)
        self._setup_revolute_joint()
    ...
    def test_articulation_collision_resync(self):
        self._disable_gravity()
    ...
    def test_articulation_collision_delete(self):
        self._disable_gravity()
        self._link_length = 0.05 * self._length_scale
        self._link_width = self._link_length
        size = Gf.Vec3f(self._link_width, self._link_length, self._link_width)
    ...
    #
    def test_articulation_rigid_body_enabled_false(self):
        self._setup_stage_and_bodies()
    ...
    def test_articulation_max_velocity_angular(self):
        self._disable_gravity()
        self._revolute_test_setup("X", "vel", setupAsArticulation=True, targetValue=360.0 * 100.0)
        rigidBodyAPI = UsdPhysics.RigidBodyAPI(self._dynamic_box)
    ...
    def test_articulation_max_velocity_linear(self):
        self._disable_gravity()
        self._prismatic_test_setup("X", "vel", setupAsArticulation=True, targetValue=1000000.0)
        rigidBodyAPI = UsdPhysics.RigidBodyAPI(self._dynamic_box)
        physxJointAPI = PhysxSchema.PhysxJointAPI.Apply(self._joint.GetPrim())
