...
#
# Ported from omni.physx.tests PhysicsDriveAPI.py for standalone ovruntime testing.
...
class PhysicsDriveAPITestMemoryStage(PhysicsMemoryStageBaseTestCase, ArticulationTestBase):
    ...
    def test_articulation_spherical_joint_with_attached_d6_drive(self):
        cone_angle_x = 45
        cone_angle_z = 45
    ...
    def test_articulation_revolute_joint_drive(self):
    ...
    def test_articulation_revolute_joint_drive_force_limit(self):
        self._setup_revolute_joint()
        deflection_angle = 5
        self._setup_revolute_drive(deflection_angle)
    ...
    # and not an impulse).
    #
    def test_prismatic_joint_drive_linear_force_limit(self):
        for i in range(2):
            if i == 0:
    ...
    def test_articulation_prismatic_joint_drive(self):  # OM-85167
    ...
    def test_articulation_revolute_joint_drive_limit_wrapped(self):
        ...
        with ExpectMessage(self, message, expected_result=False):
            self.step()
    ...
    def test_drive_time_sampled_values(self):
        ...
        for joint_type, drive_type, axis, articulation in itertools.product(joint_types, drive_types, axes, articulation):
            with self.subTest(joint_type=joint_type, drive_type=drive_type, axis=axis, articulation=articulation):
                if joint_type == "Revolute":
                    ...
                    self._revolute_test_setup(axis=axis, driveType=drive_type, setupAsArticulation=articulation, targetValue=target)
                elif joint_type == "Prismatic":
                    target = linerar_target
                    self._prismatic_test_setup(axis=axis, driveType=drive_type, setupAsArticulation=articulation, targetValue=target)
                elif joint_type == "D6":
                    target = angular_target
                    self._d6_revolute_test_setup(axis=axis, driveType=drive_type, setupAsArticulation=articulation, targetValue=target)
