...
#
# Ported from omni.physx.tests PhysxContactReportAPI.py for standalone ovruntime testing.
# Only the PhysxContactReportAPITestMemoryStage class is ported (no Kit dependencies).
...
from physicsUtils import (
    add_ground_plane, add_rigid_sphere, add_physics_material_to_prim,
    create_mesh_concave,
)
...
class PhysxContactReportAPITestMemoryStage(PhysicsMemoryStageBaseTestCase):
    ...
    def _on_contact_report_event(self, contact_headers, contact_data):
        for contact_header in contact_headers:
            if contact_header.type == _physx.ContactEventType.CONTACT_FOUND:
                self.contact_found = True
                self.num_contact_found = self.num_contact_found + 1
            elif contact_header.type == _physx.ContactEventType.CONTACT_LOST:
    ...
    def contact_report_found_lost(self):
        self.contact_found = False
        self.contact_lost = False
    ...
    def contact_report_found_lost_point_instancer(self):
        self.contact_found = False
        self.contact_lost = False
    ...
    def contact_report_found_lost_convex_decomposition(self):
        self.stage = self.new_stage()
    ...
    def contact_report_found_lost_sdf(self):
        self.stage = self.new_stage()
    ...
    def contact_report_multi_material(self):
        self.clear_contact_data()
    ...
    def lost_contacts_prim_delete_static(self):
        self.stage = self.new_stage()
    ...
    def lost_contacts_prim_delete_dynamic(self):
        self.stage = self.new_stage()
    ...
    def lost_contacts_prim_change_dynamic_to_static(self):
        self.stage = self.new_stage()
    ...
    def lost_contacts_prim_delete_static_convex_decomposition(self):
        self.stage = self.new_stage()
    ...
    def lost_contacts_prim_delete_dynamic_convex_decomposition(self):
        self.stage = self.new_stage()
    ...
    def lost_contacts_prim_delete_static_sdf(self):
        self.stage = self.new_stage()
    ...
    def lost_contacts_prim_delete_dynamic_sdf(self):
        self.stage = self.new_stage()
    ...
    def contact_kinekine_report_found_lost(self):
        self.clear_contact_data()
    ...
    def contact_kinestatic_report_found_lost(self):
        self.clear_contact_data()
    ...
    def test_physics_contact_report_found_lost_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.contact_report_found_lost()

        simulation_event_sub = None

    def test_physics_contact_report_found_lost_point_instancer_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.contact_report_found_lost_point_instancer()

        simulation_event_sub = None

    def test_physics_contact_report_found_lost_convex_decomposition_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.contact_report_found_lost_convex_decomposition()

        simulation_event_sub = None

    def test_physics_contact_report_found_lost_sdf_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.contact_report_found_lost_sdf()

        simulation_event_sub = None

    def test_physics_contact_report_multi_material_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.contact_report_multi_material()

        simulation_event_sub = None

    def test_physics_lost_contacts_prim_delete_static_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.lost_contacts_prim_delete_static()

        simulation_event_sub = None

    def test_physics_lost_contacts_prim_delete_dynamic_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.lost_contacts_prim_delete_dynamic()

        simulation_event_sub = None

    def test_physics_lost_contacts_prim_change_dynamic_to_static(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.lost_contacts_prim_change_dynamic_to_static()

        simulation_event_sub = None

    def test_physics_lost_contacts_prim_delete_static_convex_decomposition_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.lost_contacts_prim_delete_static_convex_decomposition()

        simulation_event_sub = None

    def test_physics_lost_contacts_prim_delete_dynamic_convex_decomposition_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.lost_contacts_prim_delete_dynamic_convex_decomposition()

        simulation_event_sub = None

    def test_physics_lost_contacts_prim_delete_static_sdf_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.lost_contacts_prim_delete_static_sdf()

        simulation_event_sub = None

    def test_physics_lost_contacts_prim_delete_dynamic_sdf_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.lost_contacts_prim_delete_dynamic_sdf()

        simulation_event_sub = None

    def test_physics_contact_kinekine_report_found_lost_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.contact_kinekine_report_found_lost()

        simulation_event_sub = None

    def test_physics_contact_kinestatic_report_found_lost_batched(self):
        simulation_event_sub = _physx.acquire_physx_simulation_interface().subscribe_contact_report_events(self._on_contact_report_event)

        self.contact_kinestatic_report_found_lost()

        simulation_event_sub = None

    def test_physics_contact_report_found_lost_immediate_api(self):
        self.stage = self.new_stage()
        ...
        for _ in range(50):
            self.step()
            contact_headers, contact_data = _physx.acquire_physx_simulation_interface().get_contact_report()
            if len(contact_headers) == 1:
                if contact_headers[0].type == _physx.ContactEventType.CONTACT_FOUND:
                    self.contact_found = True
                if contact_headers[0].type == _physx.ContactEventType.CONTACT_LOST:
                    self.contact_lost = True
    ...
    def test_physics_contact_report_disable_setting(self):
        self.stage = self.new_stage()

        DISABLE_CONTACT_PROCESSING = _physx.SETTING_DISABLE_CONTACT_PROCESSING

        settingsInterface = carb.settings.acquire_settings_interface()
        disable_contact_proc = settingsInterface.get_as_bool(DISABLE_CONTACT_PROCESSING)
    ...
    def test_physics_contact_report_multiple_scenes(self):
        self.stage = self.new_stage()
    ...
    def test_physics_contact_report_point_instancer_immediate_api(self):
        self.stage = self.new_stage()
    ...
    def test_contact_report_found_lost_runtime_add_remove(self):
        self.contact_found = False
        self.contact_lost = False
