...
#
# Ported from omni.physx.tests PhysxMimicJointAPI.py for standalone ovruntime testing.
...
class PhysxMimicJointAPITestMemoryStage(PhysicsMemoryStageBaseTestCase):
    ...
    def _create_articulation(self, stage: Usd.Stage, path: str, useFixBase: bool = True,
        nbPosIters: int = 4, nbVelIters: int = 1, position: Gf.Vec3f = Gf.Vec3f(0.0)) -> tuple:
    ...
    #
    def test_prismatic_simple(self):
    ...
    #
    def test_spherical_simple(self):
        stage = self.new_stage()
    ...
    def _test_attribute_changes(self, testAttributeType: int) -> None:
        stage = self.new_stage()
    ...
    #
    def test_gearing_change(self):
        self._test_attribute_changes(TEST_GEARING)
    ...
    def test_offset_change(self):
        self._test_attribute_changes(TEST_OFFSET)


    #
    # Ensure that changing the "referenceJoint" attribute on a mimic joint has the expected effect
    #
    def test_reference_joint_change(self):
        self._test_attribute_changes(TEST_REFERENCE_JOINT)
    ...
    # works as expected
    #
    def test_remove_api_or_delete(self):
        REMOVE_API = 0
        DELETE_PRIM = 1
    ...
    #
    def test_simple_arm_with_gravity(self):
    ...
    def _create_chain_scenario(self, stage: Usd.Stage, rootPath: str, linkPosShift: Gf.Vec3f = Gf.Vec3f(0.0),
        ...
        gearing1: float = 0.5, gearing2: float = 4.0 / 5.0) -> tuple:
    ...
    def run_mimic_compliance_correctness_test(self, axis):
    ...
    def test_mimic_compliance_correctness_rotx(self):
        self.run_mimic_compliance_correctness_test(0)

    def test_mimic_compliance_correctness_roty(self):
        self.run_mimic_compliance_correctness_test(1)

    def test_mimic_compliance_correctness_rotz(self):
        self.run_mimic_compliance_correctness_test(2)
    ...
    def _test_error_messages(self, testMessageScenario: int) -> None:
    ...
    #
    def test_message_exclude_from_articulation_target_joint(self):
        self._test_error_messages(TEST_MSG_EXCLUDE_FROM_ARTICULATION_TARGET)


    #
    # Ensure that a mimic joint on an non-articulation joint triggers an error
    #
    def test_message_exclude_from_articulation_reference_joint(self):
        self._test_error_messages(TEST_MSG_EXCLUDE_FROM_ARTICULATION_REFERENCE)


    #
    # Ensure that an unsupported token for the reference joint axis triggers an error
    #
    def test_message_reference_joint_axis(self):
        self._test_error_messages(TEST_MSG_REFERENCE_JOINT_AXIS)


    #
    # Ensure that more than one mimic joint on a prismatic triggers an error
    #
    def test_message_multiple_mimic_on_1dof_joint(self):
        self._test_error_messages(TEST_MSG_MULTIPLE_MIMIC_ON_1DOF_JOINT)


    #
    # Ensure that defining no reference joint triggers an error
    #
    def test_message_no_reference_joint(self):
        self._test_error_messages(TEST_MSG_NO_REFERENCE_JOINT)


    #
    # Ensure that defining multiple reference joints triggers an error
    #
    def test_message_multiple_reference_joints(self):
        self._test_error_messages(TEST_MSG_MULTIPLE_REFERENCE_JOINTS)


    #
    # Ensure that defining an invalid path for the reference joint triggers an error
    #
    def test_message_invalid_reference_joint_path(self):
        self._test_error_messages(TEST_MSG_INVALID_REFERENCE_JOINT_PATH)


    #
    # Ensure that defining a non-joint prim for the reference joint triggers an error
    #
    def test_message_invalid_reference_joint_prim_type(self):
        self._test_error_messages(TEST_MSG_INVALID_REFERENCE_JOINT_PRIM_TYPE)


    #
    # Ensure that defining a wrong joint type for the reference joint triggers an error
    #
    def test_message_wrong_joint_type(self):
        self._test_error_messages(TEST_MSG_WRONG_JOINT_TYPE)


    #
    # Ensure that any non-locked linear degree of freedom on a D6 triggers an error
    #
    def test_message_not_all_linear_dofs_locked(self):
        self._test_error_messages(TEST_MSG_NOT_ALL_LINEAR_DOFS_LOCKED)


    #
    # Ensure that a locked reference axis triggers an error
    #
    def test_message_reference_axis_locked(self):
        self._test_error_messages(TEST_MSG_REFERENCE_AXIS_LOCKED)


    #
    # Ensure that target and reference joint on different articulations triggers an error
    #
    def test_message_different_articulations(self):
        self._test_error_messages(TEST_MSG_DIFFERENT_ARTICULATIONS)


    def _test_error_messages_spherical(self, testMessageScenario: int) -> None:
        stage = self.new_stage()
    ...
    #
    def test_message_same_axis_on_spherical(self):
        self._test_error_messages_spherical(TEST_MSG_SAME_AXIS)


    def _test_error_messages_revolute(self, testMessageScenario: int) -> None:
    ...
    #
    def test_message_wrapped_joint_limit_target(self):
        self._test_error_messages_revolute(TEST_MSG_WRAPPED_JOINT_LIMIT_TARGET)


    #
    # Ensure that reference joint with wrapped limits triggers an error
    #
    def test_message_wrapped_joint_limit_reference(self):
        self._test_error_messages_revolute(TEST_MSG_WRAPPED_JOINT_LIMIT_REFERENCE)


    #
    # Ensure the replicator logic works for articulations with mimic joints
    #
    def test_mimic_joint_replication(self):
        stage = self.new_stage()
        ...
        _physx.acquire_physx_replicator_interface().replicate(stageId, rootPath + "/env0", (replicationCount - 1))
