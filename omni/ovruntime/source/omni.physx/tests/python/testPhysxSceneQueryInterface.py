...
#
# Ported from omni.physx.tests PhysxSceneQueryInterface.py for standalone ovruntime testing.
...
class PhysxSceneQueryInterfaceTestMemoryStage(PhysicsMemoryStageBaseTestCase):
    ...
    def test_physics_raycast_closest(self):
        ...
        hitInfo = _physx.acquire_physx_scene_query_interface().raycast_closest(Gf.Vec3f(0.0, 200.0, 0.0), Gf.Vec3f(0.0, -1.0, 0.0), 1000.0)
        if hitInfo["hit"]:
    ...
    def test_physics_raycast_closest_point_instancer(self):
        stage = self.new_stage()
    ...
    def test_physics_raycast_any(self):
        stage = self.new_stage()
        ...
        hitInfo = _physx.acquire_physx_scene_query_interface().raycast_any(Gf.Vec3f(0.0, 200.0, 0.0), Gf.Vec3f(0.0, -1.0, 0.0), 1000.0)
        if hitInfo:
            print("Hit found.")
        else:
            print("Hit not found.")
    ...
    def test_physics_raycast_all(self):
        stage = self.new_stage()
        ...
        numHits = _physx.acquire_physx_scene_query_interface().raycast_all(Gf.Vec3f(0.0, 200.0, 0.0), Gf.Vec3f(0.0, -1.0, 0.0), 1000.0, report_all_hits)
    ...
    def test_physics_raycast_closest_multi_material(self):
        stage = self.new_stage()
        ...
        hitInfo = _physx.acquire_physx_scene_query_interface().raycast_closest(Gf.Vec3f(200.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0)
        if hitInfo["hit"]:
            hitPos = Gf.Vec3f(hitInfo["position"].x, hitInfo["position"].y, hitInfo["position"].z)
            hitNormal = Gf.Vec3f(hitInfo["normal"].x, hitInfo["normal"].y, hitInfo["normal"].z)
            hitCollision = hitInfo["collision"]
        ...
        hitInfo = _physx.acquire_physx_scene_query_interface().raycast_closest(Gf.Vec3f(100.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0)
        if hitInfo["hit"]:
            hitPos = Gf.Vec3f(hitInfo["position"].x, hitInfo["position"].y, hitInfo["position"].z)
            hitNormal = Gf.Vec3f(hitInfo["normal"].x, hitInfo["normal"].y, hitInfo["normal"].z)
            hitCollision = hitInfo["collision"]
        ...
        hitInfo = _physx.acquire_physx_scene_query_interface().raycast_closest(Gf.Vec3f(300.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0)
        if hitInfo["hit"]:
            hitPos = Gf.Vec3f(hitInfo["position"].x, hitInfo["position"].y, hitInfo["position"].z)
            hitNormal = Gf.Vec3f(hitInfo["normal"].x, hitInfo["normal"].y, hitInfo["normal"].z)
            hitCollision = hitInfo["collision"]
    ...
    def test_physics_raycast_closest_multi_material_default(self):
        stage = self.new_stage()
    ...
    def test_physics_raycast_all_multi_material(self):
        stage = self.new_stage()
        ...
        self.faceIndex = 2
        numHits = _physx.acquire_physx_scene_query_interface().raycast_all(Gf.Vec3f(200.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0, report_all_hits)
        self.assertTrue(numHits == 1)

        self.material = self.defaultPrimPath + "/material1"
        self.faceIndex = 1
        numHits = _physx.acquire_physx_scene_query_interface().raycast_all(Gf.Vec3f(100.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0, report_all_hits)
        self.assertTrue(numHits == 1)

        self.material = self.defaultPrimPath + "/material1"
        self.faceIndex = 3
        numHits = _physx.acquire_physx_scene_query_interface().raycast_all(Gf.Vec3f(300.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0, report_all_hits)
        self.assertTrue(numHits == 1)

    def test_physics_raycast_all_multi_material_default(self):
        stage = self.new_stage()
    ...
    def test_physics_sweep_sphere_closest(self):
        stage = self.new_stage()

        self.setup_stage(stage)

        self.eval_sweep_test_closest(
            _physx.acquire_physx_scene_query_interface().sweep_sphere_closest(10.0, Gf.Vec3f(0.0, 200.0, 0.0), Gf.Vec3f(0.0, -1.0, 0.0), 1000.0))

    def test_physics_sweep_box_closest(self):
        stage = self.new_stage()

        self.setup_stage(stage)

        self.eval_sweep_test_closest(
            _physx.acquire_physx_scene_query_interface().sweep_box_closest(Gf.Vec3f(50.0, 50.0, 50.0), Gf.Vec3f(0.0, 500.0, 0.0),
                                                                      carb.Float4( -0.364705, 0.1159176, -0.2798498, 0.8804757), Gf.Vec3f(0.0, -1.0, 0.0), 1000.0))

    def test_physics_sweep_mesh_closest(self):
        stage = self.new_stage()
        ...
        self.eval_sweep_test_closest(
            _physx.acquire_physx_scene_query_interface().sweep_mesh_closest(path_tuple[0], path_tuple[1], Gf.Vec3f(0.0, -1.0, 0.0), 1000.0))

    def test_physics_sweep_shape_closest(self):
        for geometry in self.geoms_list:
            ...
            self.eval_sweep_test_closest(
                _physx.acquire_physx_scene_query_interface().sweep_shape_closest(path_tuple[0], path_tuple[1], Gf.Vec3f(0.0, -1.0, 0.0), 1000.0),
                desired_position=position)

    def test_physics_sweep_sphere_closest_point_instancer(self):
        stage = self.new_stage()
    ...
    def test_physics_sweep_box_closest_point_instancer(self):
        stage = self.new_stage()
    ...
    def test_physics_sweep_mesh_closest_point_instancer(self):
        stage = self.new_stage()
    ...
    def test_physics_sweep_shape_closest_point_instancer(self):
        for geometry in self.geoms_list:
            print(f"Geometry type: {geometry}")
            stage = self.new_stage()
            self.setup_stage(stage, "point_instancer")
    ...
    def test_physics_sweep_sphere_any(self):
        stage = self.new_stage()

        self.setup_stage(stage)

        if _physx.acquire_physx_scene_query_interface().sweep_sphere_any(10.0, Gf.Vec3f(0.0, 200.0, 0.0), Gf.Vec3f(0.0, -1.0, 0.0), 1000.0):
            print("Hit found.")
        else:
            print("Hit not found.")
            self.assertTrue(False)

    def test_physics_sweep_box_any(self):
        stage = self.new_stage()

        self.setup_stage(stage)

        if (_physx.acquire_physx_scene_query_interface().sweep_box_any(Gf.Vec3f(50.0, 50.0, 50.0), Gf.Vec3f(0.0, 500.0, 0.0),
                                                                      carb.Float4( -0.364705, 0.1159176, -0.2798498, 0.8804757), Gf.Vec3f(0.0, -1.0, 0.0), 1000.0)):
            print("Hit found.")
        else:
            print("Hit not found.")
    ...
    def test_physics_sweep_mesh_any(self):
        stage = self.new_stage()
        ...
        if _physx.acquire_physx_scene_query_interface().sweep_mesh_any(path_tuple[0], path_tuple[1], Gf.Vec3f(0.0, -1.0, 0.0), 1000.0):
            print("Hit found.")
        else:
            print("Hit not found.")
            self.assertTrue(False)

    def test_physics_sweep_shape_any(self):
        for geometry in self.geoms_list:
            print(f"Geometry type: {geometry}")
            stage = self.new_stage()
            self.setup_stage(stage)
            ...
            if _physx.acquire_physx_scene_query_interface().sweep_shape_any(path_tuple[0], path_tuple[1], Gf.Vec3f(0.0, -1.0, 0.0), 1000.0):
                print("Hit found.")
            else:
                print("Hit not found.")
                self.assertTrue(False)
    ...
    def test_physics_sweep_sphere_all(self):
        stage = self.new_stage()
        ...
        self.eval_sweep_test_all(
            _physx.acquire_physx_scene_query_interface().sweep_sphere_all(10.0, Gf.Vec3f(0.0, 200.0, 0.0), Gf.Vec3f(0.0, -1.0, 0.0), 1000.0, self.report_all_hits))

    def test_physics_sweep_box_all(self):
        stage = self.new_stage()
        ...
        self.eval_sweep_test_all(
            _physx.acquire_physx_scene_query_interface().sweep_box_all(Gf.Vec3f(50.0, 50.0, 50.0), Gf.Vec3f(0.0, 500.0, 0.0),
                                                                      carb.Float4( -0.364705, 0.1159176, -0.2798498, 0.8804757), Gf.Vec3f(0.0, -1.0, 0.0),
                                                                      1000.0, self.report_all_hits))

    def test_physics_sweep_mesh_all(self):
        stage = self.new_stage()
        ...
        self.eval_sweep_test_all(
            _physx.acquire_physx_scene_query_interface().sweep_mesh_all(path_tuple[0], path_tuple[1], Gf.Vec3f(0.0, -1.0, 0.0), 1000.0, self.report_all_hits))

    def test_physics_sweep_shape_all(self):
        for geometry in self.geoms_list:
            print(f"Geometry type: {geometry}")
            stage = self.new_stage()
            self.setup_stage(stage)
            ...
            self.eval_sweep_test_all(
                _physx.acquire_physx_scene_query_interface().sweep_shape_all(path_tuple[0], path_tuple[1], Gf.Vec3f(0.0, -1.0, 0.0), 1000.0, self.report_all_hits))


    def test_physics_sweep_closest_multi_material(self):
        stage = self.new_stage()
        ...
        hitInfo = _physx.acquire_physx_scene_query_interface().sweep_sphere_closest(10.0, Gf.Vec3f(200.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0)
        if hitInfo["hit"]:
            hitPos = Gf.Vec3f(hitInfo["position"].x, hitInfo["position"].y, hitInfo["position"].z)
            hitNormal = Gf.Vec3f(hitInfo["normal"].x, hitInfo["normal"].y, hitInfo["normal"].z)
            hitCollision = hitInfo["collision"]
        ...
        hitInfo = _physx.acquire_physx_scene_query_interface().sweep_sphere_closest(10.0, Gf.Vec3f(100.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0)
        if hitInfo["hit"]:
            hitPos = Gf.Vec3f(hitInfo["position"].x, hitInfo["position"].y, hitInfo["position"].z)
            hitNormal = Gf.Vec3f(hitInfo["normal"].x, hitInfo["normal"].y, hitInfo["normal"].z)
            hitCollision = hitInfo["collision"]
        ...
        hitInfo = _physx.acquire_physx_scene_query_interface().sweep_sphere_closest(10.0, Gf.Vec3f(300.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0)
        if hitInfo["hit"]:
            hitPos = Gf.Vec3f(hitInfo["position"].x, hitInfo["position"].y, hitInfo["position"].z)
            hitNormal = Gf.Vec3f(hitInfo["normal"].x, hitInfo["normal"].y, hitInfo["normal"].z)
            hitCollision = hitInfo["collision"]
    ...
    def test_physics_sweep_closest_multi_material_default(self):
        stage = self.new_stage()
    ...
    def test_physics_sweep_all_multi_material(self):
        stage = self.new_stage()
        ...
        self.material = self.defaultPrimPath + "/material0"
        self.faceIndex = 2
        numHits = _physx.acquire_physx_scene_query_interface().sweep_sphere_all(10.0, Gf.Vec3f(200.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0, report_all_hits)
        self.assertTrue(numHits == 1)

        self.material = self.defaultPrimPath + "/material1"
        self.faceIndex = 1
        numHits = _physx.acquire_physx_scene_query_interface().sweep_sphere_all(10.0, Gf.Vec3f(100.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0, report_all_hits)
        self.assertTrue(numHits == 1)

        self.material = self.defaultPrimPath + "/material1"
        self.faceIndex = 3
        numHits = _physx.acquire_physx_scene_query_interface().sweep_sphere_all(10.0, Gf.Vec3f(300.0, 0.0, 100.0), Gf.Vec3f(0.0, 0.0, -1.0), 1000.0, report_all_hits)
        self.assertTrue(numHits == 1)

    def test_physics_sweep_all_multi_material_default(self):
        stage = self.new_stage()
    ...
    def test_physics_overlap_box_any(self):
        stage = self.new_stage()

        self.setup_stage(stage)

        hitInfo = _physx.acquire_physx_scene_query_interface().overlap_box_any(Gf.Vec3f(100.0), Gf.Vec3f(0.0),  carb.Float4(0.0, 0.0, 0.0, 1.0))
        if hitInfo:
            print("Hit found.")
        else:
            print("Hit not found.")
    ...
    def test_physics_overlap_box(self):
        stage = self.new_stage()

        self.setup_stage(stage)

        hitInfo = _physx.acquire_physx_scene_query_interface().overlap_box(Gf.Vec3f(100.0), Gf.Vec3f(0.0),  carb.Float4(0.0, 0.0, 0.0, 1.0), None, True)
        if hitInfo:
            print("Hit found.")
        else:
            print("Hit not found.")
        ...
        numHits = _physx.acquire_physx_scene_query_interface().overlap_box(Gf.Vec3f(1000.0), Gf.Vec3f(0.0),  carb.Float4(0.0, 0.0, 0.0, 1.0), report_all_hits)
    ...
    def test_physics_overlap_box_point_instancer(self):
        stage = self.new_stage()
    ...
    def test_physics_overlap_sphere_any(self):
        stage = self.new_stage()

        self.setup_stage(stage)

        hitInfo = _physx.acquire_physx_scene_query_interface().overlap_sphere_any(100.0, Gf.Vec3f(0.0))
        if hitInfo:
            print("Hit found.")
        else:
            print("Hit not found.")
    ...
    def test_physics_overlap_sphere(self):
        ...
        hitInfo = _physx.acquire_physx_scene_query_interface().overlap_sphere(100.0, Gf.Vec3f(0.0), None, True)
        if hitInfo:
            print("Hit found.")
        else:
            print("Hit not found.")
        ...
        numHits = _physx.acquire_physx_scene_query_interface().overlap_sphere(1000.0, Gf.Vec3f(0.0), report_all_hits)
        if numHits == 2 and self.numHits == 2:
            print("Hits found.")
        else:
            print("Hits not found. Num hits: " + str(self.numHits))
    ...
    def test_physics_overlap_mesh_any(self):
        stage = self.new_stage()
        ...
        hitInfo = _physx.acquire_physx_scene_query_interface().overlap_mesh_any(path_tuple[0], path_tuple[1])
    ...
    def test_physics_overlap_mesh(self):
        stage = self.new_stage()
        ...
        path_tuple = PhysicsSchemaTools.encodeSdfPath(Sdf.Path(self.defaultPrimPath + '/mesh'))
        hitInfo = _physx.acquire_physx_scene_query_interface().overlap_mesh(path_tuple[0], path_tuple[1], None, True)
        ...
        numHits = _physx.acquire_physx_scene_query_interface().overlap_mesh(path_tuple[0], path_tuple[1], report_all_hits)
        if numHits == 2 and self.numHits == 2:
            print("Hits found.")
        else:
            print("Hits not found. Num hits: " + str(self.numHits))
    ...
    def test_physics_overlap_shape_any(self):
        geoms_list = ["sphere", "cube", "capsule", "cylinder", "cone", "mesh"]
        for geometry in geoms_list:
            ...
            hitInfo = _physx.acquire_physx_scene_query_interface().overlap_shape_any(path_tuple[0], path_tuple[1])
    ...
    def test_physics_overlap_shape(self):
        geoms_list = ["sphere", "cube", "capsule", "cylinder", "cone", "mesh"]
        for geometry in geoms_list:
            stage = self.new_stage()
            self.setup_stage(stage)
            ...
            hitInfo = _physx.acquire_physx_scene_query_interface().overlap_shape(path_tuple[0], path_tuple[1], None, True)
            ...
            numHits = _physx.acquire_physx_scene_query_interface().overlap_shape(path_tuple[0], path_tuple[1], report_all_hits)
            if numHits == 2 and self.numHits == 2:
                print("Hits found.")
            else:
    ...
    def test_physics_material_face_cleanup(self):
        stage = self.new_stage()

        self.setup_stage(stage, "mesh_cleanup")

        hitInfo = _physx.acquire_physx_scene_query_interface().raycast_closest(Gf.Vec3f(-1.0, 0.0, 10.0), Gf.Vec3f(0.0, 0.0, -1.0), 100.0)
        if hitInfo["hit"]:
            self.assertTrue(hitInfo["faceIndex"] == 3)
        else:
            print("Hit not found.")
    ...
    def test_overlap_mesh_multiple(self):
        # Create a stage
        stage = self.new_stage()
        ...
        query_interface = _physx.acquire_physx_scene_query_interface()
