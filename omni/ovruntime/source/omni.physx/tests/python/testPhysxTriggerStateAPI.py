...
#
# Ported from omni.physx.tests PhysxTriggerStateAPI.py for standalone ovruntime testing.
# Only the PhysxTriggerStateAPITestMemoryStage class is ported (no Kit dependencies).
...
import physicsUtils
from pxr import Usd, Gf, Sdf, UsdGeom, UsdPhysics, UsdUtils, PhysxSchema, PhysicsSchemaTools
import _physx


class PhysxTriggerStateAPITestMemoryStage(PhysicsMemoryStageBaseTestCase):
    ...
    def test_physics_trigger_enter_leave(self):
    ...
    def test_physics_trigger_collision_filter(self):
        stage = self.new_stage()
        self.setup_stage(stage)
    ...
    def test_physics_trigger_collision_groups(self):
        stage = self.new_stage()
        self.setup_stage(stage)
    ...
    def test_physics_trigger_convex_decomposition_enter_leave(self):
        stage = self.new_stage()
    ...
    def test_physics_trigger_convex_decomposition_both_enter_leave(self):
        stage = self.new_stage()
    ...
    def test_physics_trigger_api_clear(self):
        stage = self.new_stage()
        self.setup_stage(stage)
    ...
    def test_delete_trigger_with_box_in_trigger_during_runtime(self):
        stage = self.new_stage()
    ...
    def test_delete_box_in_trigger_during_runtime(self):
        stage = self.new_stage()
    ...
    def test_physics_trigger_enter_leave_script(self):
        ...
        simulateI = _physx.acquire_physx_simulation_interface()
        self.got_called_enter = False
        self.got_called_exit = False

        def report_trigger(trigger_data):
            ...
            if trigger_data.event_type == _physx.TriggerEventType.TRIGGER_ON_ENTER:
                self.got_called_enter = True
                self.assertEqual(self.got_called_exit, False)  # check for exit not to be already called
            if trigger_data.event_type == _physx.TriggerEventType.TRIGGER_ON_LEAVE:
    ...
    def test_physics_trigger_enter_leave_script_all(self):
        # Same as previous test but without specifying a path / stage in subscribe_physics_trigger_report_events
