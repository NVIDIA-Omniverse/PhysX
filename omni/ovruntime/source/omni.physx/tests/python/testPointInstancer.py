...
#
# Ported from omni.physx.tests PointInstancer.py for standalone ovruntime testing.
...
class PointInstancerTestMemoryStage(PhysicsMemoryStageBaseTestCase):
    ...
    # OM-26384
    def test_physics_point_instancer_static(self):
        stage = self.new_stage()
        self.base_setup(stage, config="static")

        self.step()

        check_stats(self, {"numBoxShapes": 2, "numStaticRigids": 2})
        ...
        check_stats(self, {"numBoxShapes": 2, "numDynamicRigids": 2})

    # OM-26384
    def test_physics_point_instancer_mixed_dynamic_static(self):
        stage = self.new_stage()
        ...
        check_stats(self, {"numBoxShapes": 2, "numStaticRigids": 1, "numDynamicRigids": 1})

    def test_physics_point_instancer_mixed_dynamic_kinematic(self):
        stage = self.new_stage()
    ...
    def test_physics_point_instancer_deactivation(self):
        stage = self.new_stage()
        ...
        check_stats(self, {"numBoxShapes": 3, "numDynamicRigids": 3})
    ...
    def test_physics_point_instancer_collisions(self):
        stage = self.new_stage()
        self.base_setup(stage, config="static")
        ...
        check_stats(self, {"numBoxShapes": 4, "numDynamicRigids": 1})

        self.instancer.DeactivateId(3)

        self.step()

        check_stats(self, {"numBoxShapes": 3, "numDynamicRigids": 1})
    ...
    def test_physics_point_instancer_initial_velocities(self):
        stage = self.new_stage()
        ...
        _physx.acquire_physx_interface().start_simulation()
        self.step()

        check_stats(self, {"numBoxShapes": 4, "numDynamicRigids": 4})
    ...
    def _run_reset_transform_test(self, config, use_kinematic_static=False):
        stage = self.new_stage()
    ...
    def test_physics_point_instancer_reset_transformation_dynamic(self):
        self._run_reset_transform_test(config="dynamic")
        # pointinstancer will reset nondynamics, so the following two test configs
        # are currently not supported:
        # self._run_reset_transform_test(config="mixed")
        # self._run_reset_transform_test(config="mixed", use_kinematic_static=True)

    def test_physics_point_instancer_invalid_index(self):
        stage = self.new_stage()
    ...
    # OM-42086
    def test_physics_point_instancer_hierarchy_transformation(self):
        stage = self.new_stage()
    ...
    # OM-43430
    def test_physics_point_instancer_prototype_outside(self):
        stage = self.new_stage()
    ...
    @unittest.skip("Data file point_instancer_blast.usda not accessible from ovruntime test environment")
    def test_physics_point_instancer_prototype_scene_instanced(self):
        pass

    # OM-48268
    def test_physics_point_instancer_prototype_added_test_arrays(self):
        stage = self.new_stage()

        self.base_setup(stage, config="dynamic")

        # run the simulation for a bit then reset it
        self.step(100, 1.0 / 60.0, reset_simulation_after=True)
        ...
        check_stats(self, {"numBoxShapes": 3})

    def test_physics_point_instancer_time_sampled_values(self):
        stage = self.new_stage()
    ...
    # OM-75944
    def test_physics_point_instancer_colliders_without_rb(self):
