...
#
# Ported from omni.physx.tests Utils.py for standalone ovruntime testing.
# Only the UtilsTestMemoryStage class is ported (no Kit dependencies).
...
import physicsUtils as utils
from pxr import Gf, UsdGeom, UsdPhysics
import carb


class UtilsTestMemoryStage(PhysicsMemoryStageBaseTestCase):
    category = TestCategory.Core

    def _assert_transform_close(self, transform, reference_transform, places=2):
    ...
    def _getNewXform(self):
        return UsdGeom.Xform.Define(self._stage, utils.get_stage_next_free_path(self._stage, "/World/xform", False))

    def test_physics_utils_set_or_add_orient_op_to_xformable(self):
        self.new_stage()
    ...
    def test_physics_utils_set_or_add_scale_op_to_xformable(self):
        self.new_stage()
        # test setting with all possible precisions:
        precisions = [UsdGeom.XformOp.PrecisionDouble, UsdGeom.XformOp.PrecisionFloat, UsdGeom.XformOp.PrecisionHalf]
    ...
    def test_physics_utils_set_or_add_transform_op_to_xformable(self):
        self.new_stage()
        # test setting with all possible precisions:
        precisions = [UsdGeom.XformOp.PrecisionDouble, UsdGeom.XformOp.PrecisionFloat, UsdGeom.XformOp.PrecisionHalf]
        initVals = [Gf.Vec3d(1, 1, 1), Gf.Vec3f(1, 1, 1), Gf.Vec3h(1, 1, 1)]
    ...
    def test_physics_utils_set_or_add_scale_orient_translate_ops_to_xformable(self):
        self.new_stage()
        # test setting with all possible precisions:
        precisions = [UsdGeom.XformOp.PrecisionDouble, UsdGeom.XformOp.PrecisionFloat, UsdGeom.XformOp.PrecisionHalf]
    ...
    def test_physics_utils_setup_xform_as_scale_orient_translate(self):
        self.new_stage()
    ...
    def test_internal_utils_xform_copy_as_scale_orient_translate(self):
        self.new_stage()
        xformSrc = self._get_xform_with_scale_rotxyz_translate()
        xformSrc.SetResetXformStack(True)
    ...
    def test_internal_utils_expect_message(self):
