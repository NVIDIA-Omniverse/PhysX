...
OMNI_APP_GLOBALS("test.benchmarks", "Omniverse physics benchmark tests.")
...
void harnessImpl(int argc, char** argv, PrintfCbFunc printCb)
{
    ...
    std::vector<BmRegistrable*> reg;
    bmGetRegister(reg, cmdLine.filterString, cmdLine.runHidden);
    ...
    // customize test name with global thread and gpu overwrites
    std::string postfix("");
...
#define GETTIME(r, s) runs[r + s * runCount]
            ...
            record.stdDev[s] = (uint64_t)sqrt((double)variance);
        }
