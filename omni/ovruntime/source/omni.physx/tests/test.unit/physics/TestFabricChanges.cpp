    ...
    "[component=OmniPhysics][owner=aborovicka][priority=mandatory]")
{
    ...
    StageReaderWriter stageRW = fabricTemplate.iStageReaderWriter->get(fabricTemplate.mStageId);
    {
        ...
        for (size_t i = 0; i != bucketCount; i++)
        {
            ...
            auto mtx = stageRW.getAttributeArrayWr<PXR_NS::GfMatrix4d>(primBuckets, i, localMatrixToken);
    ...
    SUBCASE("Transformation Change")
    {
        ...
        for (size_t i = 0; i != bucketCount; i++)
        {
            ...
            gsl::span<PXR_NS::GfMatrix4d> matrices = stageRW.getAttributeArray<PXR_NS::GfMatrix4d>(primBuckets, i, localMatrixToken);
    ...
    SUBCASE("Transformation Change Indices")
    {
        ...
        for (size_t j = 0; j < bodiesPaths.size(); j++)
        {
            if (j % 10 == 0)
            {
                PXR_NS::GfMatrix4d& matrices = *(PXR_NS::GfMatrix4d*)(stageRW.getAttributeWr<PXR_NS::GfMatrix4d>(omni::fabric::convertToPathType<omni::fabric::Path>(stageRW.getFabricId(), bodiesPaths[j]), localMatrixToken));
        ...
        for (size_t j = 0; j < bodiesPaths.size(); j++)
        {
            const PXR_NS::SdfPath primPath = bodiesPaths[j];
            // get the dynamic actor
            PxBase* basePtr = reinterpret_cast<PxBase*>(physx->getPhysXPtr(primPath, ePTActor));
            CHECK(basePtr != nullptr);
            CHECK(basePtr->is<PxRigidDynamic>());
