...

{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        PXR_NS::UsdGeomMesh concaveMesh = createConcaveMesh(stage, concaveMeshPath, 100.0f);
...
void createFixedTendons(const std::string& envPath, int index, UsdStageWeakPtr stage, const TfToken tendonName)
{
    ...
    PXR_NS::PhysxSchemaPhysxTendonAxisRootAPI rootApi = PXR_NS::PhysxSchemaPhysxTendonAxisRootAPI::Apply(fixedJointPrim, tendonName);
    
    UsdPrim prismaticJointPrim = stage->GetPrimAtPath(prismaticJointPath);

    PXR_NS::PhysxSchemaPhysxTendonAxisAPI axisApi = PXR_NS::PhysxSchemaPhysxTendonAxisAPI::Apply(prismaticJointPrim, tendonName);
}
...
void createSpatialTendons(const std::string& envPath, int index, UsdStageWeakPtr stage)
{
    ...
    PXR_NS::PhysxSchemaPhysxTendonAttachmentRootAPI rootApi =
        PXR_NS::PhysxSchemaPhysxTendonAttachmentRootAPI::Apply(dynamicLinkPrim, TfToken("root"));
    
    PXR_NS::PhysxSchemaPhysxTendonAttachmentLeafAPI leafApi =
        PXR_NS::PhysxSchemaPhysxTendonAttachmentLeafAPI::Apply(dynamicLinkPrim0, TfToken("leaf"));
    
    PXR_NS::PhysxSchemaPhysxTendonAttachmentAPI(leafApi, TfToken("leaf")).CreateParentAttachmentAttr().Set(TfToken("root"));
    PXR_NS::PhysxSchemaPhysxTendonAttachmentAPI(leafApi, TfToken("leaf")).CreateParentLinkRel().AddTarget(dynamicLinkPath);
}
