...
TEST_CASE_TEMPLATE("RigidBody Tests", T, USDChange, FabricChange)
{
    ...
    REQUIRE(PXR_NS::UsdGeomSetStageMetersPerUnit(stage, static_cast<double>(metersPerStageUnit)));
    ...
    SUBCASE("RigidBody Enabled")
    {
        ...
        callback.objectCreationNotifyFn = [](const PXR_NS::SdfPath& sdfPath, usdparser::ObjectId objectId, PhysXType type, void* userData)
        {
        ...
        callback.objectDestructionNotifyFn = [](const PXR_NS::SdfPath& sdfPath, usdparser::ObjectId objectId, PhysXType type, void* userData)
        {
            std::vector<ObjectNotficationInfo>* objectInfo = (std::vector<ObjectNotficationInfo>*)userData;
...
public:
    ...
    PxTransform getPhysXTransformation(PxRigidBody* rbo, FabricChange& fabricChange, const PXR_NS::SdfPath& primPath)
    {
        return rbo->getGlobalPose();
    }

    PXR_NS::GfTransform getTransformation(FabricChange& fabricChange, const PXR_NS::SdfPath& primPath)
    {
    ...
    PXR_NS::GfTransform getLocalTransformation(FabricChange& fabricChange, const PXR_NS::SdfPath& primPath)
    {
...
public:
    ...
    PXR_NS::GfTransform getTransformation(FabricChange& fabricChange, const PXR_NS::SdfPath& primPath)
    {
        PXR_NS::GfMatrix4d& valData =
            *(PXR_NS::GfMatrix4d*)(fabricChange.iStageReaderWriter->getAttributeRd(
                                    fabricChange.mSrwId,
...
public:
    ...
    {
        PXR_NS::GfVec3d& p =
            *(PXR_NS::GfVec3d*)(fabricChange.iStageReaderWriter->getAttributeRd(
                                 fabricChange.mSrwId,
                                 omni::fabric::convertToPathType<omni::fabric::Path>(
                                     fabricChange.iStageReaderWriter->getFabricId(fabricChange.mSrwId), primPath),
...
TEST_CASE_TEMPLATE("RigidBody Kinematic Switch", T, UsdTemplate, FabricTemplate)
{
    ...
    {
        const PXR_NS::GfTransform transform = fabricChangeTemplate.getTransformation(boxPath);
        const PXR_NS::GfVec3d position = transform.GetTranslation();
