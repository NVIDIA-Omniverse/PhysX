...
//-----------------------------------------------------------------------------
// Regression: PhysXStepper::waitForCompletion must not hold mCompleteLock while
// firing eSimulationComplete subscribers. Debug-visualization queries called
// from such a subscriber call back into waitForSimulationCompletion(false) for
// the same scene; pre-fix that re-entry tripped the carb::tasking::Mutex
// "Recursion not allowed" assertion and aborted the process.

namespace
{
struct ReentrantCompletionTestData
{
    IPhysxVisualization* iVisualization{ nullptr };
    int numCompletes{ 0 };
    int numReentrantGetNbTriangles{ 0 };
};

void onReentrantCompletionFn(SimulationStatusEvent eventStatus, void* userData)
{
    if (eventStatus != SimulationStatusEvent::eSimulationComplete)
        return;

    auto* td = static_cast<ReentrantCompletionTestData*>(userData);
    td->numCompletes++;
    if (td->iVisualization)
    {
        // getNbTriangles() calls waitForSimulationCompletion(false) internally,
        // which re-enters PhysXStepper::waitForCompletion on the very scene
        // whose outer waitForCompletion is firing this event.
        (void)td->iVisualization->getNbTriangles();
        td->numReentrantGetNbTriangles++;
    }
}
}

TEST_CASE("PhysXStepper Re-entrant waitForCompletion From eSimulationComplete",
          "[omniphysics]"
          "[component=OmniPhysics][owner=aborovicka][priority=mandatory]")
{
    PhysicsTest& physicsTests = *PhysicsTest::getPhysicsTests();
    ...
    IPhysxSimulation* physxSim = physicsTests.acquirePhysxSimulationInterface();
    REQUIRE(physxSim);
    IPhysxVisualization* iVis =
        physicsTests.getApp()->getFramework()->acquireInterface<IPhysxVisualization>();
    REQUIRE(iVis);
    ...
    const SdfPath defaultPrimPath = SdfPath("/World");
    stage->SetDefaultPrim(stage->DefinePrim(defaultPrimPath));
    ...
    const SdfPath physicsScenePath = defaultPrimPath.AppendChild(TfToken("physicsScene"));
    UsdPhysicsScene::Define(stage, physicsScenePath);
    PhysxSchemaPhysxSceneAPI::Apply(stage->GetPrimAtPath(physicsScenePath));

    physxSim->attachStage(stageId);

    ReentrantCompletionTestData td;
    td.iVisualization = iVis;
    SubscriptionId subId = physx->subscribePhysicsSimulationEvents(onReentrantCompletionFn, &td);

    // physxSim->simulate() forces async stepping, so PhysXStepper fires
    // eSimulationComplete from inside its locked region.
    physxSim->simulate(0.01f, 0.0f);
    physxSim->fetchResults();

    CHECK(td.numCompletes == 1);
    CHECK(td.numReentrantGetNbTriangles == 1);

    physx->unsubscribePhysicsSimulationEvents(subId);
