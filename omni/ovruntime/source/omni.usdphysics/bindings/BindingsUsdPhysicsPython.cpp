...
CARB_BINDINGS("omni.usdphysics.python")
...
const carb::Float3& toFloat3(const PXR_NS::GfVec3f& vec)
{
    return (const carb::Float3&) vec;
}

const carb::Float4& toFloat4(const PXR_NS::GfQuatf& vec)
{
    return (const carb::Float4&) vec;
}

const carb::Double4& toDouble4(const PXR_NS::GfVec4d& vec)
{
    return (const carb::Double4&) vec;
}

std::vector<const char*> pathToStringVector(const PXR_NS::SdfPathVector& paths)
{
...
std::vector<const char*> tokenToStringVector(const PXR_NS::TfTokenVector& tokens)
{
    std::vector<const char*> strings;
...
public:
    ...
    void addShapeData(const omni::physics::schema::ShapeDesc* desc, py::dict& outDict)
    {
        ...
        outDict["rigid_body"] = desc->rigidBody != PXR_NS::SdfPath() ? desc->rigidBody.GetText() : "";
    ...
    {
        outDict["body0"] = desc->body0 != PXR_NS::SdfPath() ? desc->body0.GetText() : "";
        outDict["body1"] = desc->body1 != PXR_NS::SdfPath() ? desc->body1.GetText() : "";
    ...
    void addAttachmentData(const omni::physics::schema::AttachmentDesc* attachmentDesc, py::dict& outDict)
    {
        ...
        outDict["src0"] = attachmentDesc->src0 != PXR_NS::SdfPath() ? attachmentDesc->src0.GetText() : "";
        outDict["src1"] = attachmentDesc->src1 != PXR_NS::SdfPath() ? attachmentDesc->src1.GetText() : "";
    ...
    void addCollisionFilterData(const omni::physics::schema::ElementCollisionFilterDesc* desc, py::dict& outDict)
    {
        ...
        outDict["src0"] = desc->src0 != PXR_NS::SdfPath() ? desc->src0.GetText() : "";
        outDict["src1"] = desc->src1 != PXR_NS::SdfPath() ? desc->src1.GetText() : "";
    }

    virtual void parsePrim(const PXR_NS::UsdPrim& prim, omni::physics::schema::ObjectDesc* objectDesc, uint64_t primTypes, const PXR_NS::TfTokenVector& appliedApis)
    {
    }

    virtual void reportObjectDesc(const PXR_NS::SdfPath& path, const omni::physics::schema::ObjectDesc* objectDesc)
    {
        if (!objectDesc)
            return;
        ...
        case omni::physics::schema::ObjectType::eRigidBody:
        {
            ...
            for (const PXR_NS::SdfPath& cols  : rigidBody->collisions)
            {
...
PYBIND11_MODULE(_usdphysics, m)
{
    using namespace carb;
    using namespace omni::physics::schema;

    m.doc() = "pybind11 omni.usdphysics bindings";

    m.def("report_object_desc_callback",
        [](report_callback_function fn) {
            if (fn)
    ...
    m.def("load_from_stage",
        [](long int stageId) {
            IUsdPhysics* schemaParser = getCachedInterface<IUsdPhysics>();

            PXR_NS::UsdStageWeakPtr stage = PXR_NS::UsdUtilsStageCache::Get().Find(
                PXR_NS::UsdStageCache::Id::FromLongInt(stageId));
            if (stage)
            {
                PXR_NS::UsdGeomXformCache xfCache;
                PXR_NS::UsdPrimRange range = stage->TraverseAll();
                PrimIteratorRange primIteratorRange(range);
