...
namespace schema
{
bool checkNestedArticulationRoot(const PXR_NS::UsdStageWeakPtr stage, const PXR_NS::UsdPrim& usdPrim, const ArticulationMap& articulationMap)
{
...
ArticulationDesc* parseArticulation(const PXR_NS::UsdStageWeakPtr stage, const PXR_NS::UsdPrim& usdPrim, const ArticulationMap& articulationMap)
{
...
using ArticulationLinkMap = std::map<PXR_NS::SdfPath, ArticulationLink>;
using BodyJointMap = PXR_NS::TfHashMap<PXR_NS::SdfPath, std::vector<const JointDesc*>, PXR_NS::SdfPath::Hash>;

bool isInLinkMap(const SdfPath& path, const std::vector<std::pair<PXR_NS::SdfPath, ArticulationLinkMap>>& linkMaps)
{
...
void traverseHierarchy(const PXR_NS::UsdStageWeakPtr stage, const SdfPath& linkPath, ArticulationLinkMap& articulationLinkMap, const BodyJointMap& bodyJointMap, uint32_t& index, SdfPathVector& linkOrderVector)
{
...
PXR_NS::SdfPath isNestedArticulation(const PXR_NS::SdfPath& topPath, const ArticulationLinkMap& map)
{
    if (map.size() > 1)
    {
...
PXR_NS::SdfPath getCenterOfGraph(const ArticulationLinkMap& map, const SdfPathVector& linkOrderVector)
{
...
void finalizeArticulations(const PXR_NS::UsdStageWeakPtr stage, ArticulationMap& articulationMap, const BodyMap& bodyMap, const JointMap& jointMap)
{
    ...
    for (ArticulationMap::const_reference& it : articulationMap)
    {
        ...
        std::vector<std::pair<PXR_NS::SdfPath,ArticulationLinkMap>> articulationLinkMaps;
