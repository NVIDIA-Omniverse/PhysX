...
namespace omni
{
    namespace physics
    {
        namespace schema
        {
            ...
            {
                PXR_NS::TfToken attrName = PXR_NS::UsdSchemaRegistry::MakeMultipleApplyNameInstance(OmniPhysicsDeformableAttrTokens->multipleApplyTemplate_purposes, instanceName);
                return posePrim.GetAttribute(attrName);
            }

            PXR_NS::TfToken getPoseNameFromPurpose(const UsdPrim prim, const PXR_NS::TfToken posePurposeToken)
            {
            ...
            DeformableBodyDesc* parseDeformableBody(const UsdStageWeakPtr stage, UsdGeomXformCache& xfCache, const UsdPrim& bodyPrim, const BodyMap& bodyMap, uint64_t primTypes)
            {
                ...
                for (PXR_NS::UsdPrimRange::const_iterator it = prims.begin(); it != prims.end(); ++it)
                {
                    ...
                    if (isSim)
                    {
                        ...
                        PXR_NS::TfToken bindPoseToken = getPoseNameFromPurpose(simMeshPrim, OmniPhysicsDeformableAttrTokens->bindPose);
                    ...
                    if (isColl)
                    {
                        ...
                        PXR_NS::TfToken bindPoseToken = getPoseNameFromPurpose(prim, OmniPhysicsDeformableAttrTokens->bindPose);
                        ...
                        PXR_NS::TfToken selfCollisionFilterPoseToken = getPoseNameFromPurpose(prim, OmniPhysicsDeformableAttrTokens->selfCollisionFilterPose);
                        desc->collisionGeomSelfCollisionFilterPoseTokens.push_back(selfCollisionFilterPoseToken);

                        // filteredPairs
                        PXR_NS::SdfPathVector filteredPairs;
