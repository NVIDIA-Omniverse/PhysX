...
void PhysicsSchemaUsdLoad::reportPrimDesc(const PXR_NS::UsdPrim& prim, ObjectDesc* desc, uint64_t typeFlags, const TfTokenVector& appliedApis)
{
...
void PhysicsSchemaUsdLoad::reportObjectDesc(const PXR_NS::SdfPath& path, const ObjectDesc* desc)
{
    for (size_t i = 0; i < mListeners.size(); i++)
    {
...
uint64_t fillDescCachePrimType(const PXR_NS::TfType& primType, DescCache& descCache)
{
...
bool PhysicsSchemaUsdLoad::loadFromRange(const PXR_NS::UsdStageWeakPtr stage, UsdGeomXformCache& xfCache, PrimIteratorBase& primIterator, bool reset)
{
    bool retVal = true;
    ...
    BodyMap bodyMap;
    JointMap jointMap;
    std::vector<std::pair<PXR_NS::SdfPath, JointDesc*>> jointVector;
            ...
            else
            {
                ...
                if (apiFlags & SchemaAPIFlag::eRigidBodyAPI)
                {
                    ...
                    if (prim.IsInstanceProxy())
                    {
                        ...
                        const PXR_NS::UsdPhysicsRigidBodyAPI rbAPI(prim);
