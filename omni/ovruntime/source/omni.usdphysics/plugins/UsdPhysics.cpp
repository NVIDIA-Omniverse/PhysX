...
bool loadFromRange(const PXR_NS::UsdStageWeakPtr stage, UsdGeomXformCache& xfCache, PrimIteratorBase& range)
{
...
{
    virtual void parsePrim(const PXR_NS::UsdPrim& prim, ObjectDesc* objectDesc, uint64_t primTypes, const PXR_NS::TfTokenVector& appliedApis)
    {
    }
...
omni::physics::schema::JointDesc* parseJoint(uint64_t stageId, const PXR_NS::SdfPath& path)
{
    ...
    PXR_NS::UsdPrimRange range(usdPrim);
...
omni::physics::schema::AttachmentDesc* parseAttachment(uint64_t stageId, const PXR_NS::SdfPath& path)
{
    UsdStageWeakPtr stage = UsdUtilsStageCache::Get().Find(UsdStageCache::Id::FromLongInt((long)stageId));
    if (!stage)
        return nullptr;
...
omni::physics::schema::ElementCollisionFilterDesc* parseCollisionFilter(uint64_t stageId, const PXR_NS::SdfPath& path)
{
    UsdStageWeakPtr stage = UsdUtilsStageCache::Get().Find(UsdStageCache::Id::FromLongInt((long)stageId));
    if (!stage)
        return nullptr;
