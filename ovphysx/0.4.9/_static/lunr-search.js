...
async function getSearchTerm(term, tokenSeparator) {
  const normalizedTerm = term.replace(/"/g, "'");  // normalized quotes
  const escapedTerm = normalizedTerm.replace(/([\\\*\+\-\:])/g, "\\$1");  // escape lunr special characters
  ...
  const searchString = Array.from(escapedTerm.split(quoteSplitter))
    .map(function (e, i) {
      return e.split(tokenSeparator)
        ...
        // also retain un-wildcarded entry to bump results with exact matches
        .map(d => i % 2 == 1 ? `+${d}` : `${d} *${d}*`)
        .join(" ");
    })
    .join(" ");
...
async function buildIndex(searchData, tokenSeparator) {
  ...
  var idx = lunr(function () {
    ...
    searchData.data.forEach(function (doc) {
      let attributes = {};
      if (doc.type == "doc" ) {
        // boost results that are pages
        attributes = { boost: 3 };
      }
      else if (doc.type == "section") {
        // boost results that are sections
        attributes = { boost: 2 };
      }
      this.add(doc, attributes);
    }, this);
  });
...
async function displayResults(highlightText, searchData, searchResults) {
  ...
  searchResults
    ...
    .map(ob => ({
      uri: `${relativeRoot}${ob.data.filename}?highlight=${highlightText}#${ob.data.anchor}`,
