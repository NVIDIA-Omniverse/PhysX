...
public:
    ...
    // Returns -1 (CPU) by default. Backends where parameter data lives on the same device
    // as the simulation (Newton) override this to return getDeviceOrdinal().
    virtual int getParamDeviceOrdinal() const { return -1; }
    ...
    // Default behavior logs no-match view creation as an error because create*View returns nullptr.
    // Callers that intentionally treat empty views as valid can quiet those logs on this view.
    virtual void setNoMatchLoggingQuiet(bool) {}
    virtual bool isNoMatchLoggingQuiet() const { return false; }
