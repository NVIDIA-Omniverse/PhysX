...
/// will fire.
///
/// \note By default this covers only creations that occur after the initial stage population;
///       no notifications are sent for the objects created by the initial attach/update
///       population unless the subscriber opts in via @ref IPhysicsObjectChangeCallback::notifyInitialPopulation.
///
...
struct IPhysicsObjectChangeCallback
{
    ...
    /// Whether these callbacks should be delivered for the objects created/destroyed by the
    /// initial stage population (the attach/update traversal that builds the scene).
    ///
    /// \note Defaults to false: the initial population is not notified, because the caller
    ///       already has that state from setup. This holds for every subscriber, including
    ///       those that set @ref stopCallbackWhenSimStopped to false (e.g. ovphysx), which
    ///       otherwise opt out of the simulation-stopped gate but must still not observe the
    ///       initial population. Set to true only for the rare in-process consumer that
    ///       explicitly wants the initial-population objects streamed through these callbacks.
    bool notifyInitialPopulation = false;
};
