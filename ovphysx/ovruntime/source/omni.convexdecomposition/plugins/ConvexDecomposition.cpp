...
public:
    ...
    // Optional callback, only invoked when V-HACD runs asynchronously, to notify that the
    // background process has completed. Occurs from a different thread.
...
public:
    ...
    ~VHACDFactory(void)
    {
        VHACDInstanceMap instances;
        {
            lock_guard _lock(mVHACDInstanceMutex);
            instances.swap(mInstances);
        }
    ...
    std::mutex mVHACDInstanceMutex;
    std::atomic< VHACDHANDLE> mId{ 0 };
    VHACDInstanceMap mInstances;

};

struct ConvexDecomposition::State
{
...
ConvexDecomposition::State& ConvexDecomposition::state()
{
    return *mState;
}

const ConvexDecomposition::State& ConvexDecomposition::state() const
{
    return *mState;
}
 ...
 * @return the unique handle/id for this instance.
 */
VHACDHANDLE ConvexDecomposition::createVHACD()
{
    return state().factory.createVHACD();
}
 ...
 * @return Returns true if the V-HACD instance was safely released.
 */
bool ConvexDecomposition::releaseVHACD(VHACDHANDLE id)
{
    return state().factory.releaseVHACD(id);
}
 ...
 * @return : Returns true if the input parameters were valid and we have started the operation.
 */
bool ConvexDecomposition::beginVHACD(VHACDHANDLE id, const Parameters& p, const SimpleMesh& sourceMesh)
{
    return state().factory.beginVHACD(id, p, sourceMesh);
}

bool ConvexDecomposition::beginSphereApproximation(VHACDHANDLE id, const Parameters& p, const SimpleMesh& sourceMesh)
{
    return state().factory.beginSphereApproximation(id,p,sourceMesh);
}
 ...
 * *return : Returns true if the convex decomposition operation is not currently running.
 */
bool ConvexDecomposition::isComplete(VHACDHANDLE id)
{
    return state().factory.isComplete(id);
}
 ...
 * @return : Returns the number of convex hulls generated
 */
uint32_t ConvexDecomposition::getConvexHullCount(VHACDHANDLE id)
{
    return state().factory.getConvexHullCount(id);
}
 ...
 * @return : Returns true if the convex hull was successfully retrieved, false if the handle or index was invalid.
 */
bool ConvexDecomposition::getConvexHull(VHACDHANDLE id, uint32_t hullIndex, SimpleMesh& hullResults)
{
    return state().factory.getConvexHull(id, hullIndex, hullResults);
}

const SimpleSphere* ConvexDecomposition::getSphereApproximation(VHACDHANDLE id, uint32_t& sphereCount, bool reducedSpheres)
{
    return state().factory.getSphereApproximation(id,sphereCount,reducedSpheres);
}
/**
 * Cancels any convex decomposition that is in process.
 *
 * @return : Returns true if the handle was valid and the operation was successfully canceled.
 */
bool ConvexDecomposition::cancelVHACD(VHACDHANDLE id)
{
    return state().factory.cancelVHACD(id);
}
 ...
 */
bool ConvexDecomposition::saveOBJ(const char* fname, const SimpleMesh& sm, bool flipWindingOrder)
{
    bool ret = false;
...
/**
    ...
    * @return : Returns the number of convex hulls produced.
    */
uint32_t ConvexDecomposition::computeVHACD(VHACDHANDLE id, const Parameters& p, const SimpleMesh& sourceMesh)
{
    return state().factory.computeVHACD(id,p,sourceMesh);
}

const SimpleSphere* ConvexDecomposition::computeSphereApproximation(
