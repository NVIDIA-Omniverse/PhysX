...
public:
    ...
    virtual uint32_t mergeConvexHulls(uint32_t maxHullCount,double _overallMeshVolume) final
    {
        ...
        // Build the set of merged convex hulls; this gets modified during the merging process
        for (auto &i:mSourceConvexHulls)
        {
        ...
        // Build the initial cost matrix representing the concavity of every pair of convex hulls
        ...
        {
            SimpleMesh *sm1 = hulls[p1];
            // Iterate only hulls with a lower index (avoids duplicate pairs)
            for (size_t p2=0; p2<p1; ++p2)
            {
                SimpleMesh *sm2 = hulls[p2];
                task->mHullA = sm1;
                task->mHullB = sm2;
                task->mPthis = this;
                if ( mJobSystem )
        ...
        // Keep merging until the hull count is below maxHullCount
        bool cancel = false;
        ...
        while ( !cancel )
        {
            ...
            const double volume1 = getVolume(*sm1);
            sm1->mVolume = volume1;
            ...
            removeHull(ch1->mMeshId);
            removeHull(ch2->mMeshId);
#if SHOW_TIME
            timeCombinedHull+=timer.getElapsedSeconds();
            timer.reset();
#endif
            // The cost matrix still holds stale references to the removed hulls; these are
            // skipped/garbage-collected lazily as they're encountered later.

            // Compute the new combined hull's cost against all still-valid existing hulls
            task = mtasks;
            for (auto &i:mHulls)
            {
    ...
    // Combine the points of convex hulls A and B and compute a new convex hull over the
    // union, returned as a SimpleMesh with an inflated AABB.
    SimpleMesh *computeCombinedConvexHull(const SimpleMesh &sm1,const SimpleMesh &sm2)
    {
        uint32_t vcount = sm1.mVertexCount+sm2.mVertexCount;
        double *vertices = new double[vcount*3];
        memcpy(vertices,sm1.mVertices,sizeof(double)*3*sm1.mVertexCount);
        double *df = vertices+(sm1.mVertexCount*3);
        memcpy(df,sm2.mVertices,sizeof(double)*3*sm2.mVertexCount);
    ...
    virtual uint32_t finalizeResults(uint32_t maxHullVertices, RaycastMesh *raycastMesh, double voxelScale,bool doShrinkWrap,double vertexScale) final
    {
        ...
        // For each convex hull: shrinkwrap its vertices to the source mesh (within voxelScale),
        // then if the vertex count still exceeds maxHullVertices, quantize down to the most
        // statistically significant subset and recompute the hull.
    ...
    void performTask(MergeTask *mt)
    {
        ...
        {
            SimpleMesh *combined = computeCombinedConvexHull(*sm1, *sm2);
            double combinedVolume = getVolume(*combined);
        ...
        // Take a lock so concurrent tasks don't race on the hull pair queue
        {
    ...
    // Use the input hulls as the output directly, without any other corrections
