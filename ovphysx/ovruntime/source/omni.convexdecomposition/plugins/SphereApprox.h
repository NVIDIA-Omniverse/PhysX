...
public:
    ...
    public:
        ...
        // The KmeansPP++ init phase is CPU-intensive and by default scans *all* input points,
        // which is slow for inputs with millions of points. 'mMaximumPlusPlusCount' controls this:
        //  - <= mMaxPoints: skip KmeansPP++ init, just sample mMaxPoints points directly.
        //  - == mPointCount: full KmeansPP++ computation over all points.
        //  - in between: KmeansPP++ over a smaller subset (quality/performance compromise).
...
public:
    ...
    virtual const float* compute(const Parameters& params, uint32_t& resultPointCount) final
    {
        ...
        uint32_t count = 0;
        // Resize means to hold 3 copies: kmeans stops once the result matches either of the last
        // two iterations, and cycling pointers between the copies avoids memory copies.
        ...
        do
        {
            ...
            // Rotate pointers: current -> old, old -> old-old, old-old -> current.
...
public:
    ...
    void recomputeRepresented(void)
    {
        mOverlapCount++;
        mCulled = true;
        // Walk our voxel index map: for any voxel we own that's already been represented by a
        // previous sphere, decrement the unrepresented voxel count.
...
public:
    ...
    // Seed points: start with all 'inside' voxels plus surface voxels with at least 10 voxel
    // neighbors, then reduce them via KmeansPP++ (in randomized order) to a well-distributed,
    // safely-spaced set that avoids clumping/aliasing.
