...
public:
    ...
    {
        mVoxelMesh.addVoxels(mSurface, mVolume);
        mVoxelMesh.addVoxels(mNewSurface, mVolume);
        mVoxelMesh.buildRaycastMesh();
    }
    ...
    bool isFinished(uint32_t minVoxelSize) const
    {
        ...
        // Below this size a convex hull doesn't have enough input points for the
        // error percentage metric to be meaningful, so stop splitting.
        else if ( dx <= int32_t(minVoxelSize) && dy <= int32_t(minVoxelSize) && dz <= int32_t(minVoxelSize) )
        {
            ret = true;
        }
        else if ( mErrorPercentage < mErrorThreshold )
        {
            ret = true;
        }
    ...
    // Split on the longest axis, but not necessarily at the midpoint: search for
    // the nearest point of discontinuity on that axis instead.
    ...
    // Split in a background thread so it can run in parallel with other active convex patches.
