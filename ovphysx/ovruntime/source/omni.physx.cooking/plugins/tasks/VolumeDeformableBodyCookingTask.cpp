...
static void writeVolumeDeformableBodyData(PxDefaultMemoryOutputStream& outData,
    ...
    const uint32_t numTetsPerElement,
...
)
{
    ...
    outData.write(&numTetsPerElement, sizeof(uint32_t));
...
void readVolumeDeformableBodyData(PhysxCookingVolumeDeformableBodyData& out, const PhysxCookedDataSpan& cookedData)
{
    ...
    out.numTetsPerElement = *reinterpret_cast<const uint32_t*>(dataPtr);
    dataPtr += sizeof(uint32_t);
...
public:
            ...
            if (cookingSucceeded)
            {
                if (m_isAutoHexahedralMeshEnabled)
                {
                    ...
                    cookingSucceeded = computeVoxelTetrahedralMeshInternal(
                        ...
                        resolution, numTetsPerElement, vertexMapExtended.data(), ResultBuffer<>::allocate);
                }
                else
                {
    ...
    virtual void finalize(void) final
    {
        ...
        else
        {
            ...
            PxDefaultMemoryOutputStream& cookDataStream = *CookingTask::getCookedDataOutputStreams()[0].get();
