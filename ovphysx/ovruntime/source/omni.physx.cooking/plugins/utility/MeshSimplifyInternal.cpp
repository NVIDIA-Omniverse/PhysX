...
namespace physx
{
    ...
    {
        // The simplifier indexes its per-vertex arrays with the triangle indices without any bounds
        // check, so an out-of-range vertex index reads out of bounds and crashes (NvBug 6282055 /
        // OMPE-97085). Reject malformed input up front: report an error and return an empty mesh.
        bool validInput = srcPointsSize > 0u && srcIndicesSize > 0u && (srcIndicesSize % 3u) == 0u;
        for (uint32_t i = 0; validInput && i < srcIndicesSize; ++i)
            validInput = srcIndices[i] < srcPointsSize;

        if (!validInput)
        {
            CARB_LOG_ERROR(
                "simplifyTriangleMeshInternal: input triangle mesh is malformed (empty, index count "
                "not a multiple of 3, or a vertex index out of range); aborting simplification and "
            ...
            {
                *dstOrigToSimpVertexMapSize = 0u;
                *dstOrigToSimpVertexMap = nullptr;
            }

            if (dstSimpToOrigTriMap && dstSimpToOrigTriMapSize)
            {
                *dstSimpToOrigTriMapSize = 0u;
                *dstSimpToOrigTriMap = nullptr;
            }
            return;
        }
