...
void OmniContactReportCallback::onContactModify(PxContactModifyPair* pairs, PxU32 count)
{
    for (PxU32 i = 0; i < count; i++)
    {
        ...
        const bool s0 = (pair.shape[0]->getSimulationFilterData().word3 & CONTACT_MODIFY_SURFACE_VELOCITY) != 0;
        const bool s1 = (pair.shape[1]->getSimulationFilterData().word3 & CONTACT_MODIFY_SURFACE_VELOCITY) != 0;
        if (s0 && s1)
        {
...
void PhysXStepper::updateQuasistaticActors(const PxU32 nbActors, PxActor** activeActors)
{
    ...
    else
    {
        ...
        for (PxU32 i = 0; i < nbActors; i++)
        {
            ...
            if (isActor || isLink)
            {
                const auto& qActors = mPhysXScene->getInternalScene()->getSceneDesc().quasistaticActors;
                const auto recKey = record.mKey;
                if (qActors.find(recKey) != qActors.end())
                    clearVelocities(*pxActor);
            }
        }
    }
...
void PhysXStepper::updateParticles(int step, int numSteps)
{
    ...
    for (InternalPbdParticleSystem* internalPS : internalScene.mParticleSystems)
    {
        ...
        internalPS->mAsyncSim = (mForceAsync || mPhysXScene->getUpdateType() == eAsynchronous);
    }
}
...
void PhysXStepper::run()
{
    ...
    for (int i = 0; i < mNbSubsteps; ++i)
    {
        ...
        omniPhysX.fireStatusEventSubscriptions(eSimulationStarting);
        {
            ...
            if (!mSkipSimulation)
            {
                if (mPhysXScene->getUpdateType() == eAsynchronous)
                {
        ...
        if (mForceAsync || mPhysXScene->getUpdateType() == eAsynchronous)
        {
            if (mLastStep)
            {
...
{
    {
        // Lock only the mComplete rendezvous. Post-step event subscribers below
        // (notably eSimulationComplete) can re-enter via the public visualization
        // and query APIs, which call waitForSimulationCompletion(false) on the
        // same scene — holding this non-recursive mutex across the event firing
        // would trip carb::tasking::Mutex's "Recursion not allowed" assertion.
    ...
    if (sendPostEvents)
    {
        ...
        if (mPhysXScene && (mForceAsync || mPhysXScene->getUpdateType() == eAsynchronous))
        {
    ...
    OmniFilterCallback* filterCallback, OmniContactReportCallback* reportCallback)
{
    ...
    // Lazy GPU context creation: create the CUDA context the first time a scene
    // requests GPU, never at startup. A CPU-only process never reaches the inner
    // block (sceneWantsGpu=false or cpuMode=true). Once created the context
    // persists for the process lifetime (cleared only by releasePhysics()).
    if (!physxSetup.getCudaContextManager())
    {
        const int gpuPipelineOverride = omniPhysX.getGpuPipelineOverride();
        const bool sceneWantsGpu = (gpuPipelineOverride > 0) ||
                                   (gpuPipelineOverride == -1 && physxSceneDesc.enableGPUDynamics);
        const omni::physx::IPhysxFoundation& physxFoundation = omni::physx::foundation::getInterface();
        const bool cpuMode = physxFoundation.isCpuMode && physxFoundation.isCpuMode();
        if (sceneWantsGpu && !cpuMode && omniPhysX.isCudaLibPresent())
        {
            if (physxFoundation.cudaDeviceCheck && physxFoundation.cudaDeviceCheck())
            {
                physxSetup.setupGPU();
            }
            else
    ...
    if (!sceneDesc.cudaContextManager)
    {
        ...
        {
            // Only warn when GPU was genuinely unavailable despite being wanted.
            // When CPU mode was explicitly requested (e.g. OVPHYSX_DISABLE_GPU, device=-2),
            // the user already knows GPU is off — a warning would just be noise.
            const omni::physx::IPhysxFoundation& physxFoundation = omni::physx::foundation::getInterface();
            const bool explicitCpuMode = physxFoundation.isCpuMode && physxFoundation.isCpuMode();
            if (!explicitCpuMode)
            {
...
PhysXScene::PhysXScene(const usdparser::AttachedStage& attachedStage)
    : mAttachedStage(attachedStage), mScene(nullptr), mControllerManager(nullptr), mInternalScene(nullptr), mTimeStepsPerSeconds(60),
      mSceneUpdateType(usdparser::eSynchronous), mMaterial(nullptr),
...
PhysXScene::~PhysXScene()
{
    ...
    // Owned via new in createPhysXScene(); ~ContactReport() destroys batched vectors.
    SAFE_DELETE_SINGLE(mContactReport);
}
    ...
    const usdparser::PhysxSceneDesc& sceneDesc)
{
    ...
    }
    {
        scene->mPBDMaterial = physxSetup.getPhysics()->createPBDMaterial(sceneDesc.defaultPBDMaterialDesc.friction,
            ...
            sceneDesc.defaultPBDMaterialDesc.cohesion, 0.0f,
            0.0f, sceneDesc.defaultPBDMaterialDesc.cflCoefficient, sceneDesc.defaultPBDMaterialDesc.gravityScale);
