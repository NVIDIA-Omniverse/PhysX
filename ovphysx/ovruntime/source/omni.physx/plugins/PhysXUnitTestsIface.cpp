...
{
    BatchedContactBufferStats stats;
    stats.liveContactReportCount = ContactReport::getLiveInstanceCount();
    const PhysXSetup& physxSetup = OmniPhysX::getInstance().getPhysXSetup();
    const PhysXScenesMap& scenes = physxSetup.getPhysXScenes();
    stats.sceneCount = scenes.size();
    if (scenes.empty())
    {
        return stats;
    }

    ContactReport* contactReport = scenes.begin()->second->getContactReport();
    if (contactReport == nullptr)
    {
        return stats;
    }

    stats.headerCapacity = contactReport->getContactHeaderVector().capacity();
    stats.dataCapacity = contactReport->getContactDataVector().capacity();
    stats.dataSize = contactReport->getContactDataVector().size();
    return stats;
}
