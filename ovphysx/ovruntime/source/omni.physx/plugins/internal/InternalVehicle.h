...
public:
    struct WheelTransformManagementEntry
    {
        ...
        // Source-agnostic handles; runtime restore goes through the source or
        // the USD data-write backend as appropriate. See InternalVehicle.cpp.
        omni::physics::parse::ObjectKey wheelRootKey;
        ...
        omni::physics::parse::ObjectKey wheelRootParentXformKey;

        omni::physics::parse::ObjectKey shapeKey; // invalid if there is no collision shape
        const ::physx::PxShape* shape; // NULL if there is no collision shape
        usdparser::Axis cylinderAxis;
    };

    struct InitialControllerValues
    {
        // Source-agnostic handle of the controller prim; resolve via primFor.
...
public:
    ...
    std::vector<WheelTransformManagementEntry> mWheelTransformManagementEntries;
...
public:
    struct InitialControllerValues
    {
        // Source-agnostic handle of the wheel-controller prim; resolve via primFor.
        omni::physics::parse::ObjectKey key;
        float driveTorque;
        float brakeTorque;
        float steerAngle;
