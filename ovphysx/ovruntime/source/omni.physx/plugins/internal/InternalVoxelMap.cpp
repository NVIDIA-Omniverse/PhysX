...
namespace
{
// The voxel map stores its root as a source-agnostic ObjectKey and resolves it via
// the active AttachedStage on demand (invalid/empty when no stage is attached).
omni::physics::parse::ObjectKey keyOfPath(const PXR_NS::SdfPath& path)
{
    AttachedStage* as = UsdLoad::getUsdLoad()->getActiveAttachedStage();
    return as ? as->keyFor(path) : omni::physics::parse::ObjectKey{};
}

PXR_NS::SdfPath pathOfKey(omni::physics::parse::ObjectKey key)
{
    AttachedStage* as = UsdLoad::getUsdLoad()->getActiveAttachedStage();
...
InfiniteVoxelMap::InfiniteVoxelMap(::physx::PxScene* scene, ::PXR_NS::UsdStageRefPtr stage, const omni::physx::usdparser::InfiniteVoxelMapDesc& desc)
    :
    mScene(scene), mRootKey(keyOfPath(desc.rootPrim))
{
    ...
    *reinterpret_cast<PxGeometryType::Enum*>(&mGeoms[0]) = PxGeometryType::eINVALID; // Need 'empty' geometry?
    ...
    auto root = stage->GetPrimAtPath(desc.rootPrim);
    if (root.IsValid())
    {
        root.GetAttribute(PXR_NS::TfToken("chunkSize")).Get(&mUsdChunkSize);

        auto blocks = root.GetPrimAtPath(PXR_NS::SdfPath("BlockLib/Blocks"));
        if (blocks.IsValid())
        {
            ...
            {
                int blockType = 0;
                child.GetAttribute(PXR_NS::TfToken("blockType")).Get(&blockType);

                if (blockType == 0) continue; // The first child appears to be garbage data

                int blockSubType;
                child.GetAttribute(PXR_NS::TfToken("blockSubType")).Get(&blockSubType);
                std::string typeName;
                child.GetAttribute(PXR_NS::TfToken("typeName")).Get(&typeName);
                std::string subTypeName;
                child.GetAttribute(PXR_NS::TfToken("subTypeName")).Get(&subTypeName);
                int blockFlags;
                child.GetAttribute(PXR_NS::TfToken("blockFlags")).Get(&blockFlags);
        ...
        {
            if (!child.IsA<PXR_NS::UsdGeomPointInstancer>())
                continue;

            const PXR_NS::UsdGeomPointInstancer& instancer = (const PXR_NS::UsdGeomPointInstancer&)child;
            ...
            PXR_NS::SdfPathVector prototypes;
...
void InfiniteVoxelMap::setVoxels_USD(const Int3& regionMin, const Int3& regionMax, const Int2& type, bool update)
{
    ...
    {
        // Resolve the stage and root prim on demand (no stored USD handles). The
        // chunk-prim DefinePrim below stays direct USD — structural authoring of
        // prims that do not pre-exist can't be expressed by an ObjectKey.
        PXR_NS::UsdStageWeakPtr stage = UsdLoad::getUsdLoad()->getActiveStage();
        auto root = stage->GetPrimAtPath(pathOfKey(mRootKey));
        if (root.IsValid())
        {
            PXR_NS::VtArray<int> indices;
            PXR_NS::VtArray<PXR_NS::GfVec3f> positions;
            ...
            if (!mDirtyUsdChunks.empty())
            {
                ...
                for (Int3 cC : mDirtyUsdChunks)
                {
                    ...
                    char chunkName[64];
                    snprintf(chunkName, sizeof(chunkName), "Chunk_%s%d%s%d%s%d", (cC.x < 0 ? "N" : "P"), (cC.x < 0 ? -cC.x : cC.x),
                                                               (cC.y < 0 ? "N" : "P"), (cC.y < 0 ? -cC.y : cC.y),
                                                               (cC.z < 0 ? "N" : "P"), (cC.z < 0 ? -cC.z : cC.z));

                    auto chunkPath = root.GetPath().AppendChild(PXR_NS::TfToken(chunkName));

                    auto chunk = stage->GetPrimAtPath(chunkPath);

                    if (!chunk.IsValid() && chunkHasBlocks)
                        chunk = stage->DefinePrim(chunkPath, PXR_NS::TfToken("PointInstancer"));
            ...
            if (!updateList.empty())
            {
                ...
                {
                    auto chunk = stage->GetPrimAtPath(item.chunkPath);
                    PXR_NS::UsdGeomPointInstancer& instancer = (PXR_NS::UsdGeomPointInstancer&)chunk;
...
void InfiniteVoxelMap::collectChildMeshes(const PXR_NS::UsdPrim& prim, std::vector<PxVec3>& vertices, std::vector<PxU32>& indices)
{
    for (const auto& child : prim.GetChildren())
    {
        if (child.IsA<PXR_NS::UsdGeomMesh>())
        {
            const auto& mesh = (const PXR_NS::UsdGeomMesh&)child;
...
void InfiniteVoxelMap::queryBlockRegion(const carb::Int3& s, const carb::Int3& e, PXR_NS::VtArray<int>& indices, PXR_NS::VtArray<PXR_NS::GfVec3f>& positions, PXR_NS::SdfPathVector& prototypes) const
{
                ...
                for (int bx = bMi.x; bx <= bMa.x; ++bx)
                    for (int by = bMi.y; by <= bMa.y; ++by)
                        for (int bz = bMi.z; bz <= bMa.z; ++bz)
                        {
                            ...
                            positions.push_back(PXR_NS::GfVec3f((float)bx, (float)by, (float)bz));
                        }
            }
    ...
    for (PxU32 t : types)
    {
        ...
        int subType = (t & 0xff);
        char blockPath[64];
        snprintf(blockPath, sizeof(blockPath), "../BlockLib/Blocks/Block_%d_%d", type, subType);
        prototypes.push_back(PXR_NS::SdfPath(blockPath));
    }
}
...
Int2 InfiniteVoxelMap::VoxelChunk::voxel(const Int3& coords) const
{
    ...
    return Int2 { static_cast<int32_t>(data >> 16), static_cast<int32_t>(data & 0xffff) };
}
