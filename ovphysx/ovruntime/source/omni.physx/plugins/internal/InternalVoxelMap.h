...
public:
    ...
    InfiniteVoxelMap(::physx::PxScene* scene, ::PXR_NS::UsdStageRefPtr stage, const usdparser::InfiniteVoxelMapDesc& desc);
    ~InfiniteVoxelMap();
...
private:
    ...
    void collectChildMeshes(const PXR_NS::UsdPrim& prim,
    ...
    void queryBlockRegion(const carb::Int3& s,
                          ...
                          PXR_NS::SdfPathVector& prototypes) const;
    ...
    // Source-agnostic handle of the voxel-map root prim; resolve to an SdfPath
    // on demand via AttachedStage::pathFor. The stage itself is resolved on
    // demand too (no stored UsdStage). Runtime chunk authoring (DefinePrim) stays
    // direct USD — structural authoring residue, see InternalVoxelMap.cpp.
    const omni::physics::parse::ObjectKey mRootKey;
    PXR_NS::GfVec3i mUsdChunkSize;
