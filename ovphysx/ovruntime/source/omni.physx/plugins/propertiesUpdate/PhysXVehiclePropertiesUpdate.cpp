    ...
    const PXR_NS::TfToken& property, const PXR_NS::UsdTimeCode& timeCode, const EngineParam::Enum engineParam)
{
    ...
    float value;
    if (!getValue<float>(attachedStage, objectRecord->mKey, property, timeCode, value))
        return true;
    ...
    T& value, InternalVehicleWheelReferenceList*& vehicleWheelRefList)
{
    ...
    if (!getValue<T>(attachedStage, objectRecord->mKey, property, timeCode, value))
        return false;
...
static bool updateVehicleWheelAttachmentData(InternalVehicleWheelAttachment* wheelAttachment,
    const PxVec3* valueFloat3, const bool* valueBool, const ObjectId* valueCollisionGroupId, const PxQuat* valueQuat,
    const WheelAttachmentParam::Enum wheelAttachmentParam)
{
    ...
    if (vehicle)
    {
        ...
        {
            PxFilterData fd;
            if (valueCollisionGroupId)
            {
                const uint32_t collisionGroup = convertToCollisionGroup(*valueCollisionGroupId);
                convertCollisionGroupToPxFilterData(collisionGroup, fd);
            }
            // valueCollisionGroupId being nullptr indicates that the collision group has been removed and thus
