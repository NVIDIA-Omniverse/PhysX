...
// Scatter compacted disabled-body pose patches into dst at their (non-contiguous)
// body indices: dst[indices[k]] = patches[k]. Batches the disabled-body pose
// readback patch into one HtoD + one kernel instead of a per-body cudaMemcpy in
// the read hot path (OMPE-94459).
