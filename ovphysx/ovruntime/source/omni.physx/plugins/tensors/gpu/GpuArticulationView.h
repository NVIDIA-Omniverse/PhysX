...
public:
    ...
    // BaseArticulationView's impls hardcode CPU device (per-shape PxShape calls
    // are CPU-only API); these overrides stage GPU tensors to host buffers and
    // forward to the base impl so the umbrella's GPU-mode test can drive them.
    ...
    // setDisableGravities: staged CPU flag write; GPU link refresh via eDIRTY_LINKS
    // in postActorFlagChange (see BaseArticulationView::setDisableGravities).
    // Stage GPU uint8 src + int32 index to host before delegating.
    bool setDisableGravities(const TensorDesc* srcTensor, const TensorDesc* indexTensor) override;
    // Read companion: write into a host buffer then memcpy to caller GPU dst.
