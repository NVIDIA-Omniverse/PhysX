...
#
# test_benchmark_contract.cmake runs the benchmark binary with --regenerate,
# which makes BmOutput's destructor rewrite the shared developer baseline at
# <app dir>/../data/benchmarkData/_baseline.txt with only the rows that narrow
# run selected. The contract therefore has to put the developer's file back.
#
# An in-memory "did it exist?" flag is not enough. If the driving cmake process
# dies after the benchmark child has already replaced the baseline, the flag
# dies with it, and the next invocation would snapshot the narrow leftover on
# top of the only surviving copy of the developer file.
#
# The transaction state therefore lives on disk:
#
...
#             the next begin() re-snapshots the untouched target.
#   commit()  restores, checks the result against the recorded hash, then
#             removes the marker BEFORE the backup. Dying in between leaves a
#             marker whose backup is still intact, so recovery repeats
...
#
# scripts/test_benchmark_baseline_txn.cmake exercises all of this against a
...
# Parses the marker into OUT_STATE ("EXISTED", "ABSENT", or "" when there is no
