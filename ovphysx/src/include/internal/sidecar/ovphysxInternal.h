...
// Sidecar lifecycle entry points (libovphysx_internal): the version probe the
// SDK loader uses as a handshake after dlopen/LoadLibrary, and the setters that
// inject the owner's runtime accessors.
...
// 3 -> 4: the SDK loader seeds the sidecar's framework and OmniCore built-ins once at load via
...
// 5 -> 6: injected owner-side PhysX runtime accessors. The sidecar no longer
...
// 7 -> 8: injected the owner-side IPhysxVisualization accessor. The sidecar no longer
...
// 10 -> 11: removed the USD-linked SdfPath encoder and UsdUtilsStageCache erase exports.
//           The sidecar no longer links OpenUSD.
#define OVPHYSX_INTERNAL_INTERFACE_VERSION 11
