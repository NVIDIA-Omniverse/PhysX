...
#ifndef PX_VEHICLE_PVD_COMPONENTS_H
#define PX_VEHICLE_PVD_COMPONENTS_H
...
protected:
	...
	/**
	\brief Rearm vehicle registration for a newly bound OmniPVD writer session.

	Call this after replacing the vehicle's object handles with handles created for the new session.
	The next update registers every vehicle object before writing its current values.
	*/
	PX_FORCE_INLINE void resetPvdRegistration()
	{
		firstTime = true;
	}
