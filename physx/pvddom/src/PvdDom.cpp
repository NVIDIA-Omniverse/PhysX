...
OmniPvdObject::OmniPvdObject()
{
    ...
    mIsSupersedeRecreate = false;
    mRecordingSegmentId = 0;
...
OmniPvdDOMState::~OmniPvdDOMState()
{
    ...
    // Classes with an OVD-registered handle are owned by this map. The named
    // internal classes above use createInternalClass() with handle 0 and are
    // never inserted here, so there's no double-free risk.
    for (auto& it : mClassHandleToClassMap)
    {
        delete it.second;
    }
