...
namespace Gu
{
	...
	void analyzeAndFixMesh(const PxVec3* vertices, const PxU32* indicesOrig, PxU32 numTriangleIndices, PxArray<PxU32>& repairedIndices)
	{
		...
		if (success)
		{
			// Normalize triangle winding to consistent + outward (positive signed volume) for any watertight,
			// single-component mesh, not only when duplicate triangles were removed. The SDF's inside/outside
			// classification is winding-sensitive, so a clean but inverted-wound mesh would otherwise produce
			// an inverted SDF. The triangle-mesh cook already normalizes the stored mesh winding (see
			// TriangleMeshBuilder::normalizeSDFCollisionMeshWinding); this repair covers SDFs built from other
			// inputs as well (e.g. PxSDFDesc::baseMesh). Feeds both the CPU and GPU SDF construction. OMPE-93218.
			if (meshIsWatertight && connectedTriangleGroups.size() == 1)
				MeshAnalyzer::orientTrianglesOutward(triangles, repairedIndices.size() / 3, vertices, flipTriangle);
		}
		else
		{
