...
namespace internalABP
{
	struct SIMD_AABB4 : public PxUserAllocated
	{
		...
		{
#ifdef ABP_USE_INTEGER_XS
			// PT: clamped for the same reason as AABB_Xi::initFromFloats - SentinelValue is 0xffffffff in
			// this configuration too, and encodeFloat() can produce it from a NaN.
			mMinX = PxMin(encodeFloat(PX_IR(box.minimum.x)), gMaxEncodedCoord);
			mMaxX = PxMin(encodeFloat(PX_IR(box.maximum.x)), gMaxEncodedCoord);
			mMinY = box.minimum.y;
			mMinZ = box.minimum.z;
			mMaxY = box.maximum.y;
			mMaxZ = box.maximum.z;
...
void BroadPhaseABP::update(PxU32 numCpuTasks, Cm::FlushPool* flushPool, PxcScratchAllocator* scratchAllocator, const BroadPhaseUpdateData& updateData, PxBaseTask* continuation)
{
