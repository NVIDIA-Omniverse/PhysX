...
struct SolverIslandParams
{
	...
	//PT: each counter is padded out to its own cache line: they are all incremented atomically and
	//spin-read (WAIT_FOR_PROGRESS) by every solver thread, and previously shared cache lines both with
	//each other and with the read-only data above/below, i.e. false sharing on the solver's hottest
	//lines. Explicit padding rather than PX_ALIGN because the struct is allocated from the flush pool
	//with 16-byte alignment only.
	PxU8 pad0[64];
	PxI32 constraintIndex;			PxU8 pad1[60];
	PxI32 constraintIndexCompleted;	PxU8 pad2[60];
	PxI32 bodyListIndex;			PxU8 pad3[60];
	PxI32 bodyListIndexCompleted;	PxU8 pad4[60];
	PxI32 articSolveIndex;			PxU8 pad5[60];
	PxI32 articSolveIndexCompleted;	PxU8 pad6[60];
	PxI32 bodyIntegrationListIndex;	PxU8 pad7[60];
	PxI32 numObjectsIntegrated;		PxU8 pad8[60];
