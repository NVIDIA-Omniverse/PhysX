...
fn main() {
    ...
    if native_source_build_supported(&target) && bundled_native_root.exists() {
        build_bundled_native_backend(&bundled_native_root);
        return;
    }

    let supported = supported_targets(manifest_dir.join("artifacts"));
    let native_help = if bundled_native_root.exists() {
    ...
    if force_source_build {
        panic!(
            "blast-stress-solver source build was forced, but target {} has no bundled native source fallback.{}",
            target, native_help
        );
    }
...
fn build_bundled_native_backend(blast_root: &Path) {
